...
import {CustomError, DateOnly, DateTime, JsonConvert, Time, Uuid} from "@simplysm/sd-core-common";

describe("core.util.JsonConvert", () => {
  ...
  it("문자열로 변환시, replacer 옵션을 통해 변환 방식을 변경할 수 있다. 이 경우, replacer 가 먼저 동작한 후, 그 결과값을 바탕으로 JSON 의 기본 파싱이 이루어 진다.", () => {
    ...
    const json = JsonConvert.stringify(obj, {
      replacer: (key, item) => {
        return item instanceof DateTime ? "!!!" : item;
      }
    });
