...
describe("(node) core.FsWatcher", () => {
  ...
  beforeEach(async () => {
    await FsUtil.removeAsync(testDirPath);
    await FsUtil.mkdirsAsync(testDirPath);
  });
  ...
  it("추가/수정/삭제 3가지에 대해 파일 혹은 디렉토리의 변경을 감지할 수 있다. (이때 감지위치 설정은 '../**/*' 방식이다)", async () => {
    ...
    let result: any;
    const watcher = await FsUtil.watchAsync(path.resolve(testDirPath, "**", "*.txt"), (changedInfos) => {
    ...
    await FsUtil.writeFileAsync(testFile1Path, "1234");
  ...
  it("최초 실행시에는 변경이벤트가 발생하지 않는다.", async () => {
    const watcher = await FsUtil.watchAsync(path.resolve(testDirPath, "**", "*.txt"), () => {
      expect.fail();
    }, (err) => {
  ...
  it("여러 감지 결과가 통합되어, 이벤트가 한번에 발생한다.", async () => {
    ...
    await FsUtil.writeFileAsync(testFile1Path, "1234");
    await FsUtil.writeFileAsync(testFile2Path, "1234");
