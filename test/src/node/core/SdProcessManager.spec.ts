...
describe("(node) core.SdProcessManager", () => {
  it("특정 프로세스를 비동기식으로 실행할 수 있다.", async () => {
    const result = await SdProcessManager.spawnAsync("echo %cd%");
  ...
  it("일반 메시지를 위한 messageHandler 와 오류 메시지를 위한 errorMessageHandler 를 설정할 수 있으며, true 를 반환시켜, 프로세스를 강제 종료 시킬 수 있다.", async () => {
    ...
    await SdProcessManager.spawnAsync(
      "node ./SdProcessManagerTestDir/test.js",
      { cwd: __dirname },
      (message) => {
  ...
  it("env 옵션을 통해 해당 프로세스만을 위한 환경변수를 설정할 수 있다.", async () => {
    const result = await SdProcessManager.spawnAsync(
      "node ./SdProcessManagerTestDir/env-test.js",
      {
        cwd: __dirname,
        env: {
          TEST: "!!!"
