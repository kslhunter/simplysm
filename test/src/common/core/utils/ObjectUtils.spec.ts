...
describe("(common) core.utils.ObjectUtils", () => {
  ...
  describe("equal", () => {
    ...
    it("string, number 등의 기본 타입이 입력되도 정상 작동한다.", () => {
      expect(ObjectUtils.equal(1, 1)).to.equal(true);
      expect(ObjectUtils.equal("1", "1")).to.equal(true);
      expect(ObjectUtils.equal(1, "1")).to.equal(false);
    });
    ...
    it("순환 Object 의 경우, 순환오류가 발생된다.", () => {
      ...
      if (process.versions.node !== undefined || navigator.userAgent.includes("Chrome")) {
        expect(() => ObjectUtils.equal(obj1, obj2)).to.be.throw(RangeError);
      }
      else {
        expect(() => ObjectUtils.equal(obj1, obj2)).to.be.throw(/스택 공간 부족/);
      }
    });
    ...
    it("Date, DateTime, DateOnly, Time, Uuid 가 있어도 정상 동작한다.", () => {
      ...
      const obj2 = ObjectUtils.clone(obj1);
    ...
    it("(array) 정상 작동 한다(배열내 객체의 순서가 다르면, 내용물이 같더라도 다른것으로 인식된다)", () => {
      ...
      expect(ObjectUtils.equal(arr1, arr4)).to.equal(false);
      expect(ObjectUtils.equal(arr1, arr5)).to.equal(false);
    });
  ...
  describe("validate", () => {
    it("값의 유효성을 확인하여, 그 결과를 반환한다", () => {
      ...
      const validator = (item: any): boolean => item !== undefined;
      expect(
        ObjectUtils.validate(undefined as any, {
          type: String,
          includes: ["1", "2"],
          notnull: true,
          validator
  ...
  describe("validateArray", () => {
    it("Array<Object> 의 내용물(Object)에 대해, Object 의 각 키별 유효성을 확인한다. 오류가 아닌 반환값을 사용한다.", () => {
      ...
      expect(
        ObjectUtils.validateArray(arr, () => ({
          a: {
            type: Number,
            includes: [1, 2],
            notnull: false,
            validator: (item1): boolean => item1 !== 3
          },
          b: {
            type: String,
            includes: ["1", "2"],
  ...
  describe("getChainValueByDepth", () => {
    it("key 와 depth 를 이용하여 Hierarchical 구조의 Object 에서 데이터 추출, child 와 같은 특정키로, 특정 깊이까지 내려가는 데이터에 대한 값을 추출한다.", () => {
      ...
      expect(
        ObjectUtils.getChainValueByDepth(obj, "b", 3)
      ).to.deep.equal({
        a: 4
      });
    });
  ...
  describe("getChainValue", () => {
    it("chainKey(aaa.bbb.ccc) 를 이용하여 Hierarchical 구조의 Object 에서 데이터 추출, 키 체인을 문자열로 제공받아 Object 에서 내부의 데이터를 가져오려 할때 사용할 수 있다.", () => {
      ...
      expect(ObjectUtils.getChainValue(obj, "a.b.c")).to.equal(1);
    });
