...
describe("(common) core.types.DateTime", () => {
  ...
  it("[Fix] 3월 31일에 1달전의 데이터를 가져오면 2월 마지막날이 아닌 다른날이 나타나는 현상", () => {
    const d1 = new DateTime(2020, 3, 3, 1).addMonths(-1);
    expect(d1.toString()).to.equal("2020-02-03T01:00:00.000+09:00");

    const d2 = new DateTime(2020, 3, 31, 1).addMonths(-1);
    expect(d2.toString()).to.equal("2020-02-29T01:00:00.000+09:00");
  });
});