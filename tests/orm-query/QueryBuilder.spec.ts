...
describe("QueryBuilder", () => {
  it("[SELECT] 기본 쿼리", () => {
    const query = new QueryBuilder()
      .from("SD_TEST_ORM_QUERY.dbo.Employee")
      .query;

    assert.strictEqual(query, `
SELECT
  *
FROM SD_TEST_ORM_QUERY.dbo.Employee`.trim());

  });

  it("[SELECT] 'FROM'에 'AS'를 설정할 수 있음", () => {
    const query = new QueryBuilder()
      .from("SD_TEST_ORM_QUERY.dbo.Employee", "emp")
      .query;

    assert.strictEqual(query, `
SELECT
  *
FROM SD_TEST_ORM_QUERY.dbo.Employee AS emp`.trim());
  });
...
SELECT
  *
FROM SD_TEST_ORM_QUERY.dbo.Employee AS emp
WHERE (emp.id = 1)
...
FROM (
  ...
  it("[SELECT] 'LEFT OUTER JOIN'할 수 있음", () => {
    const query = new QueryBuilder()
      ...
      .join(
        new QueryBuilder()
          .from("SD_TEST_ORM_QUERY.dbo.Company", "cmp")
...
SELECT
  *
FROM SD_TEST_ORM_QUERY.dbo.Employee AS emp
LEFT OUTER JOIN SD_TEST_ORM_QUERY.dbo.Company AS cmp ON (cmp.id = emp.companyId)`.trim());
  });
...
WHERE (emp.id = 1)`.trim());
  ...
  it("[UPDATE] 'LEFT OUTER JOIN'할 수 있음", () => {
    const query = new QueryBuilder()
      .from("SD_TEST_ORM_QUERY.dbo.Employee", "emp")
      .join(new QueryBuilder().from("SD_TEST_ORM_QUERY.dbo.Company", "cmp").where("cmp.id = emp.companyId"))
      .where("emp.id = 1")
      .update({
        name: "'관리자'"
      })
...
UPDATE SD_TEST_ORM_QUERY.dbo.Employee SET
  name = '관리자'
FROM SD_TEST_ORM_QUERY.dbo.Employee AS emp
LEFT OUTER JOIN SD_TEST_ORM_QUERY.dbo.Company AS cmp ON (cmp.id = emp.companyId)
WHERE (emp.id = 1)`.trim());
  });
  ...
  it("[UPDATE] 'OUTER APPLY'할 수 있음", () => {
    const query = new QueryBuilder()
      .from("SD_TEST_ORM_QUERY.dbo.Employee", "emp")
      .join(new QueryBuilder().from("SD_TEST_ORM_QUERY.dbo.Company", "cmp").select({id: "cmp.id"}).where("cmp.id = emp.companyId"))
      .where("emp.id = 1")
      .update({
        name: "'관리자'"
      })
...
VALUES (1, '관리자', 'c3018f1e248c3182a5196f20cb2c64507e51f0241c734a21a3bba79d3ba5cd84', 0)`.trim());
  ...
  it("[UPSERT] 기본쿼리", () => {
    ...
    assert.strictEqual(query, `
MERGE SD_TEST_ORM_QUERY.dbo.Employee
...
WHEN NOT MATCHED THEN
  ...
  it("[UPSERT] 'FROM'에 'AS'를 사용할 수 있음", () => {
    ...
    assert.strictEqual(query, `
MERGE SD_TEST_ORM_QUERY.dbo.Employee AS emp
USING (SELECT 0 as _using) AS _using
ON (emp.name = '관리자?')
WHEN MATCHED THEN
  UPDATE SET
