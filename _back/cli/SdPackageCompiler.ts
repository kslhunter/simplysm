...
export class SdPackageCompiler extends events.EventEmitter {
      ...
      webpackConfig.module.rules.push(
        {
          ...
          exclude: /node_modules/,
          loaders: [
            require.resolve("./ts-build-loader"),
  ...
  public async runAsync(): Promise<void> {
    ...
    if (config.type !== "library" && config.type !== "server") {
      ...
      if (config.framework === "vue") {
        ...
        this._mergeStyleConfigs(webpackConfig, {sourceMap: false, extract: false, vue: false});
      }
    }
  ...
  public async watchAsync(): Promise<NextHandleFunction[]> {
    ...
    webpackConfig.optimization!.removeAvailableModules = true;
    ...
    if (config.type === "library" || config.type === "server") {
      ...
      if (config.framework === "vue") {
        ...
        throw new Error("프레임워크를 설정해야 합니다.");
      }
