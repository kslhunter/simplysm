  ...
  public async buildAsync(argv: { watch: boolean; options?: string }): Promise<void> {
    ...
    /*await this._parallelPackagesByDepAsync(Object.keys(config.packages), async packageKey => {
      ...
      if (config.packages[packageKey].type === "library" || config.packages[packageKey].type === "server") {
        ...
        const workers = await Promise.all(
          [
            this._runWorkerAsync(`${prefix}-compile`, packageKey, config.packages[packageKey].framework === "angular" ? "scss" : undefined),
            this._runWorkerAsync(`${prefix}-check`, packageKey, config.packages[packageKey].framework === "angular" ? "scss" : undefined),
            this._runWorkerAsync(`${prefix}-lint`, packageKey),
            config.packages[packageKey].framework === "angular" ? this._runWorkerAsync(`${prefix}-metadata`, packageKey) : undefined
          ].filterExists()
        );

        workerCpuUsages.pushRange(
          workers
            .filter(item => item.cpuUsage)
            .map(worker => ({packageKey, type: worker.workerName, cpuUsage: worker.cpuUsage!}))
        );

        // 변경감지 모드이며, 서버빌드 일때, 서버 실행 (변경감지 포함)
        if (argv.watch && config.packages[packageKey].type === "server") {
          await this._runServerAsync(packageKey, workers[0]);
        }
      }
      else {
        if (!argv.watch) {
          ...
          const workers = await Promise.all(
            [
              this._runPackageBuildWorkerAsync("build", packageKey, argv.options, false),
              this._runPackageBuildWorkerAsync("check", packageKey, argv.options, argv.watch)
            ].filterExists()
          );

          workerCpuUsages.pushRange(
            workers
              .map((wk, i) => ({packageKey, type: i === 0 ? "build" : "check", cpuUsage: wk["cpuUsage"]}))
          );
        }
        else {
          // > > 빌드
    ...
    await Promise.all([
      this._parallelPackagesByDepAsync(packageKeys, async (packageKey, packageName, projectOwnDepPackageNames) => {
        ...
        if (config.packages[packageKey].type === "library" || config.packages[packageKey].type === "server") {
          const worker = await this._runWorkerAsync(`${prefix}-compile`, packageKey, config.packages[packageKey].framework === "angular" ? "scss" : undefined);
      ...
      Promise.all(packageKeys.map(async packageKey => {
        ...
        const worker = await this._runWorkerAsync(`${prefix}-metadata`, packageKey);
    ...
    const wk = await new Promise<child_process.ChildProcess>((resolve, reject) => {
      ...
      const worker = child_process.fork(
        ...
        [type, packageKey, watch ? "watch" : "build", ...(options ? [options] : [])],
        {
          stdio: ["inherit", "inherit", "inherit", "ipc"],
          cwd: process.cwd(),
          execArgv: [...execArgv, "--max-old-space-size=4096"]
        }
      );
