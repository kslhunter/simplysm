...
import { SdModalProvider } from "../modal/SdModalProvider";
...
export class SdSheetControl implements DoCheck, OnInit, AfterContentChecked {
  ...
  public async ngOnInit(): Promise<void> {
    this._zone.runOutsideAngular(() => {
      const headEl = this._el.findFirst("> dock-container > pane > ._sheet > ._head")!;
      const bodyEl = this._el.findFirst("> dock-container > pane > ._sheet > ._body")!;

      const paneEl = this._el.findFirst("> dock-container > pane")!;
      ...
      this._el.addEventListener("focus", (event) => {
        ...
        if (
          ...
          const rowEls = this._el.findAll("> dock-container > pane > ._sheet > ._body > ._row");
          const rowEl = event.target.findParent("._row")!;
