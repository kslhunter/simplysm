...
export class SdWebpackUtil {
  public static getWebpackResults(err?: Error, stats?: webpack.Stats): ISdPackageBuildResult[] {
    if (err != null) {
      return [{
        type: "compile",
        filePath: undefined,
    ...
    if (stats.hasWarnings() && info.warnings) {
      results.push(
        ...info.warnings.map((item) => ({
          ...
          message: item.message.startsWith("(undefined)") ? item.message.split("\n").slice(1).join(os.EOL) : item.message
        }))
      );
    }
