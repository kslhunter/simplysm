    ...
    return {
      ...watch ? {
        ...
        resolve: {
          ...
          fallback: this._config.resolveFallback
            ? Object.keys(this._config.resolveFallback).map((key) => ({
              key,
              val: require.resolve(this._config.resolveFallback![key])
            })).toObject((item) => item.key, (item) => item.val)
            : undefined
        }
      } : {
        mode: "production",
      ...
      ...this._platform.type === "windows" ? {
        externals: [
          (data, callback): void => {
            if (externalModuleNames.includes(data.request!)) {
            ...
            if (data.request === "fsevents") {
              callback(undefined, `commonjs ${data.request}`);
              return;
            }
