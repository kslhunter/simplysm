import React, { Dispatch, MutableRefObject, SetStateAction, useCallback, useEffect, useMemo, useReducer, useRef } from "react";
...
export function useForceUpdateRef(): MutableRefObject<() => void> {
...
function runWithRootForceUpdateRef<R>(rootForceUpdateRef: MutableRefObject<() => void> | undefined, fn: (forceUpdateRef: MutableRefObject<() => void>) => R): R {
  if (rootForceUpdateRef) {
    return fn(rootForceUpdateRef);
  }
  else {
    const forceUpdate = useReducer((x) => x + 1, 0)[1];
    const forceUpdateRef = useRef(forceUpdate);
