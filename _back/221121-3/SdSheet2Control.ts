...
export class SdSheet2Control<T> implements OnInit, AfterViewChecked, OnChanges {
  public icons = {
   ...
   * 화면에 표시할 컬럼 컨트롤 목록
   */
  public get displayColumnDefs(): IColumnDef[] | undefined {
    return this.columnControls?.toArray()
      .map((item) => {
  ...
  public get hasSelectableItems(): boolean {
    ...
    return this.displayItemDefs.some((item) => this.getItemSelectable(item));
  }
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if ("items" in changes || "page" in changes) {
