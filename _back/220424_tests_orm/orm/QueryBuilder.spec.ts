...
IF NOT EXISTS(SELECT * FROM sys.databases WHERE name = 'TEST_DB') CREATE DATABASE [TEST_DB] COLLATE Korean_Wansung_CS_AS`.trim());
    });
...
BEGIN
  ...
  -- 프록시저 초기화
  SELECT @sql = @sql + 'DROP PROCEDURE [TEST_DB].' + QUOTENAME(sch.name) + '.' + QUOTENAME(o.name) +';' + CHAR(13) + CHAR(10)
  FROM [TEST_DB].sys.sql_modules m
  INNER JOIN [TEST_DB].sys.objects o ON m.object_id=o.object_id
  INNER JOIN [TEST_DB].sys.schemas sch ON sch.schema_id = [o].schema_id
  WHERE type_desc like '%PROCEDURE%'
    
  -- 함수 초기화
  SELECT @sql = @sql + 'DROP FUNCTION [TEST_DB].' + QUOTENAME(sch.name) + '.' + QUOTENAME(o.name) + N';' + CHAR(13) + CHAR(10)
  FROM [TEST_DB].sys.sql_modules m
  INNER JOIN [TEST_DB].sys.objects o ON m.object_id=o.object_id
  ...
  -- 뷰 초기화
  SELECT @sql = @sql + 'DROP VIEW [TEST_DB].' + QUOTENAME(sch.name) + '.' + QUOTENAME(v.name) + N';' + CHAR(13) + CHAR(10)
  FROM [TEST_DB].sys.views v
  INNER JOIN [TEST_DB].sys.schemas sch ON sch.schema_id = [o].schema_id
  WHERE sch.name <> 'sys'
    
  -- 테이블 FK 끊기 초기화
  SELECT @sql = @sql + N'ALTER TABLE [TEST_DB].' + QUOTENAME(sch.name) + '.' + QUOTENAME([tbl].[name]) + N' DROP CONSTRAINT ' + QUOTENAME([obj].[name]) + N';' + CHAR(13) + CHAR(10)
  ...
  -- 테이블 삭제
  SELECT @sql = @sql + N'DROP TABLE [TEST_DB].' + QUOTENAME(sch.name) + '.' + QUOTENAME([tbl].[name]) + N';' + CHAR(13) + CHAR(10)
  FROM [TEST_DB].sys.tables [tbl]
  INNER JOIN [NEOE].sys.schemas sch ON sch.schema_id = [tbl].schema_id
  WHERE [type]= 'U'
...
CREATE INDEX [IDX_TEST_SCHEMA_TEST_TABLE_test] ON [TEST_DB].[TEST_SCHEMA].[TEST_TABLE] ([test1] ASC, [test2] DESC)`.trim());
    });
