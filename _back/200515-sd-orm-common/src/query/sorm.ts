...
export const sorm = {
  ...
  cast<T extends TQueryValue>(db: DbContext, src: TEntityValue<TQueryValue>, targetType: Type<T>): QueryUnit<StripTypeWrap<T>, TEntityValueOrQueryableArray> {
    return new QueryUnit(targetType, ["CONVERT(", new QueryBuilder(db.dialect).type(targetType), ", ", QueryUtils.getQueryValue(src), ")"]) as QueryUnit<StripTypeWrap<T>, TEntityValueOrQueryableArray>;
  },
  ...
  concat(...args: TEntityValue<string | String>[]): QueryUnit<string, TEntityValueOrQueryableArray> {
    return new QueryUnit(String, [
      "CONCAT(",
      ...args.mapMany(arg => [QueryUtils.getQueryValue(arg), ", "]).slice(0, -1),
      ")"
    ]) as QueryUnit<string, TEntityValueOrQueryableArray>;
  },
  //endregion
