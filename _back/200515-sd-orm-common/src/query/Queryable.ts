...
export class Queryable<D extends DbContext, T> {
  ...
  public constructor(private readonly _db: D, arg1?: Queryable<D, T> | Type<T>, arg2?: string | TEntity<T>, arg3?: TEntity<T>, arg4?: IQueryableDef) {
    ...
    // 일반 생성
      ...
      for (const colDef of this._tableDef.columns) {
        this._entity[colDef.propertyKey] = new QueryUnit(colDef.typeFwd(), `${this._db.qb.wrap(`TBL${this._as !== undefined ? `.${this._as}` : ""}`)}.${qb.wrap(colDef.name)}`);
      }
  ...
  public static union<D extends DbContext, T>(qrs: Queryable<D, T>[], as?: string): Queryable<D, T> {
    ...
    for (const entityKey of Object.keys(cqrs[0]._entity)) {
      ...
      entity[entityKey] = new QueryUnit(QueryUtils.getQueryValueType(entityValue), `${this._db.qb.wrap(`TBL${as !== undefined ? "." + as : ""}`)}.${this._db.qb.wrap(entityKey)}`);
    }
  ...
  public join<A extends string, J, R>(joinTypeOrQrs: Type<J> | Queryable<D, J>[], as: A, fwd: (queryable: Queryable<D, J>, entity: TEntity<T>) => Queryable<D, R>, isSingle?: boolean): Queryable<D, T & { [K in A]: R | R[] }> {
    const qb = new QueryBuilder(this._db?.dialect);
    if (this._def.join?.some(item => item.as === qb.wrap(`TBL.${as}`))) {
  ...
  public getSelectDef(): ISelectQueryDef {
    ...
    // AS 구성
    result.as = qb.wrap(`TBL${this._as !== undefined ? `.${this._as}` : ""}`);
    ...
    const addSelectValue = (key: string, value: any): void => {
      if (QueryUtils.canGetQueryValue(value)) {
        ...
        if (!result.select) throw new NotImplementError();
        result.select[`${qb.wrap(key)}`] = QueryUtils.getQueryValue(value);
      }
  ...
  private _getParentEntity<P>(fromEntity: TEntity<P>, rootAs: string | undefined, parentAs: string | undefined): TEntity<P> {
    ...
    for (const key of Object.keys(fromEntity)) {
      ...
      if (QueryUtils.canGetQueryValue(entityValue)) {
        const qb = new QueryBuilder(this._db?.dialect);
        result[key] = new QueryUnit(QueryUtils.getQueryValueType(entityValue), qb.wrap("TBL" + (rootAs !== undefined ? "." + rootAs : "")) + "." + qb.wrap((parentAs !== undefined ? `${parentAs}.` : "") + key));
      }
  ...
  public getInsertDef(obj: InsertObject<T>): IInsertQueryDef {
    ...
    for (const key of Object.keys(obj)) {
      const qb = new QueryBuilder(this._db?.dialect);
      record[qb.wrap(key)] = QueryUtils.getQueryValue(obj[key]);
    }
  ...
  public getUpdateDef(arg: UpdateObject<T> | ((entity: TEntity<T>) => UpdateObject<T>)): IUpdateQueryDef {
    ...
    for (const key of Object.keys(recordEntity)) {
      record[qb.wrap(`${key}`)] = QueryUtils.getQueryValue(recordEntity[key]);
    }
    ...
    return ObjectUtils.clearUndefined({
      ...
      record,
      output: ["*"],
      as: qb.wrap(`TBL${this._as !== undefined ? `.${this._as}` : ""}`),
      join: this._def.join,
  ...
  public getUpsertDef(updateObjOrFwd: UpdateObject<T> | ((entity: TEntity<T>) => UpdateObject<T>), insertObj?: InsertObject<T>): IUpsertQueryDef {
    ...
    for (const key of Object.keys(updateRecordEntity)) {
      updateRecord[qb.wrap(`${key}`)] = QueryUtils.getQueryValue(updateRecordEntity[key]);
    }

    let insertRecord = {};
    if (insertObj) {
      for (const key of Object.keys(insertObj)) {
        insertRecord[qb.wrap(`${key}`)] = QueryUtils.getQueryValue(insertObj[key]);
      }
    }
    else {
  ...
  public async insertAsync(...records: InsertObject<T>[]): Promise<T[]> {
    ...
    if (this._db.dialect === "mysql") {
      ...
      if (aiColNames.length > 1) {
        throw new NotImplementError("하나의 테이블에 AI 컬럼이 2개 이상인 상황에 대해 구현되어있지 않음");
      }
      ...
      const prepareDefs: TQueryDef[] = queryDefs
        .mapMany(queryDef => {
          ...
          const selectDef: TQueryDef | undefined = {
            type: "select",
            select: {
              [qb.wrap("id")]:
                aiColNames.length === 1 && !Object.keys(queryDef.record).includes(qb.wrap(aiColNames[0])) ?
                  "LAST_INSERT_ID()" :
                  "0"
            }
          };
        ...
        return (await this._db.executeDefsAsync(
          queryDefs.mapMany(queryDef => [
            ...
            },
            {
              ...
              from: queryDef.from,
              select: {
                "\"val\"": `setval(pg_get_serial_sequence('${queryDef.from}', '${aiColNames[0]}'), coalesce(max(${qb.wrap(aiColNames[0])}) + 1,1), false)`
              }
            }
          ]),
          queryDefs.map(() => parseOption)
        )).map((item, i) => (i % 2 === 0 ? item[0] : undefined)).filterExists();
      }
  ...
  public insertPrepare(...records: InsertObject<T>[]): void {
    ...
    if (this._db.dialect === "mysql" || this._db.dialect === "postgresql") {
  ...
  public async updateAsync(arg: UpdateObject<T> | ((entity: TEntity<T>) => UpdateObject<T>)): Promise<T[]> {
    ...
    if (this._db.dialect === "mysql") {
      ...
      for (const colDef of this._tableDef.columns) {
        newEntity[colDef.propertyKey] = new QueryUnit(colDef.typeFwd(), `${qb.wrap(`TBL${this._as !== undefined ? `.${this._as}` : ""}`)}.${qb.wrap(colDef.name)}`);
      }
      ...
      const pkColNames = this._tableDef.columns.filter(item => item.primaryKey).map(item => item.name);

      queryDef.where = queryDef.where ?? [];
      for (const pkColName of pkColNames) {
        queryDef.where.push(" AND ", [qb.wrap("_") + "." + qb.wrap(pkColName), " = ", queryDef.as! + "." + qb.wrap(pkColName)]);
      }
      if (queryDef.where[0] === " AND ") {
        queryDef.where = queryDef.where.slice(1);
      }
  ...
  public async upsertAsync(updateObjOrFwd: UpdateObject<T> | ((entity: TEntity<T>) => UpdateObject<T>), insertObj?: InsertObject<T>): Promise<T[]> {
    ...
    if (this._db.dialect === "mysql") {
      ...
      const prevItems = await clone.resultAsync();
      if (prevItems.length > 0) {
        return await this.updateAsync(updateObjOrFwd);
      }
      else {
        return (await this.insertAsync(insertRecord as any));
      }
