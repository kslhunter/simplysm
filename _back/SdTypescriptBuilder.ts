...
import {FileChangeInfoType, FileWatcher, optional} from "@simplysm/sd-core";
...
export class SdTypescriptBuilder {
  ...
  public constructor(private readonly _tsConfigFilePath: string) {
    ...
    this._host["sourceFileImportsVersions"] = {};
  ...
  public async watch(callback: (changeInfos: ITsFileChangeInfo[]) => void, watch?: () => void): Promise<void> {
    ...
    callback(
      this._fileInfos.filter(item => item.isMyFile).map(item => ({
        type: "add",
    ...
    const createWatcher = async (first?: boolean) => {
      const watchFiles = this._fileInfos.mapMany(item1 => [
        item1.filePath,
        ...(item1.dependencies || []),
      ...
      const watcher = await FileWatcher.watch(
        ...
        async changeInfos => {
          ...
          this._program = ts.createProgram(this._tsConfig.fileNames, this._tsConfig.options, this._host, oldProgram);
          this._checker = this._program.getTypeChecker();
          this._updateDependencies();
  ...
  public getNgModules(): { module: string | undefined; path: string; name: string; exports: string[]; providers: string[] }[] {
    ...
    for (const sourceFile of sourceFiles) {
      try {
        ...
        const metatataObj = this._getMetadata(sourceFile.fileName);
  ...
  public getImports(filePath: string): ITsImportInfo[] {
    ...
    if (this._host["sourceFileImportsVersions"][path.normalize(filePath)] !== fileInfo.version) {
      const sourceFile = this._program.getSourceFile(filePath)!;
    ...
    this._host["sourceFileImportsVersions"][path.normalize(filePath)] = fileInfo.version;
  ...
  private _getMetadata(filePath: string): { metadata: ModuleMetadata | undefined; diagnostics: ts.Diagnostic[] } {
    ...
    if (this._host["sourceFileMetadataVersions"][path.normalize(filePath)] !== fileInfo.version) {
      const sourceFile = this._program.getSourceFile(filePath)!;
    ...
    this._host["sourceFileMetadataVersions"][path.normalize(filePath)] = fileInfo.version;
...
type TsFileChangeInfoType = FileChangeInfoType | "dependency" | "embed-dependency";
