...
export class SdTypescriptChecker {
  ...
  public static async createAsync(tsConfigPath: string): Promise<SdTypescriptChecker> {
    ...
    const logger = Logger.get(["simplysm", "sd-cli", packageKey, "check", isNode ? "node" : "browser"]);
  ...
  public async watchAsync(): Promise<void> {
    ...
    const host = ts.createWatchCompilerHost(
      ...
      (diagnostic) => {
        if (ts.flattenDiagnosticMessageText(diagnostic.messageText, os.EOL).startsWith("File change detected")) {
