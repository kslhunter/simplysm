...
`spec.md` 의 골격·섹션 구조·각 섹션 작성법·포함 내용 규정. 작업 절차(분석 사이클·합의 단계 등)는 범위 밖 — 문서 형식·내용만 다룸.
...
| `[OPEN: YYYY-MM-DD]`                   | 섹션 미확정 상태. 본문 = 분석 방법. 초안 (BPMN·와이어프레임·항목 등) 은 채우지 않음.                                                                  |
...
특정 분석 단위 (§X.Y 또는 그 내부의 sub-section/sub-단위) 한정으로 적용되는 진행 방식·메모는 해당 단위 도입부에 NOTE blockquote 작성:

- 형식: `> **NOTE** (진행 방법): <한 줄 요약>` blockquote + 다단 (`> 1. ...`·`> 2. ...` 형식).
- spec 전체에 적용되는 진행 방식은 §10 의 진행 방법 항목으로 누적 (`- YYYY-MM-DD: ...` bullet 형식).
...
**형식 일탈 금지**: ASCII 박스 (┌─┐ │ └┘) ·영역 추상화 표기 (`<필터>`·`<시트>`·`<폼>` 등) ·액션 버튼 텍스트 표기를 임의로 변형 금지.
...
- `관련 섹션:` "본문 내 참조" 절 룰 따름.
- `참조 매뉴얼:` (프레임워크 기본 기능인 경우) 구현 매뉴얼 파일명 (`.claude/references/simplysm14/manuals/` 기준). 이 줄이 있으면 처리·구성·예외 detail 은 적지 않음.
...
- **진행 방법**: spec 전체 작업에 대한 진행 방식 (작성 순서·범위 한정·전체 적용 규칙 등).
