...
Capacitor 플랫폼에서 네이티브 파일 시스템 접근(읽기·쓰기·삭제·디렉토리 관리)을 추상화한 플러그인. Android는 MANAGE_EXTERNAL_STORAGE/READ_WRITE_EXTERNAL_STORAGE 권한으로, 브라우저는 IndexedDB로 에뮬레이션.
...
파일 시스템 권한 확인 여부. Android 11+는 MANAGE_EXTERNAL_STORAGE, Android 10-는 READ_WRITE_EXTERNAL_STORAGE 권한 확인. 브라우저는 항상 true 반환.
...
- `data` (string | Bytes) — 파일 내용. string 또는 Uint8Array 바이너리. Cross-realm 환경(예: iframe)에서도 안전.
...
- `encoding` ("utf8", 선택) — 문자 인코딩. 미지정(또는 "base64"): Bytes(Uint8Array) 반환. "utf8": string 반환.
