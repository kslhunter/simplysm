...
다건 목록, 편집 표를 구성하는 `sd-sheet` 와 컬럼/셀 템플릿/설정 modal/type 군입니다.
standalone, OnPush, `ViewEncapsulation.None`.
컬럼 설정 저장은 [client-system-config.md](../../manuals/client-system-config.md), 표준 목록 골격은 [client-crud.md](../../manuals/client-crud.md) 참조.
...
`<sd-sheet-column>` 을 투영(또는 `columnControlsInput`)해 컬럼을 정의하는 데이터 그리드입니다.
선택, 정렬, 펼침, 페이징, 셀 편집, 키보드 내비게이션, 컬럼 고정, 리사이즈를 내장합니다.
...
- `selectMode` — `undefined`(선택 UI 없음), `"multi"`(체크박스 컬럼 + 전체선택 + shift 범위 선택 + 드래그 페인트 선택), `"single"`(행 화살표 anchor).
  - 드래그 페인트 선택 — 체크박스를 누른 채 세로로 끌면 지나간 행이 연속 선택/해제. 별도 input 없이 항상 동작.
    - 목표 상태는 시작 행을 누른 결과(미선택→선택, 선택→해제)이며 시작 행-현재 행 범위에 동일 적용.
    - 드래그를 되돌려 범위에서 벗어난 행은 드래그 시작 전 상태로 복원.
    - shift 를 누른 채면 페인트 대신 기존 범위 선택으로 동작.
    - 마우스 주 버튼으로 누른 경우만 동작. 우클릭, 가운데 버튼은 무시.
    - `pointerType` 이 `touch` 면 미동작(터치 스크롤 보존).
    - `getItemSelectableFn` 비활성 행은 건너뛰고 계속 진행.
    - 가로 위치는 무관하며 포인터 y 가 속한 행으로 판정. 빠르게 움직여 건너뛴 행도 모두 적용.
    - 판정 대상은 고정 헤더 아래로 실제 보이는 행. 가장자리를 넘기면 벗어난 거리에 비례해 자동 스크롤하며 이어서 칠함.
    - 드래그 중 브라우저 텍스트 선택은 억제됨.
    - 뗀 위치와 무관하게 칠해진 그대로 확정(취소 경로 없음). `pointercancel` 도 동일 확정.
    - 행을 넘지 않고 제자리에서 떼면 드래그가 아니므로 체크박스 단일 토글만 발생.
...
- `totalPageCount` — `>0` 이면 그 값을 페이지 수로(서버 페이징).
- `itemsPerPage` — `>0` && totalPageCount≤0이면 클라이언트 페이징.
...
- `sorts`(model) — 정렬 정의.
  - 헤더 클릭 토글(shift=다중 정렬).
  - 컬럼당 none→asc→desc→제거.
- `currentPage`(model) — 현재 페이지 index(0-based).
- `itemKeydown`/`cellKeydown` — 행/셀 keydown 시 `{ item, event }` / `{ item, key, event }`.
- 셀 편집, 키보드 조작
  - `F2`/더블클릭 편집 진입, `Escape` 종료.
  - 편집 중 `Enter` 아래 셀 이동(멀티라인은 줄바꿈, `Ctrl+Alt+Enter` 아래 이동).
  - 비편집 Arrow 키 셀 이동.
  - `Ctrl+C`/`Ctrl+V` 셀 복사/붙여넣기.
...
- `fixed` — 좌측 고정(sticky).
- `hidden` — 컬럼 제외(설정 override 가능).
- `collapse` — 접힘.
- `disableSorting` — 정렬 아이콘/클릭 제거.
- `disableResizing` — resize handle 숨김.
...
셀 본문 템플릿입니다.
`[cell]="items()"` 로 item 타입을 추론해 `let-item`/`let-index`/`let-depth`/`let-edit` 를 타입화합니다.
