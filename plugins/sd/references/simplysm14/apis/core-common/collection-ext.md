...
진입점을 import 하면 전역 설치되는 Array/ReadonlyArray/Set/Map 프로토타입 확장과 결과 타입. 컬렉션 조회·그룹화·정렬·diff/merge·트리 변환을 한 작업에서 다룰 때 본다.

## 설치와 규약

- `import "@simplysm/core-common"`(또는 임의 심볼 import) 시 `index.ts` 가 `arr-ext`/`set-ext`/`map-ext` 를 먼저 import 하므로 프로토타입 메서드가 전역 설치된다. 별도 import 없이 배열/Set/Map 인스턴스에서 호출한다.
- `ReadonlyArray<T>` — 원본을 바꾸지 않는 읽기 확장(`ReadonlyArrayExt`)을 제공한다.
- `Array<T>` — 읽기 확장 + 원본 변경 확장(`MutableArrayExt`)을 함께 제공한다.
- `@mutates`(원본 변경) 메서드: `distinctThis`, `orderByThis`, `orderByDescThis`, `insert`, `remove`, `toggle`, `clear`(Array), `adds`, `toggle`(Set), `update`(Map). 그 외 Array 메서드는 새 배열/값을 반환한다.
...
- `ofType(type)` — 지정 타입 요소만 반환. `PrimitiveTypeStr`(`"string"|"number"|"boolean"|"DateTime"|"DateOnly"|"Time"|"Uuid"|"Bytes"`, `"Bytes"`=Uint8Array)는 `typeof`/`instanceof` 로, 생성자 타입은 `instanceof` 또는 `constructor` 일치로 판정한다.
- `mapAsync(selector: (item, index) => Promise<R>): Promise<R[]>` — selector 를 순차 실행해 결과 배열을 만든다.
- `mapMany(): T extends readonly U[] ? U[] : T` — selector 없이 중첩 배열을 한 단계 평탄화하고 null/undefined 를 제거한다.
- `mapMany(selector: (item, index) => R[]): R[]` — selector 결과 배열을 평탄화하고 null/undefined 를 제거한다.
- `mapManyAsync(selector: (item, index) => Promise<R[]>): Promise<R[]>` — 비동기 selector 를 순차 실행 후 평탄화한다.
- `parallelAsync(fn: (item, index) => Promise<R>): Promise<R[]>` — `Promise.all(this.map(fn))` 으로 **병렬** 실행한다. 하나라도 reject 되면 전체가 즉시 reject 된다.
...
- `shuffle(): T[]` — Fisher-Yates 로 섞은 새 배열을 반환한다.
...
- `toMapValues(keySelector, valueSelector: (items: T[]) => V): Map<K, V>` — key 별 원본 배열을 모은 뒤 그룹 배열 전체를 집계값으로 변환한다.
...
- `toTree(keyProp: keyof T, parentKey: keyof T): TreeArray<T>[]` — `parentKey` 값이 null/undefined 인 항목을 루트로 삼는다. 각 항목을 `obj.clone` 한 뒤 `children` 을 붙이며 내부적으로 `toArrayMap` 으로 O(n).
...
- `diffs(target: P[], options?): ArrayDiffsResult<T, P>[]` — this(source)와 target 을 비교해 삭제/추가/업데이트 후보를 반환한다.
  - `options.keys?: string[]` — 지정 시 해당 key 값 배열을 JSON 문자열화해 같은 key 후보를 찾는다(전체 일치 우선, 없으면 key 일치). target 중복 key 는 남은 첫 매칭만 사용.
  - `options.excludes?: string[]` — `obj.equal` 비교에서 제외할 최상위 속성.
- `oneWayDiffs(orgItems, keyPropNameOrGetValFn, options?): ArrayOneWayDiffResult<T>[]` — this 를 새 상태로 보고 원본과 비교해 생성/수정/동일을 판정한다.
...
- `merge(target: P[], options?): (T | P | (T & P))[]` — `diffs` 결과 중 양쪽에 있는 항목은 `obj.merge(source, target)` 로 병합하고, target 에만 있는 항목은 뒤에 추가한다. `options.keys/excludes` 는 `diffs` 와 동일.

## Array 원본 변경 메서드 (@mutates)

- `distinctThis(options?): T[]` — `distinct` 규칙으로 원본에서 중복을 제거하고 원본을 반환한다.
- `orderByThis(selector?)` / `orderByDescThis(selector?)` — 원본을 오름/내림차순으로 in-place 정렬한다.
- `insert(index: number, ...items: T[]): this` — `index` 위치에 `items` 를 삽입한다.
- `remove(item: T): this` — `===` 로 같은 항목을 모두 제거한다(뒤에서 앞으로 순회).
- `remove(selector: (item, index) => boolean): this` — selector 가 true 인 항목을 모두 제거한다.
...
- `getOrCreate(key: K, newValueFn: () => V): V` — key 가 없으면 팩토리를 호출해 반환값을 저장한다. **주의**: V 자체가 함수 타입이면 직접 전달 시 팩토리로 호출되므로, 함수 값을 저장하려면 함수를 반환하는 팩토리로 감싼다.
- `update(key: K, updateFn: (v: V | undefined) => V): void` (@mutates) — 현재 값(없으면 undefined)을 `updateFn` 에 넘기고 반환값을 set 한다. key 가 없어도 호출되어 카운터 증가·배열 누적 등에 쓴다.
