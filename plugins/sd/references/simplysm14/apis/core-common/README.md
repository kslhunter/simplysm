...
브라우저·Node 공용 기반 패키지. 값 타입·에러 클래스·컬렉션 프로토타입 확장·객체 조작·직렬화·비동기 런타임·문자열/숫자/경로 유틸을 제공함. 진입점 import 시 Array/Set/Map 프로토타입 확장이 전역 설치됨.
...
- `getKoreanSuffix(text: string, type: "을" | "은" | "이" | "와" | "랑" | "로" | "라"): string` — `text` 마지막 한글의 종성 유무에 따라 조사를 반환함. `"을"`=을/를, `"은"`=은/는, `"이"`=이/가, `"와"`=과/와, `"랑"`=이랑/랑, `"로"`=으로/로(종성이 ㄹ이면 로), `"라"`=이라/라. 빈 문자열이나 한글 범위(0xAC00-0xD7A3) 밖 문자는 받침 없는 형태를 반환함.
...
- `parseInt(text: unknown): number | undefined` — number 는 `Math.trunc`, string 은 숫자·`.`·`-` 외 문자를 제거한 뒤 정수로 파싱함. 선행 `-` 만 음수 부호로 유지하므로 `"010-1234"` → `101234`. 파싱 불가면 undefined.
- `parseFloat(text: unknown): number | undefined` — number 는 그대로, string 은 숫자·`.`·`-` 외 문자를 제거한 뒤 실수로 파싱함. 선행 `-` 만 음수로 유지. 파싱 불가면 undefined.
...
- `format(val: number | undefined, digit?: { max?: number; min?: number }): string | undefined` — `toLocaleString` 으로 천단위 구분 포맷함. `digit.max` 는 최대 소수 자릿수, `digit.min` 은 최소 소수 자릿수(부족 시 0 채움)임. `val` 이 undefined 면 undefined 반환.
...
- `typeStr(value: PrimitiveTypeMap[PrimitiveTypeStr]): PrimitiveTypeStr` — 값의 런타임 타입을 검사해 `"string" | "number" | "boolean" | "DateTime" | "DateOnly" | "Time" | "Uuid" | "Bytes"` 중 하나를 반환함. `Uint8Array` 는 `"Bytes"` 임. 지원하지 않는 타입이면 `ArgumentError`.

## template string tags

`import { js, ts, html, tsql, mysql, pgsql } from "@simplysm/core-common"`. 모두 런타임 동작은 같다(IDE 코드 하이라이팅용 구분): 보간값을 `String(value)`(null/undefined 는 빈 문자열)로 합친 뒤 앞뒤 빈 줄과 공통 최소 들여쓰기를 제거함. 차이는 에디터 하이라이팅 언어 힌트뿐임.
...
- `createLogger(tag: string): ConsolaInstance` — `consola.withTag(tag)` 생성을 첫 메서드 접근 시점까지 지연하는 Proxy 로거임. 모듈 레벨에서 선언해도 이후의 consola 설정(level/reporters) 변경을 반영하며, 테스트 spy 호환을 위해 target 자체 속성을 cached 인스턴스보다 우선 조회함. 콘솔 로깅 코드 작성: [logging.md](../../manuals/logging.md)
