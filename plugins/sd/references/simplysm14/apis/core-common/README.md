...
브라우저·Node 공용 기반 패키지. 값 타입·에러 클래스·컬렉션 프로토타입 확장·객체 조작·직렬화·비동기 런타임·문자열/숫자/경로 유틸을 제공한다. 진입점 import 시 Array/Set/Map 프로토타입 확장이 전역 설치된다.
...
- `getKoreanSuffix(text: string, type: "을" | "은" | "이" | "와" | "랑" | "로" | "라"): string` — `text` 마지막 한글의 종성 유무에 따라 조사를 반환한다. `"을"`=을/를, `"은"`=은/는, `"이"`=이/가, `"와"`=과/와, `"랑"`=이랑/랑, `"로"`=으로/로(종성이 ㄹ이면 로), `"라"`=이라/라. 빈 문자열이나 한글 범위(0xAC00~0xD7A3) 밖 문자는 받침 없는 형태를 반환한다.
...
- `parseInt(text: unknown): number | undefined` — number 는 `Math.trunc`, string 은 숫자·`.`·`-` 외 문자를 제거한 뒤 정수로 파싱한다. 선행 `-` 만 음수 부호로 유지하므로 `"010-1234"` → `101234`. 파싱 불가면 undefined.
- `parseFloat(text: unknown): number | undefined` — number 는 그대로, string 은 숫자·`.`·`-` 외 문자를 제거한 뒤 실수로 파싱한다. 선행 `-` 만 음수로 유지. 파싱 불가면 undefined.
...
- `format(val: number | undefined, digit?: { max?: number; min?: number }): string | undefined` — `toLocaleString` 으로 천단위 구분 포맷한다. `digit.max` 는 최대 소수 자릿수, `digit.min` 은 최소 소수 자릿수(부족 시 0 채움)다. `val` 이 undefined 면 undefined 반환.

## path

`import { path } from "@simplysm/core-common"` 네임스페이스. POSIX 슬래시(`/`) 경로만 지원하며 Windows 백슬래시 경로는 지원하지 않는다(브라우저·Capacitor 용).

- `join(...segments: string[]): string` — 세그먼트를 `/` 로 결합한다. 첫 세그먼트는 뒤 슬래시만, 이후 세그먼트는 앞뒤 슬래시를 제거하며 빈 세그먼트는 제외한다.
- `basename(filePath: string, ext?: string): string` — 마지막 `/` 뒤 파일명을 반환한다. `ext` 가 비어 있지 않고 파일명 끝과 일치하면 그 확장자를 제거한다.
- `extname(filePath: string): string` — 마지막 `.` 뒤 확장자(`.` 포함)를 반환한다. `.` 으로 시작하는 숨김 파일이나 확장자 없는 이름은 빈 문자열을 반환한다.

## primitive

`import { primitive } from "@simplysm/core-common"` 네임스페이스.

- `typeStr(value: PrimitiveTypeMap[PrimitiveTypeStr]): PrimitiveTypeStr` — 값의 런타임 타입을 검사해 `"string" | "number" | "boolean" | "DateTime" | "DateOnly" | "Time" | "Uuid" | "Bytes"` 중 하나를 반환한다. `Uint8Array` 는 `"Bytes"` 다. 지원하지 않는 타입이면 `ArgumentError`.

## template string tags

`import { js, ts, html, tsql, mysql, pgsql } from "@simplysm/core-common"`. 모두 런타임 동작은 같다(IDE 코드 하이라이팅용 구분): 보간값을 `String(value)`(null/undefined 는 빈 문자열)로 합친 뒤 앞뒤 빈 줄과 공통 최소 들여쓰기를 제거한다. 차이는 에디터 하이라이팅 언어 힌트뿐이다.
...
- `createLogger(tag: string): ConsolaInstance` — `consola.withTag(tag)` 생성을 첫 메서드 접근 시점까지 지연하는 Proxy 로거다. 모듈 레벨에서 선언해도 이후의 consola 설정(level/reporters) 변경을 반영하며, 테스트 spy 호환을 위해 target 자체 속성을 cached 인스턴스보다 우선 조회한다. 콘솔 로깅 코드 작성: [logging.md](../../manuals/logging.md)
...
- `PrimitiveTypeMap` — `string`, `number`, `boolean`, `DateTime`, `DateOnly`, `Time`, `Uuid`, `Bytes` 키를 실제 값 타입에 매핑한다(orm-common 과 공유).
...
- `DeepPartial<TObject>` — `PrimitiveType` 속성은 그대로 두고 객체/배열 속성만 재귀적으로 optional 처리한다.
- `Type<TInstance> extends Function` — `new (...args: unknown[]) => TInstance` 생성자 타입이다. DI·팩토리·`instanceof` 체크에 쓴다.
