...
- `dispose()` — 모든 이벤트의 wrapped 리스너를 제거하고 내부 맵을 비움.
...
- `run(fn)` — 내부 FIFO 배열에 추가하고 처리 루프를 시작함. 이미 처리 중이면 새 루프를 만들지 않고 기존 루프가 순서대로 실행.
- `fn` 오류 — `SdError(error, "큐 작업 실행 중 오류 발생")` 로 감싸고 후속 작업은 계속 실행. `error` 리스너가 있으면 emit, 없으면 내부 logger 로 출력.
- `gap` 동작 — `gap > 0` 이고 대기 작업이 남아 있으면 다음 작업 전 `wait.time(gap)` 으로 쉼.
- `dispose()` — 대기 큐를 비우고 부모 `dispose()` 로 리스너 제거. **현재 실행 중인 함수는 중단하지 않음.**
