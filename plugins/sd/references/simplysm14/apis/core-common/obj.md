...
`import { obj } from "@simplysm/core-common"` 네임스페이스.
객체 깊은 복사, 동등 비교, 병합, 3-way 병합, 체인 경로 접근, 타입 안전 Object 헬퍼를 한 컨텍스트에서 다룰 때 봅니다.
`@simplysm/*` 의 커스텀 값 타입(DateTime/DateOnly/Time/Uuid/Uint8Array)을 인지해 처리합니다.
...
깊은 복사. 원시값은 그대로 반환하고 객체는 타입별로 복사합니다.

- 지원 타입 — `Date`, `DateTime`, `DateOnly`, `Time`, `Uuid`, `RegExp`, `Error`, `Uint8Array`, `Array`, `Map`, `Set`, 일반 객체.
- 순환 참조 — `WeakMap` 으로 이미 복사한 객체를 재사용합니다.
- Error — 생성자 대신 prototype 기반 복사로 커스텀 Error 호환.
  - `message`, `name`, `stack`, `cause`(재귀), 그 외 enumerable 속성을 복사합니다.
- 일반 객체 — prototype 을 유지(`Object.setPrototypeOf`)하고 `Object.keys` 속성을 재귀 복사합니다.
  - getter/setter 는 현재 값으로 평가되어 복사됩니다.
- 함수, Symbol — 객체가 아니므로 참조를 그대로 유지합니다.
  - WeakMap/WeakSet 은 미지원(빈 객체가 됩니다).
...
- `Date` — `getTime()` 비교.
- `DateTime`, `DateOnly`, `Time` — 같은 타입끼리 `tick` 비교.
- `Uuid` — 문자열 비교.
- `RegExp` — `source`+`flags` 비교.
- `Array` — 길이 먼저 비교 후, `ignoreArrayIndex` 에 따라 index 순서 또는 순열 비교.
- `Map` — null/undefined 값 key 는 제외.
  - 문자열 key 는 직접 조회, 비문자열 key 는 깊은 비교로 매칭(O(n²)).
  - `topLevelIncludes/Excludes` 는 Map 에 적용되지 않습니다(모든 key 비교).
...
- `useDelTargetNull?: boolean` — true 면 target 의 null 이 해당 key 삭제로 이어집니다. false/미지정이면 source 값 유지.
...
- 병합 규칙 — source==origin 이면 target 값, target==origin 이면 source 값, source==target 이면 source 값을 씁니다.
  - 셋 다 다르면 `conflict = true` 로 두고 origin 값을 유지합니다.
- 반환 — `{ conflict: boolean; result }`.
- `optionsObj?: Record<string, Merge3KeyOptions>` — key 별 비교 옵션.
  - 구현은 이를 `equal(source[key], result[key], optionsObj[key])` 에 직접 전달합니다.
- `Merge3KeyOptions.ignoreArrayIndex?: boolean` — `equal` 과 필드명이 같아 배열 순서 무시 비교에 적용됩니다.
- `Merge3KeyOptions.keys?: string[]` / `excludes?: string[]` — 현재 구현에서는 직접 효과가 없습니다.
  - 선언된 필드이나 `equal` 의 인식 필드명은 `topLevelIncludes`/`topLevelExcludes` 입니다.
...
- `omitByFilter<T>(item: T, omitKeyFn: (key: keyof T) => boolean): T` (@internal) — `omitKeyFn(key)` 가 true 인 key 를 제외합니다.
...
- `chain` 파싱 — `.`, `[`, `]` 로 분리하고 `?`, `!`, `'`, `"` 를 제거합니다.
  - 숫자로만 된 조각은 number index 로 변환.
- `getChainValueByDepth<TObject, TKey extends keyof TObject>(obj, key, depth, optional?)` (@internal) — 같은 `key` 로 `depth` 만큼 내려갑니다.
  - `depth < 1` 이면 `ArgumentError`.
  - `optional` true 면 중간 null/undefined 에서 undefined 반환.
- `setChainValue(obj, chain: string, value: unknown): void` — 중간 경로가 없으면 `{}` 를 만들어 마지막 segment 에 value 설정.
  - 빈 chain 이면 `ArgumentError`.
- `deleteChainValue(obj, chain: string): void` — 마지막 segment 삭제.
  - 중간 경로가 없거나 객체가 아니면 아무 동작 없이 반환.
  - 빈 chain 이면 `ArgumentError`.
...
- `nullToUndefined<T>(obj: T): T | undefined` (@mutates) — null/undefined 를 undefined 로 바꾸고 배열/객체 내부도 재귀 변환.
  - 값 타입(Date 등)은 그대로 두고 순환 참조는 `WeakSet` 으로 방지.
  - `json.parse` 가 null-free 규칙을 위해 사용.
- `unflatten(flatObj: Record<string, unknown>): Record<string, unknown>` — `"a.b"` 같은 dot key 를 중첩 객체로 펼칩니다.
...
- `map<TSource, TNewKey extends string, TNewValue>(obj, fn): Record<TNewKey | Extract<keyof TSource, string>, TNewValue>`
  — 각 엔트리를 `fn(key, value) => [newKey | null, newValue]` 로 변환해 새 객체를 만듭니다.
  - `newKey` 가 null 이면 원래 key 유지.
