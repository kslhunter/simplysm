...
브라우저 전용 유틸리티: DOM 요소의 `Element`/`HTMLElement` 프로토타입 확장으로 조회·탐색·탭 이동·위치 계산·가시성 판정을 제공하고, IndexedDB 저장소 래퍼와 경로 키 기반 가상 파일시스템, Blob 다운로드·URL 바이너리 수신·파일 선택 함수를 포함한다.
