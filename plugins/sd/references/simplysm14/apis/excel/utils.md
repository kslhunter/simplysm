...
- `addr` — 셀 주소 문자열. 행 파싱은 끝 숫자, 열 파싱은 앞 알파벳을 쓴다.
- `rangeAddr` — `:` 로 구분된 범위 주소. 끝 주소가 없으면 시작과 같은 좌표로 처리.

메서드 동작:

- `stringifyAddr(point)` — 열 문자 + 행 숫자로 셀 주소(`"A1"`)를 만든다.
- `stringifyRowAddr(r)` — 0 기반 행 인덱스를 1 기반 행 주소 문자열로 만든다.
- `stringifyColAddr(c)` — 0 기반 열 인덱스를 Excel 열 문자로 만든다(예: `0→"A"`, `26→"AA"`). 범위 밖이면 throw.
- `parseRowAddr(addr)` — 주소 끝 숫자를 0 기반 행 인덱스로 파싱한다. 정수 파싱 실패 시 throw.
- `parseColAddr(addr)` — 주소 앞 알파벳을 0 기반 열 인덱스로 계산한다.
- `parseCellAddr(addr)` — `parseRowAddr`/`parseColAddr` 결과를 `{ r, c }` 로 묶는다.
- `parseRangeAddr(rangeAddr)` — 시작/끝 셀 주소를 `{ s, e }` 로 파싱한다.
- `stringifyRangeAddr(point)` — 시작=끝이면 단일 주소, 다르면 `시작:끝` 문자열을 반환한다.
...
Excel 은 1899-12-30 을 날짜 0(1900-01-01 = 1)으로 계산한다.

- `tick` — JS timestamp(ms). 변환 전 timezone offset 을 빼고 Excel serial 숫자를 계산한다.
...
- `convertNumFmtIdToName(numFmtId)` — 내장 ID 범위 표에 따라 분류한다.
- `convertNumFmtNameToId(numFmtName)` — literal 을 대표 내장 numFmtId 로 변환한다.
