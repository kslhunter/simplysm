...
Android 디바이스의 Intent 기능을 JavaScript에서 제어하는 Capacitor 플러그인입니다. 브로드캐스트 수신/송신, 앱 실행 인텐트 조회, 외부 Activity 실행 및 결과 수신이 가능합니다.

**주요 용도**: 산업용 디바이스(바코드 스캐너, PDA 등) 연동을 통한 Intent 기반 통신.

**플랫폼 지원**: Android (네이티브). 웹 환경에서는 경고 로그를 남기고 stub 값을 반환합니다.

---

## Intent — 정적 API

`abstract class Intent`는 registerPlugin으로 등록된 플러그인을 감싼 정적 API입니다. 인스턴스를 생성하지 않고 모든 메서드를 정적으로 호출합니다.

### subscribe(filters, callback) → Promise<() => Promise<void>>

Android 브로드캐스트 수신기를 등록합니다.
...
**반환값:** 구독 해제 함수 (`async () => void`). 이 함수를 호출하면 리스너가 제거됩니다.

**동작:**

- 플러그인에 필터를 등록하면 내부 ID가 발급됩니다.
- 초기 응답(ID만 포함된 `{ id }`)은 `result.action != null` 검사로 필터링되어 콜백에 전달되지 않습니다.
- 실제 브로드캐스트(action 포함)가 도착할 때만 콜백이 호출됩니다.
...
모든 등록된 브로드캐스트 수신기를 한 번에 해제합니다.

**동작:** 플러그인의 `unsubscribeAll` 메서드를 호출하여 활성 구독을 모두 정리합니다.

---

### send(options) → Promise<void>

Android 브로드캐스트를 시스템에 전송합니다.
...
**동작:** 지정된 액션과 데이터를 포함한 브로드캐스트를 전송합니다.
...
앱이 인텐트를 통해 실행될 때 전달받은 초기 인텐트를 조회합니다.

**반환값:** `IntentResult` — 앱 시작 시점의 인텐트 정보

**동작:**

- 앱이 다른 앱에서 인텐트로 호출되었을 때, 그 인텐트의 액션과 데이터를 반환합니다.
- 정상 시작인 경우 빈 객체를 반환할 수 있습니다.
...
앱 실행 중 수신되는 새로운 인텐트에 대한 리스너를 등록합니다.
...
- `subscribe`와 달리 필터 없이 앱이 **실행 중인 상태에서 들어오는 모든 인텐트**를 감시합니다.
- 핸들의 `.remove()` 메서드를 호출하면 개별 리스너가 제거됩니다.
...
`addListener`로 등록된 모든 이벤트 리스너를 제거합니다.

**동작:** 플러그인의 `removeAllListeners` 메서드를 호출하여 모든 `newIntent` 리스너를 정리합니다.

---

### startActivityForResult(options) → Promise<StartActivityForResultResult>

외부 Activity를 실행하고 실행 결과를 수신합니다.
...
1. 조건에 맞는 Activity를 찾아 실행합니다.
2. 사용자가 Activity를 완료할 때까지 기다립니다.
3. 결과 코드와 반환 데이터를 반환합니다.
...
- `extras`: 액션 데이터와 함께 전달되는 부가 정보. 객체 형태로 자유로운 값을 담을 수 있습니다.

---

### StartActivityForResultOptions

외부 Activity 실행 요청 매개변수입니다.
...
- `action`: 어떤 작업을 수행할 것인지 지시하는 문자열. `packageName`/`className` 없으면 Android는 이 액션을 처리할 앱을 찾습니다.
- `uri`: 처리 대상 데이터 주소. `type`과 함께 사용하면 앱 선택 다이얼로그가 표시됩니다.
- `type`: MIME type. `uri` 없이 `type`만 지정하면 해당 타입을 처리할 앱들을 필터링합니다.
- `packageName`/`className`: 직접 지정하면 특정 앱/Activity만 실행되므로 선택 다이얼로그 없이 즉시 실행됩니다.
...
`Intent` 정적 API가 내부에서 호출하는 Capacitor 플러그인 인터페이스입니다. 네이티브(Android)와 웹(`IntentWeb`) 구현이 이 계약을 따릅니다.
...
일반적으로 `Intent` 정적 메서드를 사용하며, 이 인터페이스는 구현·해석 확인 목적입니다.

---

## 웹 환경 지원

`IntentWeb` 클래스가 웹 환경용 구현을 제공합니다.
...
**실 동작은 Android 디바이스/에뮬레이터에서만 이루어집니다.** 웹 브라우저에서 개발 시 이 차이를 고려하세요.
...
1. **Android 권한**: `AndroidManifest.xml`에서 필요한 권한을 명시해야 합니다.
   - 브로드캐스트 수신: API 31 이상에서는 명시적 필터 필요
   - 특정 기능에 따라 추가 권한 필요 (카메라, 파일 접근 등)

2. **메모리 누수 방지**: `subscribe` 또는 `addListener` 등록 후 반드시 리스너를 제거해야 합니다.
   - Angular 컴포넌트의 경우 `ngOnDestroy`에서 정리
   - 단일 `unsubscribe` 또는 `handle.remove()` 사용

3. **웹에서 완전 테스트 불가**: 브라우저 환경에서는 Intent 기능을 테스트할 수 없습니다.
   - 실제 Android 디바이스 또는 에뮬레이터에서 검증 필수

4. **액션 문자열 정확성**: 브로드캐스트 액션은 정확한 문자열이어야 수신됩니다.
   - 디바이스 제조사 또는 타사 앱 문서 참고
   - 오타는 브로드캐스트 수신 실패를 초래

5. **타이밍**: `getLaunchIntent()`는 **앱 시작 직후 가능한 빨리** 호출해야 정확한 데이터를 얻을 수 있습니다.
