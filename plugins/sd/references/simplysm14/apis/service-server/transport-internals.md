...
`ServiceServer.listen()`이 내부에서 엮는 실행기, HTTP/WebSocket/업로드/정적 파일 핸들러, 프로토콜 래퍼, 설정 캐시를 직접 테스트·확장할 때 같이 읽는 묶음이다. 정적 파일 SPA 셸(`index.csr.html`) fallback 맥락: [client-ssg.md](../../manuals/client-ssg.md).
...
- `server: ServiceServer` — `options.services`·`options.auth`를 읽을 서버 인스턴스.
- `def.serviceName: string` — `ServiceDefinition.names`에 포함되는 이름으로 서비스를 찾는다. 없으면 `"서비스 [...]를 찾을 수 없습니다."`를 throw한다.
- `def.methodName: string` — 팩토리가 반환한 메서드 객체에서 찾을 키. 값이 함수가 아니면 `"메서드 [<service>.<method>]를 찾을 수 없습니다."`를 throw한다.
- `def.params: unknown[]` — 찾은 메서드에 spread로 전달할 인자 배열.
- `def.socket?: ServiceSocket` — WebSocket 요청 출처. 컨텍스트 생성과 인증 토큰 조회에 쓰인다. `clientName`에 `..`·`/`·`\\`가 포함되면 보안 오류로 throw한다.
- `def.http?: { clientName; authTokenPayload? }` — HTTP 요청 출처. `clientName`은 보안 검사·컨텍스트 생성에, `authTokenPayload`는 인증 검사에 쓰인다. `clientName`에 `..`·`/`·`\\`가 포함되면 보안 오류로 throw한다.
- 인증 순서 — 메서드 `auth` 권한이 있으면 그것을, 없으면 서비스 수준 `authPermissions`를 쓴다. 권한 메타가 있는데 `auth` 옵션이 `undefined`면 설정 오류, `false`면 인증 검사를 건너뛴다. 권한 배열이 비어 있으면 로그인만 요구하고, 비어 있지 않으면 토큰 `roles` 중 하나라도 포함되어야 통과한다.
...
- `clientId: string` — 연결 식별자. 반환 객체에 저장하지 않고 상위 `WebSocketHandler`가 소켓 map key로 쓴다.
- `clientName: string` — 반환 객체의 `clientName`으로 노출되어 서비스 컨텍스트 클라이언트 이름 출처가 된다.
...
- `send(uuid, msg): Promise<number>` — 서버 메시지를 protocol wrapper로 encode해 chunk들을 보내고 전송 바이트 수를 반환한다. 소켓이 OPEN이 아니면 `0`을 반환한다.
  - `uuid: string` — 요청·응답 상관관계 ID. progress/error/response 전송에 같이 쓰인다.
  - `msg: ServiceServerMessage` — 클라이언트로 보낼 서버 메시지.
- `addListener(key, eventName, info)` — 이 소켓의 이벤트 리스너 목록에 항목을 추가한다.
  - `key: string` — 리스너 식별자. 제거·대상 필터링에 쓰인다.
...
- 연결 유지 — 5초마다 ping을 보내고 pong을 받지 못한 주기에는 소켓을 terminate한다. 1바이트 `0x01` 메시지는 `0x02` 응답으로 처리된다.
...
- `runMethod` — 서비스 RPC 메시지를 실제 메서드 실행으로 넘기는 함수. 보통 `executeServiceMethod`를 서버에 바인딩한다. `def`는 메시지 이름의 첫 `.` 앞/뒤로 나눈 `serviceName`/`methodName`, body 배열 `params`, 요청 소켓 `socket`을 받는다.
- `jwtSecret: string | undefined` — 소켓 `auth` 메시지 검증 시크릿. 없는데 `auth` 메시지가 오면 오류 응답이 된다.
...
- `reply: FastifyReply` — 인증 실패·잘못된 요청·미지원 메서드·성공 응답을 전송할 응답 객체.
...
- `http.authTokenPayload?: AuthTokenPayload<TAuthInfo>` — Authorization(`Bearer <token>`) 검증 성공 시 전달되는 페이로드. 실패 시 401 응답이다.
...
- `req: FastifyRequest` — multipart 여부·Authorization 헤더·parts stream을 읽는 요청 객체. multipart가 아니면 400 응답이다.
- `reply: FastifyReply` — 400/401/500 또는 성공 결과를 전송할 응답 객체.
- `rootPath: string` — 업로드 저장 기준 경로. 파일은 `rootPath/www/uploads/<uuid><ext>`에 저장된다.
- `jwtSecret: string | undefined` — 업로드 인증 토큰 검증 시크릿. 헤더·시크릿이 없거나 검증 실패하면 401 응답이다.
...
- 실패 처리 — 저장 중이던 현재 파일과 이미 저장된 파일을 삭제 시도한 뒤 500 `업로드 실패`를 보낸다. 파일 크기 제한 초과(`truncated`)도 실패로 처리된다.
...
- 숨김 파일 — basename이 `.`으로 시작하면 403 HTML 응답이다.
- 파일 없음 — 확장자 없는 경로면 현재 위치에서 `www` 루트까지 거슬러 가장 가까운 `index.csr.html`(SSG SPA 셸)을 찾아 전송한다. 없거나 확장자 있는 요청이면 404 HTML 응답이다.
- 그 외 전송 실패 — 500 HTML 응답이다.
...
- `encode(uuid, message)` — 메시지를 protocol chunks로 인코딩한다. `body`가 `Uint8Array`이거나 배열 안에 `Uint8Array`가 있으면 공유 worker로 위임하고, 아니면 메인 스레드에서 처리한다.
  - 반환 `chunks: Bytes[]` — 전송할 byte chunk 배열.
  - 반환 `totalSize: number` — encode 결과 전체 크기.
- `decode(bytes)` — 수신 bytes를 누적·파싱한다. 청크 재조립(stateful)은 항상 메인 스레드 단일 누적기에서 하고, 재조립 중이면 `type: "progress"` 결과를 반환한다. 재조립 완료 후 payload가 30KB를 초과하면 worker에서, 아니면 메인 스레드에서 JSON parse한다.
...
- 만료 — 1시간 무접근이면 캐시가 만료되고 watcher가 닫힌다(10분마다 GC).
