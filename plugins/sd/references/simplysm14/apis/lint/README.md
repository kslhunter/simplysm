...
TypeScript/Angular 코드 린팅을 위한 ESLint 플러그인. TypeScript strict 모드와 Angular 최적 사례를 강제하는 9개 커스텀 규칙과, typescript-eslint 기반 권장 flat config를 제공한다.
...
- **전체 규칙 제공** — `plugin.rules` 객체에 9개 규칙 구현이 포함된다.
- **사용 방식** — ESLint flat config에서 플러그인 등록 후 규칙을 명시적으로 활성화한다.
  ```typescript
...
- **구성 요소** — 공통 규칙, JS/TS/HTML 별 세부 규칙, 테스트/설정파일 예외, Angular 템플릿 규칙을 포함한다.
- **사용 방식** — ESLint flat config 배열로 그대로 spread하거나 export한다.
  ```typescript
  import config from "@simplysm/lint/eslint-recommended";

  export default config;
  ```
- **포함 규칙** — @typescript-eslint, angular-eslint, import-x, unused-imports, 그리고 @simplysm 커스텀 규칙을 통합한다.
...
Angular `effect()` 콜백에 async 함수를 직접 전달하는 것을 금지한다.

- **사유** — async 콜백의 `await` 이후에 읽은 signal은 의존성 추적이 끊겨 반응형이 아니며, 반환값이 `Promise<void>`가 되어 cleanup 등록도 불가능하다.
- **올바른 패턴** — `untracked(async () => { ... })` 내부에서 비동기 작업을 수행한다.
...
`@simplysm/*` 패키지의 'src' 하위 경로 import를 금지한다.
...
- **용도** — 미구현 코드가 프로덕션에 포함되었을 가능성을 인지시킨다.
...
클래스 내 `inject()` 필드 중 미사용 필드를 감지해 보고한다.
...
Angular `@Component` 내 미사용 `protected readonly` 필드를 감지해 보고한다.
...
Angular 템플릿의 `=== null`, `!== null`, `=== undefined`, `!== undefined` 사용을 금지한다.
...
각 규칙의 상세 동작, 메시지, 예제는 [rules.md](./rules.md)를 참고한다.

eslint-recommended 설정의 각 섹션(JS/TS/HTML/test/config) 역할과 규칙 조정은 [eslint-recommended.md](./eslint-recommended.md)를 참고한다.
