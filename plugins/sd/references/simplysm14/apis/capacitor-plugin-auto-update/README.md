...
- **AutoUpdate** — 앱 부팅 시점에 "최신 확인 → 권한 → 다운로드 → 설치 → 앱 freeze" 를 한 번에 도는 진입점. 서버 연동 배포면 `run`, USB/SD 등 외부 저장소 폴더 사이드로딩이면 `runByExternalStorage`.
...
- 권한 미승인(`granted:false`) 시 `requestPermissions` 로 설정 화면으로 보낸 뒤 최대 5분(1초 간격 300회) 동안 `checkPermissions` 로 권한 부여를 폴링 대기.
...
- VersionInfo.versionName: string — 사람이 읽는 표시 버전(semver, 예 `"1.2.3"`). `AutoUpdate` 의 버전 비교(`semver.gt`/`semver.valid`)가 이 값을 semver 로 사용.
