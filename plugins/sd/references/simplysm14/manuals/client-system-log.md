...
- `data` 는 가변 인자 배열. 각 인자를 string→그대로 / Error→`stack` / 객체→`json.stringify` 로 문자열화해 공백으로 join 하여 저장함(Error 의 `stack` 보존).
...
- `SdToastProvider.try()` — 잡은 `Error` 를 danger 토스트로 띄울 때 `writeAsync("error", ...)`. `.danger()` 직접 호출은 토스트만 표시하므로 외부 적재가 필요하면 `SdSystemLogProvider.writeAsync()` 를 별도 호출함.
