...
앱의 메뉴, 권한, 기능 모듈을 한 군데서 정의하는 방법.
메뉴 트리, 화면 접근 권한, 모듈별 on/off 가 모두 이 구조 하나에서 나옴.
새 화면을 메뉴에 올리거나 권한을 거는 작업 시 참조함.
...
**새 화면 등록 관례**: 기존 화면 1개의 등록 위치, 방식을 그대로 본떠 추가함.

- `title`: 화면명을 그대로 씀.
- `code`: 화면명을 dash-case 영문으로 음역한 슬러그. 프로젝트의 기존 슬러그 규칙이 일관되면 그 규칙 우선함.
...
권한을 걸려면 화면에 `perms` 를 정의함. 정의하면 아래 셋이 적용됨.

- 권한 관리 페이지에 체크 항목으로 나옴.
- 권한 없는 사용자에게는 메뉴가 자동으로 숨겨짐.
...
- `perms`: 부여할 권한 종류. `"use"`(조회) / `"edit"`(편집).
  - `perms` 를 지정한 화면만 권한 페이지, 권한 체크 대상이 됨.
...
**화면 안에서 권한 체크** — 화면 컴포넌트에서 `injectPermsSignal(<path>, <actions>)` 로 정의된 권한을 읽어 체크함.

- 첫 인자(권한 path)는 이 구조의 화면 fullCode(들).
- 체크 작성 관례(단순 체크는 인라인, `computed` 사용 기준)는 [client-component.md](./client-component.md) 의 '권한' 참조함.
