...
이 매뉴얼은 v12(`@simplysm/sd-angular`) 기반 프로젝트에서 **모든 화면 컴포넌트가 공통으로 따르는 규약**을 다룬다. 목록/편집 시트의 골격(`AbsSdDataSheet` / `sd-data-sheet`, `AbsSdDataDetail` / `sd-data-detail`) 자체는 [client-data-sheet.md](./client-data-sheet.md) 로 위임하고, 여기서는 화면의 종류와 무관하게 적용되는 사항(파일·데코레이터·시그널·권한·토스트·모달·레이아웃·아이콘·DI)을 정리한다.

---

## 새 화면 파일을 만들 때 — 이름·역할 접미사·위치·selector

화면 파일은 **PascalCase 클래스명 + 역할 접미사**로 짓고, 파일명을 클래스명과 동일하게 둔다(`UserPage.ts` 안에 `class UserPage`). 역할 접미사로 화면의 책임을 표시한다.
...
**위치**: 화면은 도메인 폴더에 클래스 파일을 모은다. 예: `src/app/home/base/user/UserPage.ts`, `src/app/home/base/goods/GoodsPage.ts` + `GoodsDetail.ts`. 라우팅 폴더 경로는 dash-case(`base/goods`), 그 안의 파일·클래스는 PascalCase.

**`Modal` 과 `Detail` 의 구분**은 표시 방식이 아니라 **화면의 본질**로 정한다. 한 레코드를 로드·저장하는 단건 화면이면 `AbsSdDataDetail` 을 상속한 `XxxDetail.ts` 로 만들고, 모달로 띄울 때도 그 `Detail` 을 그대로 `showAsync` 한다(아래 "다른 화면을 모달로 띄우기"). `XxxModal.ts` 는 단건 CRUD 도 라우팅 페이지도 아닌, 모달로만 존재하는 비-CRUD UI(예: `PasswordResetModal` — 아이디·이메일로 임시 비밀번호 재발급)에만 쓴다.

**selector** 는 항상 `app-` prefix 를 붙인 dash-case 로 짓는다. 클래스명을 그대로 dash-case 화하지 않아도 되며 화면을 식별할 수 있는 짧은 이름이면 된다.
...
## `@Component` 데코레이터 표준값을 설정한다

v12 화면 컴포넌트의 `@Component` 데코레이터는 다음을 **항상** 그대로 둔다(Angular 기본과 다른 부분만 명시).
...
- **`standalone: true`** — 항상. NgModule 을 새로 만들지 않는다.
- **`imports`** — 템플릿에서 쓰는 컨트롤·디렉티브·파이프를 모두 명시한다. 위 `UserPage` 처럼 `sd-data-sheet` 를 쓰면 `SdDataSheetControl` + `SdDataSheetColumnDirective` + 셀 템플릿 디렉티브 `SdSheetColumnCellTemplateDirective` 를 함께 import 한다.
...
화면(`Page`)은 `routes.ts` 에 `loadComponent` 로 lazy 등록한다. 경로 세그먼트는 dash-case 이며, 도메인 트리를 `children` 으로 중첩한다.
...
- `import("...경로...").then((m) => m.XxxPage)` 형태로 클래스를 지연 로드한다. 경로는 확장자 없이 파일 경로, `m.클래스명` 으로 해당 export 를 가리킨다.
- 도메인 그룹(`base`, `accounting`, `system` 등)은 `children` 으로 묶고, 그룹 노드 자체에는 `loadComponent` 없이 하위만 둘 수 있다.
- `Detail` / `Modal` 은 라우팅에 등록하지 않는다. 목록 화면 안에서 `showAsync` 로 직접 띄우거나 부모 화면에 임베드한다.
- `provideSdAngular` 가 라우터 네비게이션 동안 전역 busy 오버레이를 자동 표시하므로(`NavigationStart` 에서 `globalBusyCount` 증가), lazy 로딩 중 별도 로딩 표시를 추가할 필요는 없다.

메뉴/권한 트리(`appStructureItems`)와의 대응은 [client-app-structure.md](./client-app-structure.md) 를 참조.

---

## 상태를 `$signal` / `$computed` 로 구성하고, 객체 내부 변경은 `$mark` 로 알린다

화면 상태는 `@simplysm/sd-angular` 의 시그널 헬퍼로 만든다. `$signal` 으로 만든 시그널에는 `$mark()` 메서드가 부착되어 있어, 값 참조를 바꾸지 않고 강제로 변경 알림을 보낼 수 있다.
...
- `$effect([deps], async fn)` — `deps` 배열에 나열한 시그널만 추적하고 본문은 `untracked` 로 실행한다(본문 내부에서 시그널을 읽어도 재실행 트리거가 되지 않음). `[]` 를 주면 최초 1회만 실행 — 화면 진입 시 초기 로드에 쓴다.

### 객체·배열 시그널의 내부 필드 변경 알림 — `$mark()`

`$signal` 이 들고 있는 객체의 **필드만** 바꾸면 시그널 자체는 변경 알림을 보내지 않는다(참조가 그대로이기 때문). 양방향 바인딩 입력의 변경 이벤트에 `$mark()` 를 묶어 호출한다.
...
<sd-checkbox [(value)]="filter().isIncludeLeft" (valueChange)="filter.$mark()">퇴사자 포함</sd-checkbox>
```
...
- 객체 시그널(`filter`, `data`)은 `시그널.$mark()`, 배열 시그널(`items`)도 `items.$mark()` 로 알린다.
- `[(value)]="filter().name"` 같이 시그널이 반환한 객체의 필드를 직접 양방향 바인딩하므로, 그 필드 변경을 외부에 알리려면 반드시 `(valueChange)="filter.$mark()"` 를 함께 둔다. 빠뜨리면 입력은 반영되지만 그 값에 의존하는 `$computed` / `$effect` 가 재발화하지 않는다.

`AbsSdDataSheet` / `AbsSdDataDetail` 를 상속한 화면은 `items` / `filter` / `data` 같은 표준 시그널을 기반 클래스가 제공하므로 직접 선언하지 않는다(자세히는 [client-data-sheet.md](./client-data-sheet.md)). 위 `$mark()` 호출 규약은 동일하게 적용된다.

---

## 권한(`usePermsSignal`)으로 표시·편집을 가드한다

화면 권한은 `usePermsSignal(<권한 path 목록>, <확인할 action 목록>)` 으로 받는다. 반환 시그널을 `perms` 라는 이름으로 둔다.
...
perms = usePermsSignal(
  ["base.user"],
  ["use", "edit", "auth.use", "auth.edit", "personal.use", "personal.edit", "payroll.use", "payroll.edit"],
);
...
- 둘째 인자: **확인할 action 목록**. `perms()` 의 반환값은 현재 사용자가 보유한 action 의 문자열 배열이다.

**사용 패턴** — `perms().includes("...")` 를 인라인으로 쓴다.
...
권한 + 추가 조건이 **여러 곳에서 같은 결합으로 반복 참조**될 때만 `$computed` 로 묶는다. `AbsSdDataSheet` 는 `canUse` / `canEdit` 를 오버라이드 멤버로 요구하므로 여기에 결합 결과를 둔다.
...
`AbsSdDataSheet` 는 `canUse()` 가 false 면 접근 제한 화면을, `canEdit()` 이 false 면 편집/저장 도구를 비활성으로 처리한다(상세 동작은 [client-data-sheet.md](./client-data-sheet.md)). `Abs*` 를 상속하지 않는 화면에서는 위 `@if (perms().includes("use"))` 처럼 직접 가드한다.

---

## 비동기 작업을 `SdToastProvider.try` 로 감싸고, `busyCount` 로 busy 를 표시한다

데이터 로드·저장 같은 비동기 작업은 `SdToastProvider.try(async () => { ... })` 로 감싼다. 콜백 안에서 throw 된 `Error` 는 danger 토스트로 표시되고 시스템 로그에 기록되며 외부로 전파되지 않는다. 작업 전후로 `busyCount` 를 증감해 busy 상태를 표시한다.
...
**표준 형태**: `busyCount + 1` → `try(async () => { ... })` → `busyCount - 1`. `busyCount` 증감은 `try` 바깥에 두어, try 내부에서 에러가 나도 항상 `- 1` 이 실행되게 한다.

**busy 표시는 컨테이너 컨트롤에 `busyCount() > 0` 를 바인딩**한다.
...
**최초 로드도 같은 패턴**으로, 생성자에서 `$effect([], async () => { ... })` 안에 둔다. 첫 로드 완료 시 `initialized` 를 true 로 set 해 컨테이너의 초기 busy 오버레이를 해제한다.
...
**메시지 직접 표시** — 성공/안내는 작업 성공 직후 직접 띄운다.
...
## 다른 화면을 모달로 띄운다 (`SdModalProvider.showAsync`)

다른 화면을 모달로 띄울 때는 `SdModalProvider.showAsync({ type, title, inputs })` 를 쓴다. 모달로 띄울 컴포넌트는 `ISdModal<O>` 를 구현해야 한다(`initialized: Signal<boolean>` + `close: output<O | undefined>`).
...
- **`type`** — `ISdModal` 을 구현한 컴포넌트 클래스. `GoodsDetail`(`AbsSdDataDetail` 상속, `ISdModal` 구현), `PasswordResetModal`(`implements ISdModal<void>`) 처럼 단건 편집 `Detail` 도 별도 `Modal` 없이 그대로 띄운다.
- **`title`** — 모달 헤더 제목. 등록/수정에 따라 분기해도 된다.
...
- 둘째 인자 `options` 의 `key` 는 모달 위치·크기를 저장할 식별자다(선택). 그 외 옵션(헤더 숨김·배경 클릭 닫기 등)은 [overlay.md](../apis/sd-angular/overlay.md) 참조.

모달로 띄워질 컴포넌트 쪽 구현(`close` output·`initialized`·`busyCount`)은 위 `PasswordResetModal` 코드를 따른다.

---

## flex 유틸 클래스와 `sd-base-container` 로 레이아웃을 잡는다

화면 영역 분할·배치는 글로벌 flex 유틸 클래스로 구성한다(자체 `styles` 작성은 최후 수단).
...
**`sd-base-container`** 는 `Abs*` 를 쓰지 않는 화면(특히 PDA·리포트 화면)의 공통 골격이다. busy·초기화 오버레이·제목 영역을 제공하고, 본문은 `#contentTpl` 슬롯에 둔다.
...
- 본문은 반드시 `<ng-template #contentTpl>` 안에 둔다(이 슬롯이 필수).
- `AbsSdDataSheet`/`AbsSdDataDetail` 를 상속한 목록/편집 화면은 `<sd-data-sheet>`/`<sd-data-detail>` 이 내부에서 `sd-base-container` 역할을 겸하므로 직접 감싸지 않는다.

폼 항목 정렬에는 `form-table`(`<table>` 기반, `<th>` 라벨 + `<td>` 입력)과 검색 폼의 `<div><label>...</label><입력></div>` 나열(`#filterTpl` 안)을 쓴다. 폼 컨트롤 상세는 [client-form.md](./client-component.md) 참조.

---

## 아이콘을 사용한다

v12 화면은 **`@fortawesome` 아이콘 + `FaIconComponent`** 를 쓴다. 사용할 아이콘을 단일 import 경로에서 가져와 컴포넌트 클래스에 `protected readonly` 멤버로 노출한 뒤, 템플릿에서 `<fa-icon [icon]=...>` 로 바인딩한다.
...
**아이콘 패키지는 프로젝트마다 다르므로 대상 코드의 실제 import 를 확인해 따른다.**
...
- 아이콘은 `import { faXxx } from "@fortawesome/.../faXxx"` 처럼 **개별 경로로 import** 한다(트리셰이킹).
- 클래스에 `protected readonly faXxx = faXxx;` 로 노출하고 템플릿에서 `[icon]="faXxx"` 로 바인딩한다.
- `imports` 에 `FaIconComponent` 를 반드시 추가한다.

---

## inject 멤버를 `#private` 로 명명한다

`inject()` 한 의존성은 외부에 노출되는 멤버(시그널·input·output·공개 메서드)와 구분하기 위해 **hard private(`#`) 필드**로 둔다.
...
- 시그널(`perms`, `data`, `busyCount`, `initialized`), `input()`, `output()`, 공개 메서드(`onSubmit` 등)는 prefix 없이 공개 멤버로 둔다.
- 템플릿에서 참조해야 하는 멤버(아이콘 상수 등)는 `protected readonly` 로 둔다 — `#` private 은 템플릿에서 접근할 수 없기 때문이다(예: `protected readonly faCheck = faCheck;`).
