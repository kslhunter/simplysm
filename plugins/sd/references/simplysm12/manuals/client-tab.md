...
`<sd-tab>` 은 **현재 탭 값을 고르는 선택 컨트롤**일 뿐, 콘텐츠 컨테이너가 아님. 탭바는 어떤 값이 선택됐는지만 양방향 바인딩으로 알려주고, 선택값에 따라 무엇을 보여줄지는 `<sd-tab>` **바깥에서** Angular 제어 흐름(`@if`)으로 직접 분기해 렌더링함. (`sd-tab.control.ts` 의 template 은 `<ng-content>` 뿐이며, `value = model<any>()` 하나만 노출함. `sd-tab-item.control.ts` 는 부모 `value` 와 자기 `value` 를 비교해 선택 표시만 함.)

## 탭바를 구성하고 선택값을 바인딩하려면

`<sd-tab>` 에 `[(value)]` 로 시그널을 양방향 바인딩하고, 각 탭을 `<sd-tab-item value="...">` 로 나열함. 탭 라벨 텍스트는 `<sd-tab-item>` 의 콘텐츠로 넣음. 컴포넌트 `imports` 에 `SdTabControl`, `SdTabItemControl` 을 명시함(둘 다 `@simplysm/sd-angular`).

선택값 시그널은 **literal union** 으로 타입을 좁혀 오타·리네이밍 안전성을 확보하고, 초기값은 반드시 자식 `<sd-tab-item>` 중 하나의 `value` 와 일치시킴. 어느 항목과도 일치하지 않으면 모든 탭이 미선택 상태로 시작함.
...
콘텐츠는 `<sd-tab>` 안이 아니라 그 **바깥**에서 `selectedTab()` 값을 `@if` 로 비교해 렌더링함. 탭바는 상단에 두고 본문은 `flex-fill` 영역에서 분기하는 구조가 표준임.
...
탭 항목 수가 고정이 아니라 데이터에서 동적으로 결정되면 `@for` 로 탭을 펼침. 이때 선택값 시그널 초기값을 미리 지정할 수 없으므로, 탭 목록이 정해지는 시점에 `$effect` 로 첫 항목을 선택해 줌.
...
라벨은 `<sd-tab-item>` 의 콘텐츠이므로 보간식을 그대로 넣을 수 있음. 카운트가 0 이면 숨기고 1 이상일 때만 괄호로 붙이는 식으로 조건부 표기함.
...
선택값은 `model()` 기반이라 양방향 바인딩과 별개로 `(valueChange)` 출력을 함께 받을 수 있음. 탭이 바뀔 때 마지막 선택 탭을 사용자별로 저장해 두었다가 다음에 복원하는 식으로 씀.
...
복원은 선택 항목이 바뀌는 시점에 저장값을 읽어 `selectedTab.set(...)` 함.
...
- `<sd-tab>` 은 선택 컨트롤임. 콘텐츠를 `<sd-tab>` 안에 넣지 말고 바깥에서 `@if (selectedTab() === ...)` 로 분기함.
- 선택값 시그널은 literal union 으로 타입을 좁히고(`$signal<"할일" | "메뉴">("할일")`), 초기값을 자식 `<sd-tab-item>` 의 `value` 중 하나와 일치시킴. 일치하는 항목이 없으면 전부 미선택으로 시작함.
- 탭을 `@for` 로 동적 생성할 때는 초기값을 정적으로 줄 수 없으므로 `$effect` 로 첫 항목을 선택해 미선택 시작을 막음.
- 라벨 보간식의 카운트는 `> 0` 일 때만 노출하는 조건부 표기를 씀.
- 컴포넌트 `imports` 에 `SdTabControl`, `SdTabItemControl`(`@simplysm/sd-angular`)을 명시함.

> 트리뷰처럼 콘텐츠 영역까지 컨트롤이 직접 담당하는 탭이 필요하면 `<sd-tabview>` / `<sd-tabview-item>` 을 씀(예: centurymes `LotTrackingPage.ts` 의 역추적/정추적). 본 매뉴얼이 다루는 `<sd-tab>` 과는 별개 컨트롤이며, 본문 분기를 직접 제어하려면 `<sd-tab>` 을 씀.
