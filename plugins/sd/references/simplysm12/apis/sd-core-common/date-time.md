...
C# DateTime 스타일의 날짜·시간 값 객체. `set*`/`add*` 메서드는 모두 **새 인스턴스 반환**(원본 불변). getter/setter 프로퍼티(year/month/...)는 내부 Date 를 직접 변형함(가변). 비교는 `.tick` 으로(배열 정렬·`ObjectUtils.equal` 이 활용). 포맷은 `DateTimeFormatUtils` 토큰(README 참조).
