...
클래스/프로퍼티에 붙여 테이블 정의(`ITableDef`)를 `Reflect` 메타데이터로 누적한다. 정의는 `DbContext.tableDefs`/`Queryable`/초기화·마이그레이션에서 읽힌다. 모든 데코레이터는 `@simplysm/sd-core-common` 의 `TClassDecoratorReturn`/`TPropertyDecoratorReturn` 을 반환. `@Column` 의 타입 추론은 `Reflect.getMetadata("design:type")` 에 의존하므로 emitDecoratorMetadata 활성화 필요.
