...
책무:
  1. ${CLAUDE_PLUGIN_ROOT}/rules/*.md 행동·설계 규칙 주입.
  2. 프로젝트의 @simplysm/sd-cli major → 활성 references 카탈로그 주입.
  3. (part 0 에서만) statusline.py 를 ${CLAUDE_PLUGIN_DATA} 로 복제 + ~/.claude/settings.json
...
# Codex 는 native PLUGIN_ROOT 도 set(Claude 는 CLAUDE_PLUGIN_ROOT 만). statusline 은 Claude 전용.
...
# --- 1. rules 주입 (공유 rules/*.md + 호스트별 _host-<host>.md) ---
try:
    if PLUGIN_ROOT:
        ctx_dir = Path(PLUGIN_ROOT) / "rules"
        for md in sorted(ctx_dir.glob("*.md")):
            if md.name.startswith("_host-"):
                continue  # 호스트별 보강 룰은 매칭 호스트만 주입(아래)
            out.append(md.read_text(encoding="utf-8"))
        host_md = ctx_dir / ("_host-codex.md" if IS_CODEX else "_host-claude.md")
        if host_md.is_file():
            out.append(host_md.read_text(encoding="utf-8"))
except Exception:
    pass
...
# --- 3. statusline 복제 + settings 멱등 주입 (part 0 에서만; 다중 part race·중복 방지) ---
# Codex 는 커스텀 statusline 미지원 → skip (Claude 의 ~/.claude/settings.json 을 건드리지 않음).
if PART == 0 and not IS_CODEX:
    try:
    ...
    if IS_LAST:
        ...
        if len(rest) > 1:
            warn = (
                ...
                f"해결: `plugins/sd/hooks/{'hooks-codex.json' if IS_CODEX else 'hooks.json'}` 의 SessionStart 에 `--part N` command 를 늘리고"
