...
# git 차단 기본값. 통과 경로는 둘:
#  1. 조회만 가능한 형태는 항상 허용 — status/diff/log/show/check-ignore(인자 무관 부작용 없음)와
#     tag -l/--list(목록 조회). 단 명령 내 모든 git 호출이 조회이고, 서브커맨드가 `git` 바로 뒤에 와야 허용
...
GIT_READONLY = CMD_POS + r"git\s+(?:(?:--no-pager|-C\s+" + GIT_DIR + r")\s+)*(?:status|diff|log|show|check-ignore|tag\s+(?:-l|--list))\b"
...
if not git_allowed:
    ...
    if all_git and not all_git <= readonly_git:
        deny("git (forbidden by default; read-only status/diff/log/show/check-ignore and tag -l/--list are allowed; use the commit skill for commits).")
