...
import { access, mkdir, open, readFile, realpath, stat, unlink, writeFile } from "node:fs/promises";
...
export function registerWriteHashGuard(pi: ExtensionAPI) {
  ...
  return {
    ...
    async writeFile(absolutePath, content) {
      ...
      await withFileLock(lockKey, async () => {
        ...
        readHashes.set(nextKey, { hash: sha256Text(content) });
      });
    },
  };
}
...
async function recordReadHash(absolutePath: string, content: Buffer): Promise<void> {
  ...
  readHashes.set(key, { hash: sha256(content) });
}

async function resolveFileKey(filePath: string): Promise<string> {
...
async function withFileLock<T>(fileKey: string, fn: () => Promise<T>): Promise<T> {
  ...
  const lockPath = join(LOCK_DIR, `${sha256Text(fileKey)}.lock`);
