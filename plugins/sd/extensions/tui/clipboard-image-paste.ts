  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.setEditorComponent((tui, theme, keybindings) => {
      ...
      editor.handleInput = (data: string) => {
        ...
        isPastingImage = true;
        void pasteWindowsClipboardImage(editor, imageMarkers, ctx.ui.notify.bind(ctx.ui), () =>
          tui.requestRender(),
        ).finally(() => {
...
function installImageMarkerExpansion(
  ...
  if (segment) {
    editorWithSegment.segment = (text, mode) =>
      segmentImageMarkers(text, segment(text, mode), imageMarkers);
  }
...
function segmentImageMarkers(
  ...
  for (const segment of baseSegments) {
    while (
      markerRangeIndex < markerRanges.length &&
      markerRanges[markerRangeIndex]!.end <= segment.index
    ) {
      markerRangeIndex += 1;
    }
...
`;
  ...
  if (result.timedOut) {
    throw new Error(
      `PowerShell 클립보드 읽기가 ${POWERSHELL_TIMEOUT_MS}ms 안에 끝나지 않았습니다.`,
    );
  }
