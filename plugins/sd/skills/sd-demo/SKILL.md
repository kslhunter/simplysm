...
- 화면 명세의 출처 — 다음 중 하나:
    - spec.md 경로 (있을 때) — sd-spec-si(SI) 의 "화면"(§4) 섹션의 화면 항목을 명세로 사용. (범용 sd-spec 의 spec.md 는 화면 단위가 없어 본 스킬 대상 아님.)
...
**입력 spec.md 읽기 범위** (spec.md 사용 시): 대상 화면 항목만 추출하기 전 ① 대상 화면(§4.x) 본문 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[모델.X]`·`[공통 정의.X]` 등)로 가리키는 § 를 재귀적으로(참조의 참조까지) → ③ §10 본문 외 확정 사항 전체 를 읽음. 그 화면 관련 결정·제약을 다른 세션이 §10(전역) 또는 해당 단위의 진행방법 NOTE·인라인 `[OPEN]`(단위 한정)에 남겼을 수 있고, 형식이 그렇게 모이도록 규정함. 부분 검색·부분 읽기로 §4 대상 화면만 보고 §10·참조 § 를 빠뜨리지 않음.
