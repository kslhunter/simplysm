...
**입력 spec.md 읽기 범위** (spec.md 사용 시): 대상 화면 항목만 추출하기 전 ① 대상 화면(§4.x) 본문 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[모델.X]`·`[공통 정의.X]` 등)로 가리키는 § 를 재귀적으로(참조의 참조까지) → ③ §10 본문 외 확정 사항 전체 를 Read. 그 화면 관련 결정·제약을 다른 세션이 §10(전역) 또는 해당 단위의 진행방법 NOTE·인라인 `[OPEN]`(단위 한정)에 남겼을 수 있고, 형식이 그렇게 모이도록 규정함. grep·부분 읽기로 §4 대상 화면만 보고 §10·참조 § 를 빠뜨리지 않음.
...
- `@simplysm/*` 사용 시 — SessionStart 가 주입한 활성 references 경로의 `README.md` Read 후 "개발 매뉴얼" 표에서 현재 작업 트리거에 해당하는 항목 진입.
