...
- 화면 명세의 출처 — 다음 중 하나:
  - spec 경로 (있을 때) — sd-spec-si(SI) spec bundle 루트(`.specs/{title}/`) 또는 그 루트 `spec.md`. 루트 `spec.md` §4 인덱스 표에서 대상 화면 행을 찾아 그 `파일` 컬럼의 page(`design/screens/*.md`) 본문을 명세로 사용. (범용 sd-spec 의 spec 은 화면 단위가 없어 본 스킬 대상 아님. 과거 단일 파일 spec.md 는 §4 섹션 본문을 그대로 사용.)
...
**입력 spec 읽기 범위** (spec 사용 시): 대상 화면 항목만 추출하기 전 ① 루트 `spec.md` §4 인덱스로 대상 화면(§4.x) page 를 찾아 본문을 읽음 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[모델.X]`·`[공통 정의.X]` 등)로 가리키는 §/page 를 재귀적으로(참조의 참조까지) → ③ 루트 `spec.md` 의 §10 결정 맥락·§11 작업 메모 전체 를 읽음. 그 화면 관련 결정·제약을 다른 세션이 §10(전역) 또는 해당 단위 page 의 NOTE·인라인 `[OPEN]`(단위 한정)에 남겼을 수 있고, 형식이 그렇게 모이도록 규정함. 부분 검색·부분 읽기로 대상 화면 page 만 보고 §10·§11·참조 page 를 빠뜨리지 않음.
