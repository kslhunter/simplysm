...
- 화면 명세의 출처 — 다음 중 하나:
  - spec 경로 (있을 때) — sd-spec-si(SI) spec bundle 루트(`.specs/{title}/`) 또는 그 루트 `spec.md`. 루트 `spec.md` §4 인덱스 표에서 대상 화면 행을 찾아 그 `파일` 컬럼의 page(`design/screens/*.md`) 본문을 명세로 사용. (범용 sd-spec 의 spec 은 SI식 화면 명세 형식(§4 화면 인덱스·와이어프레임·항목표)이 없어 본 스킬 대상 아님. 과거 단일 파일 spec.md 는 §4 섹션 본문을 그대로 사용.)
...
1. 루트 `spec.md` §4 인덱스로 대상 화면(§4.x) page 를 찾아 본문을 읽음.
