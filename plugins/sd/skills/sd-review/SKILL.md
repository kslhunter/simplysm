...
리뷰 요청 시 메인 루프가 직접 오케스트레이션. 계획을 직접 세운 뒤, 대상 규모에 따라 차원별 전수 룰 대조와 발견별 적대 검증을 독립 서브에이전트로 펼치거나 직접 수행하고, 검증 통과분을 병합·분류·적용·결정 처리.
...
1. **대상 식별** — 구체 리뷰 단위로 확정.
   - 경로/디렉터리면 파일 목록을 수집 (dist/.back/node_modules/.gitignore 등재 경로 제외).
   - 자연어 설명이면 직접 검색으로 산출물을 찾아 단위 확정.
   - 각 단위 경로는 직접 읽을 수 있는 형태로 확보. 사용자가 절대경로(다른 워크스페이스 등)를 줬으면 그대로 보존 — 상대경로로 깎지 말 것. 현재 레포 내부 대상일 때만 레포 기준 상대경로 사용.
...
3. **적용 룰 동적 발견** — 도메인에 맞는 룰 소스를 실제로 찾아 경로를 적음.
   ...
   - 도메인 관련 매뉴얼 — SessionStart 가 주입한 활성 references 경로 하위 (예: simplysm 화면이면 `manuals/client-*.md`, `orm.md` 등; 실제로 직접 존재 확인).
   - 도메인 관련 스킬 정의 (예: spec.md 면 골격으로 분기 — SI 골격(§1~10·화면·업무 프로세스)이면 `sd-spec-si`, 범용 골격(§1~8·산출 단위)이면 `sd-spec`; 매뉴얼이면 `sd-manual`).
...
- **Review**: 차원이 2개 이상이거나 단위 파일이 많아 한 컨텍스트에 다 읽으면 과부하 → 차원마다 서브에이전트 펼침. 차원 1개·소형이면 메인 루프가 직접 전수 대조.
- **Verify**: 적대 검증의 독립성(발견자 ≠ 검증자)이 핵심. **발견이 1건이라도 검증은 독립 서브에이전트로 수행함이 기본** — 메인 루프는 발견자이므로 자기검증 편향을 피해야 함. 발견이 다수면 발견마다 서브에이전트 병렬. 예외: 대상이 극히 작고 발견이 명백한 형식 항목(자동 분류)뿐이면 직접 가능.
- 서브에이전트 다수를 펼칠 때는 **한 응답에 dispatch 를 여러 개** 두면 병렬 실행(응답당 하나면 순차).

### 4. Review — 전수 룰 대조

각 차원에 대해 (서브에이전트 또는 직접) 아래를 수행. 서브에이전트로 펼칠 때 프롬프트:
...
적용할 룰 소스(존재 파일은 전부 읽기; auto-injected/existing-pattern 는 컨텍스트·코드베이스 조사로 처리):
- <ruleSources...>
...
발견이 0건이면 생략(7단계로). 1건 이상이면 각 발견을 (독립 서브에이전트가 기본) 적대 검증. 서브에이전트 프롬프트:
...
[1] 발견(문제) 검증: 인용된 룰이 실제로 그렇게 규정하는지, 대상이 실제 그 위치에서 그러한지 파일을 직접 읽어 확인. 과장·오인·룰 오인용이면 verdict=rejected, 불명확하면 uncertain, 사실이면 confirmed. 의심스러우면 기각 쪽. final_severity/final_category 재산정.
...
**fail-fast**: 펼친 서브에이전트 중 하나라도 실패(에러)하면 부분 결과로 진행 금지. "위반 없음" 으로 보고하지 말고 실패 사실을 알린 뒤 해당 서브에이전트만 재실행해 보완. 전건 정상 반환이어야 전 차원·전 발견 검증 완료로 간주.
