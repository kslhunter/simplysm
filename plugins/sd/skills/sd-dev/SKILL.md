...
sd-impl 은 외부 영속 산출물(spec.md)을 비교 기준으로 삼지만, sd-dev 의 권위 기준은 같은 LLM 이 보유한 대화 합의뿐임. 따라서 1단계에서 합의해 3단계 표로 굳힌 **합의된 산출 단위 + 입출력·동작·예외 동작** 목록을 입력으로 명시하여, 독립 에이전트가 그 목록을 비교 기준으로 코드와 항목 단위 1:1 대조하게 함. 이 목록이 검증의 **유일한 기준**이고 서브에이전트는 대화 히스토리를 볼 수 없으므로, 합의 내용을 누락·왜곡 없이 표·요약에 온전히 옮긴다 — 여기서 빠뜨린 합의는 검증에서도 걸러지지 못한다.
