...
- **자동 진행**: 의도 합의 후 워크플로는 자동 진행 — 단 파일 삭제·시연 등 사용자 확인 지점, 그리고 `결정 근거` 없는 분기·정의·값을 발견한 지점에서는 멈춰 질문·승인. 자동 진행이 이 멈춤보다 우선하지 않음.
- **대상 무제한 (산출물 종류)**: spec 단위 풀 구현 워크플로를 거칠 규모가 아닌 증분이면 산출물 종류로 한정하지 않음. 화면 일부 변경(버튼 추가·기존 화면 버그 수정 등)도 대상.

## 본질 (횡단 룰)

워크플로 전 단계 적용:

- **임의 채움 금지 (`결정 근거` 없으면 질문)**: 분기·정의·값·계산식·경계를 만나면 `결정 근거`(sd.md — 사용자 발언·코드베이스의 **일관된** 구현 패턴·공식문서 명시) 유무로 판정. 있으면 그에 따라 채우고, 없거나 불명확하면 즉시 멈춰 1건씩 질문. 추정·단발 사례·"추론 가능"을 근거로 대체 금지. 도메인 값·분기·계산식은 사용자 발언이 유일 근거 — 코드 추론으로 채우기 금지. 분석·분해·구현 어느 단계에서 발견하든 동일 적용. spec 같은 영속 권위 자료가 없어 모든 근거가 대화 합의이므로 적용 빈도가 더 높음.
...
`결정 근거`가 충분하면 묻지 않고 진행, 없거나 불명확한 항목은 가능한 안을 만들어 1건씩 `제안` (행동 규칙 "제안 규칙"). 코드베이스 참조는 **일관된** 구현 패턴만 근거 — 단발 사례·추정은 근거 아님. 외부 자료(예: 디자인 시안·샘플 데이터·접근 자격증명)가 필요하면 이 단계에서 사용자에게 요청.
...
**모호 발견 시**: 사이클 진행 중 1단계에서 합의되지 않은 분기·모호점을 발견하면 즉시 멈추고 사용자에게 다시 `제안`해 재합의. 자체 판단으로 채우기 금지 (본질 "임의 채움 금지").
...
- **매뉴얼 규약 정합 (횡단)**: 2-A 매뉴얼 규약 중 여러 항목·파일에 걸쳐 작성 후에만 드러나는 위반만 점검·수정 (개별 항목 준수는 4단계에서 선반영).
...
sd-impl 은 외부 영속 산출물(spec)을 비교 기준으로 삼지만, sd-dev 의 권위 기준은 같은 LLM 이 보유한 대화 합의뿐임. 따라서 1단계에서 합의해 3단계 표로 굳힌 **합의된 산출 단위 + 입출력·동작·예외 동작** 목록을 입력으로 명시하여, 독립 에이전트가 그 목록을 비교 기준으로 코드와 항목 단위 1:1 대조하게 함. 이 목록이 검증의 **유일한 기준**이고 서브에이전트는 대화 히스토리를 볼 수 없으므로, 합의 내용을 누락·왜곡 없이 표·요약에 온전히 옮긴다 — 여기서 빠뜨린 합의는 검증에서도 걸러지지 못한다.
