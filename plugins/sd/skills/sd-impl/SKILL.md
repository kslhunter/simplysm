...
- 대상 spec.md 경로.
- **spec 포맷 판정** — spec.md 골격으로 두 포맷 구분 (이후 모든 단계가 이 판정을 따름):
  - **SI 포맷** (sd-spec-si 산출 — §1~10, §4 화면·§5 자동 처리·§6 공통·기반): 단위 식별자 = `§4.x`(화면)/`§5.x`(자동 처리)/`§6.x`(공통·기반).
...
없으면 묻기. spec.md 에서 식별자 매칭이 안 되면 §4 단위 목록 표(SI=화면 목록 / 범용=산출 단위 목록)·§5·§6 헤더 목록에서 후보를 제시 후 확정.

**입력 spec.md 읽기 범위**: 경로·식별자 확보 후 ① 구현 대상 단위(SI=§4.x/§5.x/§6.x, 범용=§4.x) 본문 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[단위.X]`·`[모델.X]`·`[타입.X]`·`[공통 정의.X]` 등)로 가리키는 § 를 재귀적으로(참조의 참조까지) → ③ 본문 외 확정 사항 전체(SI=§10, 범용=§8) 를 읽음. spec.md 는 다른 세션이 누적한 결정의 영속 산출물이라 안 읽은 영역의 결정·제약을 추측으로 덮어쓰면 안 되며, 그 결정·제약은 전역이면 본문 외 확정 사항·단위 한정이면 그 단위의 진행방법 NOTE 나 인라인 `[OPEN]` 에 모이도록 spec 작성 스킬(SI=sd-spec-si / 범용=sd-spec) 형식이 규정함. 부분 검색·부분 읽기로 대상 § 만 보고 본문 외 확정 사항·참조 § 를 빠뜨리지 않음.

### 2단계: 외부 입력 점검

작업 대상 단위(SI=§4.x/§5.x/§6.x, 범용=§4.x)와 그 단위가 참조하는 § (SI=§5/§6/§7/§8/§9, 범용=§5/§6/§7) 항목을 점검:
...
대상 단위(SI=§4.x/§5.x/§6.x, 범용=§4.x)와 참조 § (SI=§5/§6/§7/§8/§9, 범용=§5/§6/§7) 본문을 **항목 단위까지 분해**하여 표 작성. 표는 assistant 응답으로 출력 (이후 5·6단계의 입력). 일치 항목은 손대지 않음(차이만 작업).
...
1. spec.md 의 §<단위> 본문 + 참조 § (SI=§5/§6/§7/§8/§9, 범용=§5/§6/§7) 항목을 다시 읽기.
2. 현재 구현 파일을 모두 읽기.
...
- 대상 spec.md 의 해당 구현 단위 헤더(SI=§4.x/§5.x/§6.x, 범용=§4.x; SI 화면은 (장치) 표기 뒤)에 `[구현]` 마커를 부착. 이미 `[구현]` 이 있으면 그대로 둠 (날짜·`[확정]` 부착 안 함).
