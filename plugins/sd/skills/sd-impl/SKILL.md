...
**입력 spec.md 읽기 범위**: 경로·식별자 확보 후 ① 구현 대상 단위(§4.x/§5.x/§6.x) 본문 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[모델.X]`·`[공통 정의.X]` 등)로 가리키는 § 를 재귀적으로(참조의 참조까지) → ③ §10 본문 외 확정 사항 전체 를 읽음. spec.md 는 다른 세션이 누적한 결정의 영속 산출물이라 안 읽은 영역의 결정·제약을 추측으로 덮어쓰면 안 되며, 그 결정·제약은 전역이면 §10·단위 한정이면 그 단위의 진행방법 NOTE 나 인라인 `[OPEN]` 에 모이도록 sd-spec 형식이 규정함. 부분 검색·부분 읽기로 대상 § 만 보고 §10·참조 § 를 빠뜨리지 않음.
...
1. **sd-spec SKILL.md 읽기** — 룰 인지: 부분 수정 인라인 경로·신뢰도 표기·형식 준수(§ 작성법 읽기)·이름 기반 참조 cascade.
2. **sd-spec "부분 수정 (인라인)" 경로로 편집** — 정밀 반영(도메인 답이 이미 확정된 인라인 `[OPEN]`·6단계 결정 반영)은 바로 편집, 초안·판단 필요분(섹션 `[OPEN]` 초안·구조 변경·모순 정정)은 진단·초안 제시 → 합의 후 편집. 수정분은 사후 보고.
