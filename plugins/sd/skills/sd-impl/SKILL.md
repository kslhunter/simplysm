...
- **spec 포맷 판정** — 루트 `spec.md` 골격과 하위 파일 구조로 포맷을 구분 (이후 모든 단계가 이 판정을 따름):
  - **SI wiki bundle 포맷** (sd-spec-si 산출 — 루트 `spec.md` + `requirements/`·`design/`·`common/`·`data/`): 단위 식별자 = `§4.x`(화면)/`§5.x`(자동 처리)/`§6.x`(공통·기반). 루트 인덱스로 대상 page 를 찾아 본문을 읽고, `[구현]` 마커는 해당 page 헤더에 붙인다.
  - **SI 단일 파일 포맷(legacy)** (과거 sd-spec-si 산출 — §1~10, §4 화면·§5 자동 처리·§6 공통·기반): 단위 식별자 = `§4.x`/`§5.x`/`§6.x`.
  - **범용 포맷** (sd-spec 산출 — §1~8, §4 산출 단위): 단위 식별자 = `§4.x`(산출 단위, `kind` 태그). 이후 "화면" 전제 지침은 단위 `kind` 로 읽음 — 화면 컴포넌트·브라우저 시연은 UI 성격 kind 에만, 그 외 코드 kind(api·cli 등)는 모듈·단위 테스트·실행 확인으로 대체.
- 구현할 단위 식별자.

없으면 묻기. 식별자 매칭이 안 되면 포맷별 후보를 제시 후 확정한다: SI wiki bundle 은 루트 인덱스 표(§4~§6), SI 단일 파일 legacy 는 §4 화면 목록 + §5·§6 헤더, 범용 포맷은 §4 산출 단위 목록.

**입력 spec 읽기 범위**: 경로·식별자 확보 후 ① 구현 대상 단위(SI=§4.x/§5.x/§6.x, 범용=§4.x) 본문을 읽음. SI wiki bundle 은 루트 인덱스로 대상 page 를 찾고 그 page 를 읽음 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[단위.X]`·`[모델.X]`·`[타입.X]`·`[공통 정의.X]` 등)로 가리키는 §/page 를 재귀적으로(참조의 참조까지) 읽음 → ③ 본문 외 확정 사항 전체(SI=루트 `spec.md` §10, 범용=§8) 를 읽음. spec 은 다른 세션이 누적한 결정의 영속 산출물이라 안 읽은 영역의 결정·제약을 추측으로 덮어쓰면 안 되며, 그 결정·제약은 전역이면 본문 외 확정 사항·단위 한정이면 그 단위의 진행방법 NOTE 나 인라인 `[OPEN]` 에 모이도록 spec 작성 스킬(SI=sd-spec-si / 범용=sd-spec) 형식이 규정함. 부분 검색·부분 읽기로 대상 본문만 보고 본문 외 확정 사항·참조 본문을 빠뜨리지 않음.
...
4. **마커 제거** — 추적된 §4.x/§5.x/§6.x 헤더의 `[구현]` 마커 제거 (SI wiki bundle 은 해당 page 헤더, 단일 파일은 해당 섹션 헤더). 합의 절차도 같은 절 따름.
...
- **spec 인용**: spec 위치(단일 파일은 § 위치, SI wiki bundle 은 page 경로 + ref/절 위치) + 본문 문구.
...
**대상**:
- spec 경로: <단일 spec.md 절대경로 또는 SI wiki bundle 루트 절대경로 또는 해당 bundle 의 spec.md 절대경로>
- 단위 식별자: <§4.x / §5.x / §6.x>
...
1. spec 의 §<단위> 본문 + 참조 §/page (SI=§5/§6/§7/§8/§9, 범용=§5/§6/§7) 항목을 다시 읽기. SI wiki bundle 이면 루트 `spec.md` 인덱스에서 대상 page 를 찾고 참조 page 까지 읽기.
2. 현재 구현 파일을 모두 읽기.
...
- 대상 spec 의 해당 구현 단위 헤더(SI=§4.x/§5.x/§6.x, 범용=§4.x; SI 화면은 (장치) 표기 뒤)에 `[구현]` 마커를 부착. SI wiki bundle 은 해당 단위 page 헤더에 붙이고, 단일 파일은 해당 섹션 헤더에 붙인다. 이미 `[구현]` 이 있으면 그대로 둠 (날짜·`[확정]` 부착 안 함).
