...
디버깅 요청 시 메인 루프가 직접 오케스트레이션. 관점을 직접 도출한 뒤, 문제 규모에 따라 관점별 가설 발굴과 가설별 검증·해결책·적대검증을 독립 서브에이전트로 펼치거나 직접 수행하고, 검증된 해결책을 병합·우선순위화·결정 처리.

## 디버깅 원칙 (전 단계 공통)

발굴·검증·적대검증 서브에이전트에 항상 주입하고, 메인 루프가 직접 수행할 때도 동일하게 적용:

- 모든 판정은 실제 코드/설정을 직접 읽어 확인. 근거 없는 추측·일반론 금지(추측은 '가설'로만 등록, 사실로 단정 금지).
...
- 입력 정보가 부족하면 코드베이스를 직접 검색·조사해 보강.
...
- **가설 발굴**: 관점이 여러 개이고 코드베이스 조사 범위가 넓으면 → 관점마다 서브에이전트 펼침(각 서브에이전트는 자기 관점만 보므로 사각지대가 줄고 재현율이 오름). 관점이 적고 조사 범위가 좁으면 메인 루프가 직접 발굴.
- **검증·적대검증**: 검증자가 가설을 발굴자와 무관하게 코드로 재확인하는 독립성이 핵심. **가설이 1건이라도 검증·적대검증은 독립 서브에이전트로 수행함이 기본**. 가설이 다수면 가설마다 서브에이전트 병렬.
- 서브에이전트 다수를 펼칠 때는 **한 응답에 dispatch 를 여러 개** 두면 병렬 실행(응답당 하나면 순차).

### 4. 가설 발굴

(서브에이전트 펼침 또는 직접) 관점별로 원인 가설을 발굴하고, 어떤 관점에도 매이지 않는 자유탐색 가설도 1벌 추가(사각지대 안전망).
...
자유탐색 서브에이전트는 위에서 `perspective='free'`, 정해진 관점 목록에 잘 안 들어가는 원인일수록 가치가 크다는 지시로 1벌.
...
각 가설을 (독립 서브에이전트가 기본 — `subagent_type: sd:sd-verify`, 검증 전용 read-only·xhigh) 코드를 직접 읽어 검증하고, 통과 시 같은 근거 위에서 해결책까지 도출. 서브에이전트 프롬프트:
...
통과 가설의 각 해결책을 (독립 서브에이전트가 기본 — `subagent_type: sd:sd-verify`, 검증 전용 read-only·xhigh) 4관점에서 적대적으로 공격. 해결책마다 서브에이전트 1개. 프롬프트:
...
**fail-fast**: 펼친 서브에이전트 중 하나라도 실패(에러)하면 부분 결과로 진행 금지. 실패 사실을 알린 뒤 해당 서브에이전트만 재실행해 보완. 전건 정상이어야 전 가설·전 해결책 검증 완료로 간주.
...
채택 해결책 후보가 1건 이상이면 행동 규칙 "응답 방식·결정 질의 규칙" 의 결정 진행 모드로 전환(우선순위 순). 사용자가 고른 해결책만 실제 수정에 착수.
