...
각 가설을 (독립 서브에이전트가 기본 — `subagent_type: sd:sd-verify`, 검증 전용) 코드를 직접 읽어 검증하고, 통과 시 같은 근거 위에서 해결책까지 도출. 서브에이전트 프롬프트:
...
통과 가설의 각 해결책을 (독립 서브에이전트가 기본 — `subagent_type: sd:sd-verify`, 검증 전용) 4관점에서 적대적으로 공격. 해결책마다 서브에이전트 1개. 프롬프트:
...
채택 해결책 후보가 1건 이상이면 행동 규칙 "제안 규칙" 대로 스스로 우선순위를 정해 가장 우선한 1건만 `제안`(전체 나열 금지). 사용자가 고른 해결책만 실제 수정에 착수.
