...
> sd-spec-si 가 산출하는 spec bundle 의 형식 모범. 실제 bundle 은 `.specs/{title}/` 아래 여러 `.md` 로 분리됨(루트 `spec-si.md` + 하위 page) — 이 예시는 그 파일들을 한 문서에 이어 붙여 보여주며, 각 page 는 `<!-- 경로 -->` 주석으로 경계를 표시함.
...
전용 모달(품목 등록·편집·Location 등록·편집)은 각각 호출 화면(품목 관리·Location 관리) 한 곳에서만 쓰이므로 별도 §4.x page 로 분리하지 않고 부모 page 안에 포함 (sub-section).

셸·프레임 UI 체크 결과 — 레이아웃 셸·전역 네비게이션·사이드바는 프레임워크 기본 제공 + [기반.앱 구조 정의] 가 클라이언트별 메뉴 배열을 담당하므로, 별도 `기타` 유형 셸 화면을 §4 에 두지 않음 (생략 근거).
...
- 박스 1개는 단일 품목·단일 Location 만 가짐 (박스 1개가 2개 Location 에 동시 존재 불가) (근거: 회의록.md L20-24)
- 입고 등록 시점 = 박스의 최초 Location 적치 시점 (재입고·이동은 본 프로세스에서 제외)
- 박스 바코드는 외부에서 이미 부착된 상태로 도착함 (창고에서 발급하지 않음) (근거: 회의록.md L15-18)
- ERP 통보는 박스번호를 키로 멱등 처리됨 (동일 박스 재시도 시 ERP 측이 중복 무시) (근거: 회의록.md L88-92)
