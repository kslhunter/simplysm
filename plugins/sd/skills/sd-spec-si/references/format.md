...
sub-section 헤더 레벨은 `## spec bundle 형식` 의 "sub-section 헤더 레벨" 표 참조.
...
설계 단계에서 §4/§5/§6 page 를 §2/§3 본문에서 도출함 (§8 도메인 모델은 이 단계의 입력이 아니라 §4~6 을 작성하며 파생됨). 절차:
...
- 장치: PC / PDA / 기타 중 택1.
- 구현: 구현 완료 단위는 `[구현]`, 미구현은 빈칸. 부착·제거는 "신뢰도 표기"·"§4~6 `[구현]` 마커 제거" 절 참조.
- 화면이 추가·제거되면 표도 함께 갱신. 같은 분류끼리 인접 배치.

### 화면 정의 (§4.x page 표준 구조)

화면 page 헤더에 ref·장치 표기를 유지: `# [화면.화면명] 화면명 (PC)` 또는 `(PDA)`. 구현 완료 표시(`[구현]`)는 page 헤더가 아니라 루트 인덱스 표(§4) `구현` 컬럼에 둔다.
...
자동 처리 page 헤더는 `# [자동 처리.처리명] 처리명` 형식으로 작성한다. 구현 완료 표시(`[구현]`)는 page 헤더가 아니라 루트 인덱스 표(§5) `구현` 컬럼에 둔다.
...
공통·기반 기능 page 헤더는 `# [기반.기능명] 기능명` 형식으로 작성한다. 구현 완료 표시(`[구현]`)는 page 헤더가 아니라 루트 인덱스 표(§6) `구현` 컬럼에 둔다.
...
**분류**: 설계 자연 도출 — §4~~6 설계가 참조한 모델에서 파생. §4~~§6 의 모델 참조·양식 매핑이 실제로 가리킨 엔티티·필드만 모아 정의. **§4~6 어디에서도 참조되지 않는 엔티티는 만들지 않음** (설계 산물로서 §4~6 의 종속).
...
**분류**: 설계 자연 도출 — §4~~6 설계(특히 §5 자동 처리)가 호출·연동하는 외부 인터페이스에서 파생. **§4~6 이 참조하지 않는 외부 인터페이스는 만들지 않음** (설계 산물로서 §4~~6 의 종속).
