...
초안 배치(메인 직접 수행)와 부분 수정(§1~9 편집)이 따르는 §별 작성법. 요건·공통 작성법(§1·§2·§3·§7)과 설계 작성법(§4·§5·§6·§8·§9)을 한 파일로 통합.
...
- **설계 입력**: 확정된 §1~3 + §7 을 입력으로 §4~6 을 도출하고, §4~6 이 참조한 모델·외부호출에서 §8·§9 를 파생.
...
설계 단계에서 §4/§5/§6 헤더를 §2/§3 본문에서 도출함 (§8 도메인 모델은 이 단계의 입력이 아니라 §4~6 을 작성하며 파생됨). 절차:
...
4. **§8 도메인 모델·§9 외부 인터페이스 파생 (점진 누적)** — §4~6 단위를 작성하면서 그 단위가 참조하는 `[모델.X.Y]`·`[외부 인터페이스.X]` 를 **그 자리에서 §8·§9 에 즉시 추가·갱신** (없으면 신설, 있으면 재사용·필드 확장). 끝나고 따로 모으는 집계 단계는 없음 — 누적이 곧 §8·§9 작성 (§8·§9 불변식은 §8·§9 섹션 머리 참조).

메인이 §4~6 을 한 줄기로 순차 작성하며 §8·§9 를 그때그때 누적하고, 화면 목록 표·모달/시트 재활용 교차참조도 단위가 늘 때마다 함께 정합. 도출 근거가 없는 항목은 [OPEN].
...
화면 목록 표에서 유형이 "마스터" 인 §4.x 중 편집을 모달로 처리하는 마스터(조회 전용 시트 + 편집 모달) 기준. 화면 정의 sub-header (와이어프레임·항목·동작) 와 함께 작성.
...
**분류**: 설계 자연 도출 — §4~6 설계가 참조한 모델에서 파생. §4 화면 항목 매핑(`[모델.X.Y]`)·§5 자동 처리·§7 양식이 실제로 가리킨 엔티티·필드만 모아 정의. **§4~6 어디에서도 참조되지 않는 엔티티는 만들지 않음** (설계 산물로서 §4~6 의 종속).
