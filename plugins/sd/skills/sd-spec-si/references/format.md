...
초안 배치(메인 직접 수행)와 부분 수정(루트 `spec.md` 및 하위 page 편집)이 따르는 §별 작성법. 요건·공통 작성법(§1·§2·§3·§7)과 설계 작성법(§4·§5·§6·§8·§9)을 한 파일로 통합.
...
1. **§4 화면 도출**:
   ...
   - 시스템 기반 UI — 레이아웃 셸·전역 네비게이션·사이드바 등 고객이 명시하지 않아도 앱에 필요한 UI 프레임 → `기타` 유형 화면. (요구만이 아니라 기반까지 보고 §4 를 뽑음)
   - 도메인 묶음 또는 장치별로 그룹화 → 루트 §4 화면 인덱스 표.
2. **§5 자동 처리 도출** — §2/§3 의 시스템 백그라운드 처리 (스케줄·이벤트 트리거·외부 자료 수집·적재) 를 추출.
3. **§6 공통·기반 기능 도출** — 판별 5번(앱 자체 독립 기반)만: 부팅 초기화·부트스트랩 등 화면·배치 수반도 공통 규칙도 아닌 것. 대개 프레임워크 기본이라 매뉴얼 참조 stub. 크로스컷(가드·캐싱)·화면(셸)·발송·규칙은 §7·§4 로 분해되므로 §6 은 비는 것이 정상이다.
4. **§8 도메인 모델·§9 외부 인터페이스 파생 (점진 누적)** — §4-6 단위 page 를 작성하면서 그 단위가 참조하는 `[모델.X.Y]`·`[외부 인터페이스.X]` 를 **그 자리에서 §8·§9 page 에 즉시 추가·갱신** (없으면 신설, 있으면 재사용·필드 확장). 끝나고 따로 모으는 집계 단계는 없음 — 누적이 곧 §8·§9 작성 (§8·§9 불변식은 SKILL.md 의 루트 섹션 구조 참조).

메인이 §4-6 page 를 한 줄기로 순차 작성하며 §8·§9 page 를 그때그때 누적하고, 루트 인덱스 표·모달/시트 재활용 교차참조도 단위가 늘 때마다 함께 정합. 도출 근거가 없는 항목은 `[OPEN]`.

### 화면 인덱스 표 (루트 §4)

루트 `spec.md` §4 의 7컬럼 구조: `ref | 분류 | 화면 | 유형 | 장치 | 파일 | 구현`.
