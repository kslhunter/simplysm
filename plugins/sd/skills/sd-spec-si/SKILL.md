---
name: sd-spec-si
description: SI/업무시스템 요구사항을 분석해 spec.md 로 구조화 — 초안 배치(요건→설계 순차 빌드) + init 배치(코드-first 역작성 + [구현] 마킹) + 부분 수정 인라인 처리. (SI/업무시스템 전용 — 라이브러리·문서·LLM Skill 등 범용 개발 산출물 spec 은 sd-spec.) Use when "요구사항 분석", "SI 분석", "업무시스템 spec 작성", "/sd-spec-si init" 을 요청할 때.
---
...
| arg `init` (`/sd-spec-si init [범위]`) — 기존 코드베이스를 근거로 신규 spec.md 역작성 | init 배치   | 메인 에이전트가 직접 순차 수행 (아래 "init 배치")    |
| 그 외 모든 지시 ("X를 Y로"·"A를 B와 합쳐"·"이 섹션 이상해"·"설계만 다시"·재진입 정정) | 부분 수정   | SKILL.md 인라인 처리 (아래 "부분 수정")        |

- 초안 배치·init 배치는 메인 에이전트가 자율 진행. 실행 중 사용자에게 묻지 않음 (모르면 [OPEN]) — requirement-source-handling.md 의 "사용자에게 즉시 확인" 항목도 배치 중에는 [OPEN] + 검토 패키지 보고로 대체 (부분 수정 등 대화형 연산에서는 원문대로 즉시 확인).
...
신규 spec.md 작성 지시("분석해줘"·"초안 작성"·"spec 작성" 류). 메인 에이전트가 spec.md 를 **한 줄기로 순차 빌드**함 (코딩하듯 — 앞 단위가 만든 §7·§8·§9 를 뒤 단위가 재사용·확장). 작성은 전부 메인 직접, **검증만 독립 서브에이전트**.

**준비**:

- 폴더·경로: "입출력" 의 폴더 규칙으로 신규 `.specs/{yyMMddHHmmss}_{slug}/spec.md` 경로를 만들고, "spec.md 골격" 으로 골격(§1~§10 빈 헤더)을 작성.
- 시스템명: 사용자와 합의한 시스템명(제목 `{name}`). 날짜: 현재 날짜를 `yyyy-MM-dd` 로 (§10 로그 타임스탬프용; PowerShell `Get-Date -Format "yyyy-MM-dd"`, bash `date +%F` 등). 자료(Requirement Source)는 직접 읽음.

**단계 (직렬 — 각 단계는 그 시점에 확정된 것만 다룸)**:

1. **요건 (§1~3 + §7)**: `references/format.md` 의 요건·공통 작성법(§1·§2·§3·§7)을 읽고, 공유 형식은 본 SKILL.md 의 "spec.md 형식" 절을 따름. 자료를 직접 읽어 §1 개요·§2 업무 프로세스(pivotal event 로 §2/§3 분할)·§3 기타 요구를 작성. 작성 중 나오는 도메인 어휘·공통 규격은 §7 에 누적. **§4~9 는 빈 헤더로 두고 손대지 않음** (설계 단계 이전이므로). 비자명 항목엔 근거, 모르면 [OPEN].

2. **설계 (§4~6 순차 빌드 + §8·§9·§7 점진 누적)**: `references/format.md` 의 설계 작성법(§4~9)을 읽음. §1~3·§7 을 입력으로 §4 화면 → §5 자동 처리 → §6 공통·기반 의 단위를 **하나씩 순차로** 작성. 각 단위(§4.x 등)를 작성하면서 그 단위가 참조하는 §8 도메인 모델·§9 외부 인터페이스를 **그 자리에서 즉시 spec 에 추가·갱신** (없으면 신설, 있으면 재사용·필드 확장)하고, 새 용어는 §7 에 누적. §4 첫머리 화면 목록 표도 단위가 늘 때마다 갱신. 별도 "집계" 단계는 없음 — 누적이 곧 §8·§9 작성임 (§8·§9 불변식은 "섹션 구조" 절 참조).

3. **검증 (독립 서브에이전트, 규모 비례)**: 작성 완료 spec 을, 작성한 메인이 아닌 **독립 서브에이전트로 검증**함 (자기검증 편향 차단). 검증 항목 — ① 근거 좌표 1:1 실재 대조(원자료 재독), ② 원자료↔spec 전수 누락 역대조 + 자료 커버리지, ③ 요구 커버리지(요건 → §4~6 역대조), ④ dangling 참조 검색. 규모(자료 수·근거 수)에 비례해 항목별/자료별로 서브에이전트를 병렬로 펼치되, 대상이 아주 작으면 메인 직접도 허용. 근거 위조로 판정된 항목은 `(근거: …)` 를 떼고 [OPEN] 으로 강등(원 좌표는 메모로 보존).
...
4. **단위 검토 + `[구현]` 마킹**: §4.x/§5.x/§6.x 단위마다 검토 서브에이전트 1개를 병렬 실행. 서브에이전트는 **보고 전용** — spec.md 를 수정하지 않음. prompt 는 아래 "단위 검토 서브에이전트 prompt".
...
**입력 spec.md 읽기 범위**: 기존 spec.md 에 대한 작업이므로 부분 수정 착수 전 ① 수정 대상 § 본문 → ② 그 본문이 이름 기반 참조(`[화면.X]`·`[모델.X]`·`[공통 정의.X]` 등 "본문 내 참조")로 가리키는 § 를 재귀적으로(참조의 참조까지) → ③ §10 본문 외 확정 사항 전체 를 읽음. 다른 세션이 누적한 결정·제약은 전역이면 §10·단위 한정이면 그 단위의 진행방법 NOTE 나 인라인 `[OPEN]` 에 모이도록 형식("신뢰도 표기"·"진행 방법 누적" 절)이 규정함. 부분 검색·부분 읽기로 대상 § 만 보고 §10·참조 § 를 빠뜨리지 않음.
...
§4.x/§5.x/§6.x 본문이나 그 본문이 의존하는 § 를 수정했을 때, 그 §4~6 헤더의 `[구현]` 마커를 제거할지 판정. sd-spec-si 부분 수정·sd-impl spec 수정 양쪽이 따르는 단일 출처(sd-impl 은 지금 재구현 중인 본 단위는 제외).
...
- **진행 방법**: **spec 전체** 작업에 대한 사용자 합의 진행 방식 (예: 작성 순서·범위 한정·전체 적용 규칙 등). 분석 단위에 한정된 진행 방법은 `## spec.md 형식` 의 "진행 방법 누적" 절 참조 — sd-spec-si 자신의 후속 참조용.
...
| `${CLAUDE_PLUGIN_ROOT}/references/requirement-source-handling.md` | Requirement Source 부정확성 처리 (STT 오타·화자 모호·발화 모호·도메인 용어 다의성) |
