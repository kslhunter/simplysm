...
```
python "${CLAUDE_PLUGIN_ROOT}/skills/sd-unpack/scripts/unpack.py" <입력파일 절대경로>
```
...
Windows + MS Office 필요함 (docx/pptx/xlsx 변환용). Python 패키지 의존은 `ensure_pip` 가 자동 처리함. COM(Component Object Model) 의존 미충족 시 해당 형식 핸들러는 throw 함.

## 결과 폴더

입력 파일 옆에 `<basename>_<ext>/` 폴더 생성함. 컨테이너 첨부는 같은 패턴으로 재귀적으로 풀림. 폴더 안 `_source.<ext>` + `README.md` 가 풀린 폴더의 식별 마커임.

동일 입력 재호출 시 기존 결과 폴더는 사전 삭제 후 재생성함 (이전 산출 잔존물이 섞이지 않게).

시각은 PNG, 텍스트·구조는 JSONL/JSON/MD 로 분리 출력함.
...
`attachments/` 안 컨테이너 첨부는 같은 패턴의 `<basename>_<ext>/` 폴더로 재귀 풀이함.

## JSONL 공통 규약

모든 jsonl 출력은 **한 줄 = 한 노드 (또는 한 행/셀)**. 빈 키 생략함. JSON 네이티브 타입 보존함. datetime → ISO 8601 문자열.
...
시트별 `.jsonl` — 분석 핵심 (값·number_format·수식·merges·hyperlinks·comments) 보존함. 시각 표시 (바탕색·border·폰트)·frozen·dims 는 미보존함 (PNG 가 시각 보조 역할, 필요 시 `_source.xlsx` 직접 추출).
...
- 첫 줄: `{"_meta":{"merges":["A1:C1",...], "number_formats":{"E1":"yyyy-mm-dd",...}, "hyperlinks":{"D5":"http://..."}, "comments":{"E3":"메모"}}}`.
  ...
  - `comments`: 셀 메모.
  - 비어있는 메타 키는 생략함 (모두 비면 `{"_meta":{}}`).
- 데이터 줄: `{"r":11, "A":"P001", "I":7800, "J":12.5, "_f":{"I":"=SUM(...)", "J":"=I11*1.5"}}`.
  - `r`: 1-based 행번호 (Excel 동일).
  - 열문자 키 (`A`·`B`·...·`AA`·...): 셀 값. 빈 셀은 키 생략함.
  - `_f`: 같은 행 수식 맵 `{열문자: 수식문자열}`. 수식 없는 행은 키 생략함.
- 빈 행도 `{"r":N}` 한 줄 유지함 → 읽기 offset = 행번호 (오프바이원 방지).
...
- Chartsheet 는 `sheets/<idx>_<name>.jsonl` 미생성함 (셀 없음).
- Chartsheet 의 차트 데이터: `charts/sheet<idx>_chart.data.json`.
- README 의 sheet_summaries 항목에 `(chart sheet — "...")` 명시함.
- 일반 시트·Chartsheet 를 통합한 시트 순서 (idx) 대로 보존함.
...
슬라이드별 `slides/<idx>_<title>.jsonl` 생성함. 원본 XML 순서 (shape_idx 순) 그대로 유지함. 시각 순서는 `pos` 좌표 기반으로 필요시 정렬함.
...
같은 표·차트의 노드들은 `shape_idx` 가 동일함.

## docx jsonl 규약

문서 단일 시퀀스 `content.jsonl` 생성함. 페이지 단위는 폐기함 (Word 렌더 산물이라 원본 구조와 무관). 원본 시퀀스는 python-docx 의 문단·표·이미지 시퀀스를 따름.
...
- 노드 인덱스 자동 매핑은 미적용함 (fitz·python-docx 의 텍스트 분할 차이로 오매핑 위험) — 페이지 text 와 content.jsonl 의 노드 text 를 직접 검색 비교함.

## pdf jsonl 규약

페이지별 `pages/<NNN>.jsonl` 생성함. PDF 페이지는 원본 단위임.
...
본문 안 `<img cid:...>` 가 첨부의 어느 파일인지 추적함.

- `images.rels.json`: `{"<cid>":"attachments/image001.png", ...}` (HTML 본문 안 cid → 첨부 파일명 매핑).
- HTML→평문 변환본의 원래 `<img>` 위치에 `![image001.png](attachments/image001.png)` placeholder 삽입함.
...
- 인라인 이미지 없으면 `images.rels.json` 미생성함.

## TNEF (winmail.dat) 풀이

Outlook RTF 메일이 첨부를 `winmail.dat` 단일 binary (TNEF, Transport Neutral Encapsulation Format) 로 패키징한 경우, `tnefparse` 로 내부 첨부를 추출하여 `attachments/` 에 풀어 둠. 원본 `winmail.dat` 도 유지함 (원본 보존).

내부 첨부가 컨테이너 (xlsx·pptx 등) 면 재귀 풀이함 (다른 첨부와 동일).
...
- `body.md`: 항상 별도 파일 (인라인 cutoff 폐기).
  ...
  - 없으면 text/html → 평문 추출.
- `body.from_html.md`: text/plain·HTML 둘 다 있을 때만 별도 생성함.
  - HTML→평문 변환본 (이미지 위치 placeholder 포함).
  - `body.md` 가 plain 이라 누락된 위치 정보를 보완.
- `body.html`: 원본 HTML (있을 때).
- `headers.json`: 모든 메일 헤더를 원본 그대로 보존함 (envelope + `X-Mailer`·`Authentication-Results` 등 모두). 동일 키 다수면 list 로 누적함.
  - `README.md` 헤더 섹션에는 표준 envelope 키만 표기함 (전체는 `headers.json` 직접 조회).

## xlsb 클린업

- legacy → xlsx 변환 시 `_converted.xlsx` 는 임시 폴더에서만 처리함 (산출 폴더에 잔존시키지 않음).
- VBA(Visual Basic for Applications) 매크로는 원본 코드 그대로 `macros/<모듈명>.vba` 에 저장함 (변형 금지).
- VBA 시트 객체명 ↔ raw 시트명 매핑은 `workbook.meta.json` 의 `sheet_code_map` 키에 저장함 (예: `{"Sheet1":"BOA","Sheet3":"Mapping"}`).

## 산출물 사용

후속 스킬 (sd-spec-si 등) 은 결과 폴더의 `README.md` 를 한 번 읽어 본문 위치·헤더·첨부 목록·손실 영역을 모두 파악 가능함. 컨테이너 첨부는 자체 `README.md` 를 가지므로 같은 방식으로 재귀 진입함.
