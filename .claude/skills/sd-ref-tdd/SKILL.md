---
name: sd-ref-tdd
description: TDD 방법론 가이드. 작업 유형 분류, 테스트 원칙, TDD 사이클 절차를 제공. 직접 호출하거나 다른 스킬이 Read하여 참조
argument-hint: "<대상 파일 경로> [추가 컨텍스트]"
---

# sd-ref-tdd: TDD 방법론 가이드

> [!IMPORTANT]
> **절대 중간에 멈추지 마라.** TDD 사이클(RED → GREEN → Refactor → Commit)을 반드시 끝까지 완수한다. 한 Phase가 끝나면 즉시 다음 Phase로 진행한다.

이 가이드에 따라 `$ARGUMENTS` 대상에 대해 TDD를 수행한다.

## 1. 작업 유형 분류

대상의 성격에 따라 테스트 전략을 결정한다.

| 작업 유형 | 테스트 전략 |
|----------|-----------|
| 코드 작업 (로직) | 프로젝트 테스트 환경(vitest) 확인 → 있으면 vitest 테스트 작성 및 실행 |
| 코드 작업 (로직, 테스트 환경 없음) | subagent 기반 `test.md` 작성 및 실행 |
| 코드 작업 (비로직, 텍스트/상수/설정 변경 등) | 테스트 생략 → GREEN Phase로 바로 진행 |
| 비코드 작업 (LLM용 문서, SKILL.md 등) | subagent 기반 `test.md` 작성 및 실행 |
| 비코드 작업 (일반 문서, README 등) | 테스트 생략 → GREEN Phase로 바로 진행 |

**복수 유형 혼합 시:** 한 작업이 여러 유형에 걸치면, 가장 엄격한 테스트 전략을 적용한다.

## 2. 테스트 시나리오 작성 원칙

테스트는 **상태(정적)가 아닌 동작**을 검증한다:
- 상태 (X): "파일에 특정 텍스트가 적혀있는지 확인" → 정적 검사에 불과
- 동작 (O): "실행했을 때 기대한 동작이 나타나는지 확인" → 실제 동작 검증

예:
- LLM용 문서: "SKILL.md에 '테스트 재실행' 문구가 있는지"(X) → "LLM이 SKILL.md를 따라 실행했을 때 실제로 테스트를 재실행하는지"(O)
- 코드: "함수가 존재하는지"(X) → "함수에 특정 입력을 넣으면 기대 출력이 나오는지"(O)

## 3. RED Phase: 테스트 작성 및 실패 확인

2단계의 원칙에 따라 테스트 시나리오를 작성하고 실행한다.

### vitest 테스트인 경우

1. 대상 파일을 분석하여 테스트 시나리오를 작성한다
2. vitest 테스트 파일을 작성한다
   - 프로젝트의 기존 테스트 파일 패턴을 탐색하여 (`*.test.ts`, `*.spec.ts`, `__tests__/` 등) 동일한 패턴으로 생성한다
   - 기존 패턴이 없으면 `{대상파일명}.test.{확장자}`로 대상 파일과 동일 디렉토리에 생성한다
3. Bash로 `vitest run {테스트 파일}` 실행하여 현재 상태를 확인한다

### subagent 테스트인 경우 (LLM용 문서, 테스트 환경 없는 코드 등)

1. 대상 파일을 분석하여 테스트 시나리오를 작성한다
2. `test.md` 파일을 작성한다
   - 형식:
     ```markdown
     ## 테스트 1: {테스트 제목}
     **입력:** {subagent에게 실행시킬 지시}
     **기대 결과:** {검증 기준}
     ```
3. Agent tool로 **테스트별 독립 subagent**를 병렬로 생성하여 실행한다
   - 각 테스트(`## 테스트 N`)마다 별도 subagent를 호출한다 (단일 메시지에서 병렬 호출)
   - subagent에게 전달할 내용: 대상 파일 경로, 해당 테스트 1건의 입력과 기대 결과, "테스트를 실행하고 PASS/FAIL을 판정하라"는 지시
   - 하나의 subagent에 여러 테스트를 전달하지 않는다 (컨텍스트 격리)

### 모든 테스트가 통과하면 (vitest/subagent 공통)

사용자가 문제/버그/이슈를 제기한 것인지, 단순 테스트 요청인지 판단한다.

**문제 제기가 아닌 경우 (단순 테스트 요청):**
- "모든 테스트가 이미 통과합니다. 수정이 필요하지 않습니다."를 출력하고 종료한다

**문제 제기인 경우:**
1. 현재 테스트가 문제를 재현하지 못한 것으로 판단한다
2. 이전 테스트가 왜 통과했는지(어떤 관점을 놓쳤는지) 분석한다
3. 문제를 재현할 수 있는 다른 관점에서 테스트 시나리오를 재작성한다
4. 재작성한 테스트를 실행한다
5. 여전히 통과하면 2~4를 반복한다 (최대 3회)
6. 3회 시도 후에도 모든 테스트가 통과하면:
   - 시도한 테스트 시나리오 목록을 정리하여 사용자에게 보고한다
   - "문제를 재현하는 테스트를 작성하지 못했습니다. 문제 상황을 더 구체적으로 설명해주세요."를 출력하고 중단한다

## 4. GREEN Phase: 구현 및 재테스트

테스트 실패 항목에 대해 대상 파일을 수정한다.
- 수정 후 테스트를 재실행한다
  - vitest: Bash로 `vitest run {테스트 파일}` 재실행
  - subagent: Agent tool로 테스트별 독립 subagent를 병렬로 재실행
- 여전히 실패하면 다시 수정 → 재테스트를 반복한다
- 최대 3회 반복한다. 3회 시도 후에도 실패하면 실패 원인을 분석하여 사용자에게 보고하고 중단한다

## 5. Refactor Phase

GREEN Phase에서 대상 파일을 수정한 경우에만 수행한다.

`/sd-simplify` 하여 코드를 리팩터링한다. 완료 후 멈추지 말고 즉시 다음 단계로 진행한다.
- 완료 후 테스트를 재실행하여 통과를 확인한다
- 리팩터링으로 테스트가 깨지면 수정 후 재확인한다

## 6. Commit Phase

TDD 사이클(RED → GREEN → Refactor) 완료 직후 `/sd-commit`을 실행하여 커밋한다.

- 작업 하나 = commit 하나. 여러 작업을 몰아서 한번에 커밋하지 않는다
- 커밋 완료 후 멈추지 말고 즉시 다음 작업으로 진행한다
