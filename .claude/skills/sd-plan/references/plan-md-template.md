...
위치: `.specs/{yyMMddHHmmss}/REQ-XXX-slug/plan.md`
...
- [ ] R1: <요구 제목>
  ...
  - **설계**: (UI/API/데이터 모델/외부 연동만. 단순 변경 생략)
    - UI: wireframe(텍스트 ASCII) + 이벤트 흐름 + 상태 변화. 매핑된 demo.md 있으면 "demo.md 참조"만 + 추가 결정
    - 그 외: 자연어
    - 근거: <항상 명시>
  - **변경 위치**:
    - `<파일경로>` - `<함수/컴포넌트명>`
...
| **설계** | (R 성격별 조건부) UI wireframe(텍스트 ASCII)·이벤트 흐름 / API 엔드포인트·스키마 / 데이터 모델 / 외부 연동 시퀀스. 단순 변경은 생략 |
