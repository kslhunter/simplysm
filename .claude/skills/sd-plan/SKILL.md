---
name: sd-plan
description: Feature의 요구명세와 구현계획을 작성하는 스킬. "요구명세 작성", "구현계획 작성", "spec 만들어줘", "plan 만들어줘" 등을 요청할 때 사용한다.
---

# sd-plan: Feature 설계

Feature 하나를 대상으로, 요구명세(What)와 구현계획(How)을 수립한다.

## 공통 규칙

### 범위 겹침

어느 단계에서든 현재 Feature 범위가 다른 Feature 영역과 겹치는 것을 발견하면, 사용자에게 확인 후 조정한다.

예: Feature 1.1 "사용자 인증"의 범위에 "권한 기반 접근 제어"가 있고 Feature 1.2가 "권한 관리"라면, "권한 기반 접근 제어"는 Feature 1.2로 이관이 적절할 수 있다.

## Step 1: 입력 확인

wbs 문서 경로와 Feature 번호가 필수이다. 없으면 `/sd-wbs`를 안내하고 종료한다.

## Step 2: Feature 분석

### 2-1. 범위 설정

wbs의 범위/경계를 기반으로 세분화하여 재분석한다. wbs는 큰 그림이므로 세부 동작이 빠져 있을 수 있다.

- **범위**: wbs의 세부 기능을 **구체적 동작 수준**으로 **MUST** 빠짐없이 재나열. wbs에 없는 동작도 추가.
- **경계**: wbs의 경계를 확인하고, 모호한 부분을 구체화.
- **근거**: 범위의 출처. 참조 파일/자료 경로가 있으면 기록.

### 세부 기능 5개 초과 시

세부 기능 **5개 초과** 시 SPIDR 축으로 분리안 제시 ("그대로 진행" 포함).
**CRITICAL: 분리 수락 시 wbs 문서를 수정하고 즉시 종료한다.** Step 3 이후로 진행하지 않는다. 분리된 각 Feature별로 `/sd-plan`을 재호출하도록 안내한다.

### 2-2. Rule / Example / Question

범위의 각 세부 기능에 대해 Rule, Example, Question을 도출한다.

- **Rule**: 비즈니스 규칙 (검증 조건, 제약, 정책)
- **Example**: Rule의 구체 사례 (입력 → 기대 결과)
- **Question**: 불확실한 사항

Question 도출 기법 — Feature 성격에 맞게 선택하고, 적용 기법명을 표기한다:

| 기법                            | 적용 시점          | 방법                                      |
|-------------------------------|----------------|-----------------------------------------|
| **Decision Table**            | 조건 조합이 있을 때    | 조건 조합 나열 → 빈 칸 = Question              |
| **Boundary Value Analysis**   | 숫자/범위가 있을 때    | 경계값 강제 → 미정의 경계 = Question              |
| **Equivalence Partitioning**  | 입력 분류가 있을 때    | 입력 분류 나열 → 미정의 분류 = Question            |
| **State Transition**          | 상태가 있는 Feature | 상태 x 이벤트 나열 → 미정의 전이 = Question        |
| **Perspective-Based Reading** | 최종 검토 시        | 테스터/개발자/사용자 관점 검토 → 빠진 것 = Question    |

형식 — Rule별 그룹화, `[근거: ...]` 태그:

```markdown
### Rule: 제목은 필수 [근거: wbs "태스크 생성 시 제목 필수"]
- Example: 제목 입력 → 저장 성공
- Example: 제목 비움 → 에러
- Question: 제목 최대 길이 제한?
```

### 2-3. 명확화

`sd-rule-clarify` 스킬의 규칙에 따라 범위, Question, 기타 불명확한 부분을 명확화한다.
답변에 따라 Rule/Example/Question을 갱신하며, 모든 Question이 해소될 때까지 **MUST** 반복한다. (추가 Question 발생 가능)
- **나쁜 예:** "예약 기능 포함이 결정됨 → 예약 세부규칙(대기열 정책, 미대출 시 자동취소 기간)도 확정" — 기능 포함 여부와 세부 수치/규칙은 별개다. 세부사항은 별도로 분류·질문한다.

해소된 결정사항은 `### 설계 결정` 테이블에 기록 (D1, D2, ... 순번 부여)

## Step 3: Gherkin 생성

- Example → Scenario, Rule → `Rule:` 그룹, 공통 전제 → `Background:`
- 스펙 문서 용도 (테스트 러너 미실행)

```gherkin
Feature: {번호} {이름}
  Background:
    Given {공통 전제}
  Rule: {Rule 이름}
    # 근거: {출처}
    Scenario: {Example 이름}
      Given / When / Then
```

## Step 4: 구현계획

### 코드베이스 탐색

**CRITICAL: 코드가 source of truth이다.** 문서가 아닌 실제 코드를 기준으로 파악한다.

- 관련 엔티티/모델 구조
- 기존 API 엔드포인트 및 패턴
- 사용 중인 프레임워크와 아키텍처 패턴
- 기존 시스템 연동 방식
- 성능/보안 제약
- 관련 테스트 구조
- 관련 의존성과 설정

### Tech Design Doc

기존 코드베이스의 패턴/컨벤션을 따른다. 새 기술/패턴 도입 시 대안 검토에서 비교.

각 항목에 `[근거: ...]` 태그로 출처를 명시한다.

```markdown
### 배경
(이 Feature를 구현하는 기술적 맥락. 기존 코드의 관련 구조 간단히 언급)
### 목표
- 기술적 목표 [근거: Rule "..."]
### 비목표
- 의도적 제외 항목 [근거: wbs 경계 "..."]
### 설계
- API, 엔티티, 검증 규칙, UI 구조 등 [근거: Rule "..." 처리를 위해]
### 대안 검토
(고려했으나 선택하지 않은 접근 방식과 그 이유)
```

### Vertical Slicing

Gherkin Scenarios를 구현 단위(Slice)로 분할한다.
- 구현 무게 기준 (Rule과 1:1 아님)
- 각 Slice에 Scenario 매핑 (1:N)
- Slice 간 의존 관계로 순서 결정

```markdown
#### Slice 1: (제목)
- **구현 내용:** (요약)
- **Scenarios:** ...

#### Slice 2: (제목)
- **의존:** Slice 1
- **구현 내용:** (요약)
- **Scenarios:** ...
```

## Step 5: Feature 문서 생성

- **위치:** wbs 문서가 있는 디렉토리
- **파일명:** `{Feature번호}-{Feature이름}.md` (영문, 공백/특수문자 제거)

```markdown
# Feature {번호} {이름}

## 참조 자료
- WBS: [{wbs 파일명}]({wbs 문서 상대경로})
{수집한 구체적 정보}

### 설계 결정
| # | 결정사항 | 선택 | 근거 |

## 요구명세
{Gherkin}

## 구현계획
{Tech Design + Slicing}
```

## Step 6: 정보 유실 방지 검증

문서 작성 후, 대화에서 수집한 모든 구체적 정보가 문서에 기록되었는지 검증한다.

- 사용자의 답변에 포함된 구체적 수치, 열거 항목, 업무 규칙, 제약 조건이 누락 없이 Feature 문서에 기록되었는가?
- 언급된 참조 파일/문서 경로가 확인 목적과 함께 기록되었는가?
- 누락 발견 시 문서에 반영한 뒤 저장한다.

## Step 7: 역방향 피드백

wbs 문서에 변경/발견 사항을 반드시 반영한다. 누락/불일치 수정을 생략하지 않는다.
문서 간 정합성을 유지하고 변경 사유를 명확히 작성한다.
역방향 피드백의 목적은, 새로운 세션에서 다른 작업을 수행할때 이 세션의 결정사항을 잊지 않고 이어서 하기 위함이다.
