...
- `$ARGUMENTS` (선택): 기존 기획서 md 파일 경로
  - 기획서가 입력된 경우 **반드시 꼼꼼하게 인지**하고 작업을 진행한다. 
...
- 요구사항과 관련된 기존 코드를 분석하여 현재 구현 상태와 패턴을 파악한다.
...
기획서 갱신 후 사용자에게 다음 알린다:

```
기획서가 완성되었습니다.

**다음 추천 작업**: 새 세션에서 다음 작업을 진행하세요.
- 기획서 재 검토: `/sd-plan {{기획서파일경로}}`
- 기획 적용: `/sd-plan-dev {{기획서파일경로}}`
```

- "재리뷰" 선택 시: 작업 흐름 3번으로 돌아가 전체 과정 재수행
