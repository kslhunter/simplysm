...
모든 Open Question이 해소되면, Feature 문서의 `## 요구명세` 섹션 아래에 `## 구현계획` 섹션을 추가한다.

## Step 5. 역방향 피드백

구현계획 작성 과정(Step 2~4)에서 2단계 Gherkin에 변경이 필요하면 반드시 수정한다:
- 구현 관점에서 누락된 시나리오 발견
- 기존 시나리오의 전제 조건이 기술적으로 불가능
- 시나리오 분할/병합이 필요
- Open Question 해소(Step 4)에서 사용자 선택에 따라 요구명세가 달라짐

같은 Feature 문서의 `## 요구명세` 섹션을 직접 수정하고, 변경 내용을 사용자에게 명확히 알린다. 별도로 sd-spec 스킬을 재실행할 필요 없이, 이 스킬 내에서 직접 수정한다.
