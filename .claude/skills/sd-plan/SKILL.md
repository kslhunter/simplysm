...
### TDD 순서: 검증을 구현보다 먼저 배치

계획서의 각 구현 단계를 작성할 때 **검증 방법을 먼저, 구현을 나중에** 배치하라:
...
**올바른 예:**
> ### 단계 N: 기능 X 구현
> 1. 검증: `X.test.ts` 작성 — 기대 동작 테스트
> 2. 구현: `X.ts` 수정 — 기능 코드 작성
> 3. 확인: 테스트 통과 확인
...
`EnterPlanMode`를 호출하라. 위 **계획서 작성 규칙**을 적용하여 계획서 초안을 작성한다.
...
3. **독립 항목 일괄 질문**: 의존 대상이 없는(또는 의존 대상이 이미 해소된) 모든 항목들을 AskUserQuestion로 **한번에 하나씩** 질문하라. 
   - 각 항목마다 2~5개의 선택지를 제시하되, 사용자가 판단할 수 있도록 정확한 메시지를 먼저 제시하라.
