...
### 시나리오 1: 모노레포 전체 실행 (핵심 통합 시나리오)

- 입력: "/sd-claude-docs"
- 전제조건:
  - CLAUDE.md 없음 (루트, 패키지 모두), README.md 없음
...
- 체크리스트:
  ...
  - [ ] root `README.md`가 생성되었다
  - [ ] `packages/core/CLAUDE.md`가 생성되었다
  - [ ] `packages/core/README.md`가 생성되었다
  - [ ] `packages/web/CLAUDE.md`가 생성되었다 (private이어도 CLAUDE.md는 생성)
  - [ ] `packages/web/README.md`는 생성되지 않았다 (private 패키지)
  - [ ] root README.md의 패키지 테이블에 `packages/web`이 포함되지 않았다

### 시나리오 2: 단일 패키지 프로젝트

- 입력: "/sd-claude-docs"
- 전제조건:
  - CLAUDE.md 없음, README.md 없음
  - 단일 패키지 구조: root `package.json` (`workspaces` 필드 없음, scripts에 `build`, `dev`, `test`, main: `./dist/index.js`)
...
- [ ] internal(export되지 않은) 모듈이 README에 문서화되지 않았다
- [ ] src/ 전체를 무차별 스캔하지 않았다 (exports 기반 추적)
- [ ] private 패키지에 README.md가 생성되지 않았다
