...
### Metacognitive Preamble

Feature 범위를 작성하기 전에, 각 Feature의 세부 사항을 다음 기준으로 분류한다:

| 구분           | 레벨         | 설명                                                     |
|--------------|------------|--------------------------------------------------------|
| **VERIFIED** | —          | 사용자가 직접 말했거나 문서에 명시된 것                                 |
| **INFERRED** | **High**   | **현재 작업 대상(타겟) 코드베이스**에서 동일 패턴을 확인했거나 공식 문서에 근거가 있는 추론 |
|              | **Medium** | 일반적 도메인 관행이나 유사 사례에 기반한 추론                             |
|              | **Low**    | 약한 유추나 제한적 근거에 기반한 추론                                  |
| **ASSUMED**  | —          | 추측에 해당                                                 |

INFERRED Medium/Low와 ASSUMED는 불명확한 것으로 보고, `.claude/rules/sd-option-scoring.md`의 규칙에 따라 **반드시** 사용자에게 질문하여 명확화한다.
...
- **범위**: 해당 Feature의 세부 기능을 **MUST 빠짐없이** 나열한다. (절대 누락되어선 안된다.)
...
불명확한 점은 `.claude/rules/sd-option-scoring.md`의 규칙에 따라 **반드시** 사용자에게 질문한다. **절대** 추측하지 않는다.
