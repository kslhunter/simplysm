...
- 분석 결과는 `/sd-clarify` 스킬을 호출하여 모두 명확화한다.

### What/How 분류 규칙 (CRITICAL)

**What: 다음 항목은 이 스킬에서 반드시 확정한다 — 위임 금지(NEVER):**
...
**How: 다음 항목은 이 단계의 범위가 아니다:**

- 인증 방식, 통신 프로토콜, 프레임워크, 아키텍처 패턴
- DB 종류, API 형태, 컴포넌트 구조, 테스트 분해
- 라이브러리 선택, 의존성 주입, import 패턴

**분류가 모호하면 What으로 분류하여, 이 스킬의 범위내로 본다.**

- "이후에 결정될 것 같다"라는 자체 판단으로 위임 금지(NEVER).

## Step 1: 요구사항 수집

사용자가 제공한 요구사항에 대한 분석을 시작한다. 요구사항과 함께 **프로젝트 개요**(배경, 환경, 전제조건, 기술적 제약)를 파악한다.
...
- 기존 체크박스 상태(`[ ]`/`[x]`), Feature 번호, 사용자 메모, 이미 확정된 제외 사항을 임의 초기화·삭제하지 않는다.
