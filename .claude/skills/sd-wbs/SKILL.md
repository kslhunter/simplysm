...
- **의존성 순서는 Epic 그룹핑보다 우선한다** — 의존 관계가 Epic 경계를 넘으면 Feature가 Epic 간 교차 배치된다
...
3. **검토 결과 출력**: 검토 과정(어떤 쌍을 검토했고, 왜 순환이 있거나 없는지)을 사용자에게 출력한다. 고위험 쌍이 없으면 그 판단 근거도 출력한다
...
다음 단계(`/sd-dev-spec`)가 별도 세션에서 실행되어도 정보가 유실되지 않도록, 대화에서 수집한 구체적 정보를 기록한다 — 업무 규칙, 데이터 형식, 유효성 조건, 기술 제약(연동, 프로토콜), 화면/UI 요구사항, 참조 파일 경로 등. Feature별로 그룹화하지 않고 주제별로 자유롭게 기록하며, 범위 힌트에 담기지 않는 구체적 세부사항만 기록한다.
