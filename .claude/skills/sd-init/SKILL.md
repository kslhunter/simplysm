...
- **커스텀 CLI 또는 프로젝트 내부 스크립트** (예: `tsx packages/.../cli.ts`): Bash에서 `--help`를 실행하여(5초 타임아웃) 하위 명령어와 주요 옵션을 파악한다. 실패 시 원본 스크립트 내용만 기록
...
- 기존 `CLAUDE.md`가 없으면 → 5단계 결과를 바로 저장
- 기존 `CLAUDE.md`가 있으면:
  1. 기존 파일을 읽는다
