...
5. REQ별 spec.md 작성 — 도메인 순서대로
   - **REQ 분할 기준**: 한 화면 / 독립 동작 가능 기능 / 응집된 입력·처리·출력 단위. 분할 모호 시 최대한 분할.
   ...
   - **5단계 종료**: 남은 단계(6) 간략 안내 + 6단계 진행 Y/N 묻기. "다른 세션에서도 진행 가능" 정보 노출.
...
6. Q 다이얼로그 루프 (한 번에 하나씩, REQ 번호 순)
   ...
   - Q 총량 일시적 증가 정상.
   - **REQ 종료**: 한 REQ Q 0개 도달 → **spec.md 전체 재검토** (R 블록 간 정합성 / 누락된 결정거리 / 모호 표현 / 미해결 의존) → 신규 Q 발견 시 다이얼로그 루프 재진입 → 재검토 후에도 Q 0개 → 메타 `상태: specified` 전환 → 남은 REQ 안내 + 다음 REQ 진행 Y/N 묻기. "다른 세션에서도 진행 가능 / specified 된 REQ 는 sd-plan 으로 바로 넘길 수 있음" 노출.
...
- ❌ 마지막 Q 답변 직후 spec.md 전체 재검토 없이 specified 전환
