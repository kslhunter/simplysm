...
4. **범위 힌트는 대표 예시(seed)이며 전체 목록이 아니다.** 참조 자료에 기록된 파일 경로나 코드베이스의 관련 모듈을 탐색하여, 범위 힌트에 나열되지 않은 항목도 파악한다. 발견한 추가 항목은 Metacognitive Preamble에서 [INFERRED]로 분류한다
...
spec 작성 중 다른 문서에 영향을 주는 변경이 발생하면, 반드시 해당 문서도 함께 수정한다. 변경 내용과 사유를 사용자에게 명확히 알린다.

- Feature 범위 조정으로 WBS의 해당 Feature 설명이 달라지면, wbs.md를 수정한다
- 의존성 발견으로 항목을 다른 Feature로 이동하기로 결정하면, wbs.md의 양쪽 Feature를 모두 수정한다. 대상 Feature 문서가 이미 존재하면 그 문서도 수정한다
