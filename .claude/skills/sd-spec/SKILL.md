---
name: sd-spec
description: SI/업무시스템 요구사항을 분석해 spec.md 로 구조화한다. Use when "요구사항 분석", "SI 분석", "업무시스템 spec 작성" 을 요청할 때
---

# sd-spec

요구사항 → spec.md 1개. 사용자와 §별 임의안 합의하며 채움.

## 본질

- **Input**: Requirement Source (회의록·메일·문서·PDF·발화 등 비정형) + 사용자 대화 + (재진입) 기존 spec.md
- **Output**: spec.md (후속 도구 sd-impl·sd-demo 의 입력 계약)
- **과정**: §별 임의안 → "맞나요?" → 확정 → spec.md 갱신 (반복)
- **폴더**: `.specs/{yyMMddHHmmss}_{slug}/` — `yyMMddHHmmss` 는 Bash `date +%y%m%d%H%M%S`, slug 는 한·두 단어 (sd-base-rules "slug" 정의)

## 진입 분기

진입 시 첨부 spec.md 가 있으면 같은 폴더의 `_wip.md` 도 동반 읽기.

| 상황                                       | 진입점                                                            |
| ------------------------------------------ | ----------------------------------------------------------------- |
| spec.md 없음                               | A 단계부터                                                        |
| spec.md 첨부 + `_wip.md` 진행 중 §x.x 있음 | 그 §x.x 부터 (사용자 우선순위 확인)                               |
| spec.md 첨부 + 헤더 `[OPEN: 날짜]` 있음    | 첫 OPEN 섹션 본문 포인터로 자료 재독 → 임의안 → "맞나요?"         |
| spec.md 첨부 + 본문 인라인 `[OPEN]` 만     | 그 OPEN 항목부터 1건씩 채움                                       |

## 진행 순서

권장 순서. 의존성만 strict — 그 외 항목별 보류·점프 자유.

- **A. §2/§3 헤더 분할** — 분할 절차 ([§2 분할](#분할-단위--bpmn-end-to-end-process)) 로 외부 관계자 평면·도메인 관계도·헤더 도출. 3분류(§2 / §3 / 미분류) 채팅 출력. spec.md 미생성 상태.
- **B. spec.md 골격 생성** — [spec.md 골격](#specmd-골격-초기-생성) 박기 + A 결과 반영. §2.x/§3.x 헤더 `[OPEN: 날짜]` 일괄 + 본문 = 재분석 포인터. §9 분석 제외 항목 = A 단계 사용자 명시 제외 결정 반영.
- **C. §1 개요** — §1.1·§1.2·§1.3·§1.4 임의안. §1.4 = ASCII 구성도 + 장치 목록 분리.
- **D. §2.x/§3.x 본문** — 분할 단위 순서대로 1건씩.
- **E. §4/§5 헤더 분할** — 매핑된 §2.x/§3.x 확정 후. §4 화면 목록 표 + §4.x/§5.x 헤더 `[OPEN: 날짜]` 일괄 + 본문 = 관련 §2.x/§3.x 1줄 참조 포인터.
- **F. §4.x/§5.x 본문** — §4.x/§5.x 표준 구조 절별 임의안.

§6/§7/§8/§9/§10 은 작성 중 자연 도출 (별도 단계 X). 새 어휘·엔티티·외부 호출 발견 시 [G3](#g3-다른--영향-발생-시) 적용.

## 작업 사이클

```
임의안 작성 → "맞나요?" 출력 → 사용자 답변
  확정 → spec.md Write (헤더 [확정: 날짜] 마커)
  수정 → 임의안 재작성 → "맞나요?"
  보류 → spec.md Write (헤더 [OPEN: 날짜] + 본문 = 재분석 포인터)
```

### 임의안 단위

sd-base-rules "단위 흡수 금지" 적용 — 단번에 yes/no 가능한 최소 단위. 응답당 선택지 세트 1개. §별 본문 구조에 따라 분할되며, 단위는 §별 작성법 첫머리에 명시.

### 임의안 작성 게이트

매 임의안마다 출력 직전 자문. 하나라도 실패 = 임의안 폐기 후 재작성 또는 사용자 질문.

1. **형식** — 이 § 작성법 규약을 따랐는가? (헤더 구조·표 형식·마커 표기 등)
2. **출처** — 각 항목·필드·동작 1건마다 사용자 발언·자료에서 인용 가능한가? ([G1·G2](#안전-가드-문제-방지))
3. **영향** — 부산물 중 다른 § 반영 정보가 있는가? ([G3·G4·G5·G6](#안전-가드-문제-방지))
4. **단순화** — spec 명시 정의를 임의안에서 자체 단순화·근사화·안전 처리 추가하지 않았는가? ([G7](#g7-명시-정의-임의-단순화-금지))

### 확정 → spec.md Write

- 헤더 마커 `[확정: YYYY-MM-DD]` 부착 (Bash `date +%Y-%m-%d`)
- Write 직전 출력 1줄 (sd-base-rules "진행 근거 인용"):

  `> 진행 근거: 사용자 발언 "<verbatim 인용>" 의 "<신호 부분>"`

### 보류 → spec.md Write

- 헤더 마커 `[OPEN: YYYY-MM-DD]`
- 본문 = 재분석 포인터만 (임의안 X):
  - **자료 위치**: Requirement Source 파일·범위 (예: `회의록.md L12-30`, `첨부A.pdf p.3`)
  - **대화 발췌**: 사용자 보류 즈음 부분 의견·근거 1~2줄 (Requirement Source 발화 X — 그건 자료 위치)
  - **자료 연결 메모**: 한 섹션이 여러 자료에 걸치면 연결 추정 한 줄
  - **다른 § 확정 영향 메모**: OPEN 유지 중 다른 § 확정이 본 OPEN 재분석에 영향 주면 누적 한 줄. 형식: `§X.Y <섹션명> 확정 — <결정 한 줄>` (운용 규칙 = [G5](#g5-open-섹션-영향-푸시))

발생 경로 2가지:
- 사용자 명시 보류 (임의안 제시 후 사용자가 보류 지시)
- 헤더 분할 직후 자동 (B 단계 §2.x/§3.x · E 단계 §4.x/§5.x 헤더 분할 시 본문 미작성 상태로 일괄 OPEN)

재개 시 본문 포인터로 자료 재독 → 임의안 도출 → 확정되면 본문 임의안 교체 + 헤더 `[확정: 날짜]`.

## 안전 가드 (문제 방지)

작업 중 발동되는 가드. §별 작성법 및 게이트에서 인라인 참조.

### G1. As-Is 출처 인용 (추정 차단)

각 §2.x/§3.x/§4 항목/§5 처리 단계 1건마다, 사용자 발언 verbatim 또는 자료 위치로 출처 인용 가능한가 자문.

- 인용 불가 → 후보에서 빼고 별도 질문 항목으로 분리
- As-Is 회의록·매뉴얼·현행 화면은 추정 자료 (sd-base-rules "결정 근거 안티패턴" 참조)

### G2. 패턴 복제 차단

example-spec / 본 spec.md 다른 §4.x 에서 **형식만** 가져오고 **내용물**(항목·컬럼·필드·액션 버튼·동작)은 본 §의 §2.x/§3.x 본문·사용자 발언에서 직접 도출.

- 형식 = ASCII 박스 라인, 영역 추상화 표기, 액션 버튼 텍스트 표기, 항목 표 컬럼 구조, 마스터 화면 표준 버튼바 등
- ❌ 다른 §4.A 에 "취소 요청" 컬럼·`[취소]` 버튼 있다는 이유로 §4.B 에도 복사 (§4.B 요구사항엔 취소 X)
- ✅ §4.B 의 §2.x/§3.x·관련 섹션·사용자 발언에서 "취소" 언급 없음 확인 → 취소 컬럼·버튼 제외

### G3. 다른 § 영향 발생 시

발견 경로 무관 — 본문 작성 중·자료 재독 중·사용자 흘림·외부 조사·OPEN 섹션 재분석 부산물 등 어디서든 다른 §(기존 §x.x 본문 보강·신규 §x.x 추가·§6/§7/§8 공통 섹션 추가) 반영 정보 발견 시:

1. 발견 즉시 같은 턴에 처리 — 휘발·미루기 금지
2. 해당 § 임의안 → "맞나요?" 확정 → 원 작업 재개

- ❌ §2.1 작성 중 "BOA 는 매일 갱신, 라인이 며칠 내 사라짐" 발견 → 메모 없이 §2.1 만 계속
- ✅ §6.3 보강 임의안 즉시 제시 → 확정 → §6.3 반영 → §2.1 재개

### G4. 사용자 흘림 반영

사용자가 흘리듯 말한 가드레일·단서·미반영 의견은 영향 § 임의안 작성 시 반영. 휘발 금지.

- ❌ §1 단계에서 "Location 코드는 영문 대문자만" 흘림 → §6.1 임의안에 미반영
- ✅ §6.1 임의안에 "Location 코드 영문 대문자/숫자/하이픈" 포함

영향 §가 현재 §와 다르면 [G3](#g3-다른--영향-발생-시) 절차.

### G5. OPEN 섹션 영향 푸시

임의안 확정 시 그 결정이 OPEN 상태 다른 § 재분석에 영향 줄 수 있는지 자문.

- 영향 있으면 해당 OPEN 본문 포인터에 `§X.Y <섹션명> 확정 — <결정 한 줄>` 메모 추가
- 같은 작업에 함께. 사용자에게 한 줄 보고
- 별도 "맞나요?" X (확정된 결정의 기록일 뿐, 임의안 X)

### G6. §4/§5 구현 마커 제거

§4.x/§5.x 본문 또는 그 본문이 의존하는 § 본문 갱신 시 `[확정: X, 구현: Y]` 의 `, 구현: Y` 부분만 제거 → `[확정: X]` (이전 구현 무효화 → sd-impl 재실행 대기). 확정 날짜 갱신 여부는 무관.

**의존 식별** — §4.x/§5.x 본문에 다음 중 하나라도 나타나는 §x.x = 그 §4.x/§5.x 의 의존:
- `관련 섹션:` 줄의 `[프로세스.X]`·`[기타.X]`·`[화면.X]`·`[자동 처리.X]`·`[외부인터페이스.X]`
- 본문 어느 위치든 등장하는 `[모델.X…]`·`[공통 정의.X]`
- §6 외부 자료 명세를 컬럼명 일치로 끌어다 쓴 양식 매핑 표 (해당 §6.x)
- 모달 호출 `→ [화면.X]` 의 그 §4.x

**전파 방향**: 참조 대상 → 참조하는 §4.x/§5.x 단방향.

**처리**: [G3](#g3-다른--영향-발생-시) 와 같은 턴. 변경 임의안 확정 시 영향 §4.x/§5.x 를 번호 매겨 각 한 줄 이유(어느 §x.x 의 어떤 변경 때문에 무효화)와 함께 모아 보여주고 "맞나요?".
- 전부 동의 → 일괄 마커 제거
- 일부 유지 지목 (예: "3~5는 유지") → 지목 항목 마커 유지, 나머지만 제거

### G7. 명시 정의 임의 단순화 금지

spec 에 명시된 식·정의·분기·경계를 임의안에서 자체 단순화·근사화·안전 처리(NULL 강제·0 클램프·방어 가드·생략 분기) 추가 금지.

- 식 그대로 풀어쓸 것
- 정확 구현 부담 크면 단순화안을 사용자에 보고 후 합의 (sd-base-rules "명시 정의 자체 단순화·임의 처리 금지" 의 spec 적용형)

### G8. 와이어프레임 형식 일탈 금지

ASCII 그림 공통 규칙은 output-style `sd-tone` "ASCII 그림" 절 참조.

- example-spec.md 의 가장 가까운 화면 패턴 라인을 형식 모범으로 직접 참조 — 시트=§4.1·§4.3 / 폼 모달=§4.2·§4.4 / PDA=§4.5 / 분석 시트=§4.6
- ASCII 박스(┌─┐ │ └┘)·영역 추상화 표기(`<필터>`·`<시트>`·`<폼>` 등)·액션 버튼 텍스트 표기 자체 변형 X

## spec.md 형식

### 섹션 구조

| §   | 섹션             | 분류           |
| --- | ---------------- | -------------- |
| §1  | 개요             | 워크플로       |
| §2  | 업무 프로세스    | 워크플로       |
| §3  | 기타 요구사항    | 워크플로       |
| §4  | 화면             | 워크플로       |
| §5  | 자동 처리        | 워크플로       |
| §6  | 공통 정의        | 공통/자연 도출 |
| §7  | 도메인 모델      | 공통/자연 도출 |
| §8  | 외부 인터페이스  | 공통/자연 도출 |
| §9  | 본문 외 결정사항 | 메타           |
| §10 | 변경 이력        | 메타           |

### 마커

#### 헤더 마커 (섹션 상태)

| 마커                                   | 의미                                                                                                                                                                              |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[확정: YYYY-MM-DD]`                   | 섹션 확정. 본문 인라인 `[OPEN]` 있어도 무방.                                                                                                                                      |
| `[OPEN: YYYY-MM-DD]`                   | 섹션 미확정 (사용자 보류 또는 헤더 분할 직후 자동). 본문 = 재분석 포인터만. 임의안(BPMN/와이어프레임/항목 등) 채우지 X.                                                           |
| `[확정: YYYY-MM-DD, 구현: YYYY-MM-DD]` | §4 화면 / §5 자동 처리 한정. `[확정]` 에 `구현: 날짜` 콤마 결합. sd-impl 시연 검증 통과 시 부착. 본문 변경 시 `, 구현: …` 제거 (구버전 구현 무효화). 운용 = [G6](#g6-45-구현-마커-제거). |

대괄호 쌍 연달은 표기(`[확정: …][구현: …]`) 마크다운이 reference-style 링크로 해석 → 단일 대괄호 안 콤마 결합 형식.

#### 본문 인라인 마커

| 마커     | 의미                                      |
| -------- | ----------------------------------------- |
| `[OPEN]` | 정보 부족·근거 없는 항목. 향후 확정 필요. |

- 임의 채우거나 누락하지 말고 `[OPEN]` 표기
- 근거 부족 판정 = sd-base-rules "결정 근거" 적용. As-Is 만 근거인 항목·답변 범위 흡수 등 안티패턴 자료는 근거 없음으로 `[OPEN]`
- 헤더 `[확정]` + 본문 `[OPEN]` = OPEN 항목 인정 상태로 섹션 확정. 향후 OPEN 만 처리
- 묻기 선택지: `맞나요? [OPEN 상태로 확정 / 섹션 전체 보류 / OPEN 하나씩 해결]`

### 본문 내 참조 (이름 기반)

본문 내 다른 섹션 참조 시 § 번호 대신 `[카테고리.이름]` 형식. 다단 가능. § 번호는 분석 중 자주 바뀌므로 이름 기반이 안정적.

예:
- `[모델.재고]`·`[모델.박스.수량]`·`[모델.박스.품목.코드]`
- `[화면.재고 확인]`·`[프로세스.입고]`·`[기타.과거 재고 조회]`
- `[자동 처리.재고 스냅샷]`·`[외부인터페이스.ERP 입고 통보]`·`[공통 정의.Location 라벨]`

## §1 개요

**임의안 단위**: §1.1·§1.2·§1.3·§1.4 4건. §1.4 = ASCII 구성도 1 + 장치 목록 1 (총 2).

**모범**: example-spec.md §1.1~§1.4.

### §1.1 핵심 목적

- 한 줄 동사형 큰 단위
- A 단계 도메인 관계도 기반 사이클·관계 표현
- §2.x 동사구 항목 나열 X (그건 §1.2 역할)

### §1.2 주요 목표

- 단위: 최종 사용자 업무 흐름 1건 (화면·기능 단위 X)
- 표현: 한 줄 동사구. 최종 사용자 도메인 어휘
- §2 업무 프로세스 헤더와 1:1 매핑 일반적
- §2.x 본문 핵심 항목은 한 줄에 모두 담아 임의 축약·누락 X

### §1.3 최종 사용자/이해관계자

- 실제 담당자 이름 X
- 일반화

### §1.4 환경/장치

구성도와 장치 목록 목적 별개 → 각각 독립 작성.

- **ASCII 구성도**: 현장이 어떻게 생겼는지 시각화 — 이해당사자 간 그림 맞추기
  - 포함: 솔루션 노드(서버·DB·클라이언트) + 노드 간 연결선 + 물리 위치 그룹(IDC·사무실·창고) + 자동 연동 외부 시스템
  - 제외: Actor, 외부 관계자(인적·조직), 수동 채널(메일·종이), 구체 제품명(MySQL)·통신 프로토콜(HTTPS), 장치 사양(OS·해상도)
- **장치 목록**: 솔루션이 대상으로 하는 장치/환경 스펙 — 서버·DB·클라이언트 등. OS·버전·제품명·모델·해상도·하드웨어
  - 명시 스펙에 맞게 개발 → 다른 스펙 환경 동작은 책임 범위 외

## §2 업무 프로세스

**임의안 단위**: §2.x 1개당 위→아래 1건씩:
1. BPMN (사용자에겐 ASCII, 확정 후 mermaid 저장)
2. 흐름 설명 bullet
3. 관련 섹션

**모범**: example-spec.md §2.1.

### 분할 단위 = BPMN end-to-end process

§2.x 1건 = BPMN end-to-end process 1건 (= Value Stream L1). start event ~ end event 풀 사이클. 사이클 내부 handoff(역할 전환·시간 지연·외부 채널·동일인 다른 시점) 흡수 — BPMN swim lane / 노드 분기로 표현. **handoff 는 분리 사유 X**.

- **start event** (BPMN 시작 트리거):
  - Message — 외부 actor 메시지·자료 도착 (구매처 발주·고객 주문·거래처 EDI)
  - Timer — 시간 도래 (세무 신고일·정산 마감일)
  - Signal — 외부 조건 신호
- **end event** (BPMN 종결 outcome):
  - 외부 가치 전달·외부 자료 송신 (출하 완료·세금 제출·EDI 송신)
- **분리 기준 = pivotal event** (Event Storming):
  1. start event 별개 (입고 start ↔ 출고 start)
  2. end event 별개
  3. 도메인 어휘 자체 전환점 (DDD 경계)

### 분할 절차 (A 단계)

A 단계에서 §2/§3 분할 동시 진행. 절차 4단계.

1. **domain event 추출** (Event Storming "chaotic events") — Requirement Source 에서 동사 과거형 사건 모두 평면 추출. 원본 구조(메일·슬라이드·문서) 무관
2. **timeline 정렬** ("Enforce timeline") — 비즈니스 시간 순 정렬. 모호 시점은 인접 후보로 보류
3. **pivotal event 식별** — start/end/도메인 어휘 전환점 마킹. 외부 관계자 평면(구매처·공급처·고객·하청·감독기관)은 이 단계 자연 도출 — 이게 §1 작성용 도메인 관계도 자료
4. **bounded subdomain 그룹화** — 한 pivotal trigger ~ 다음 pivotal outcome = 1 사이클. 사이클 안 handoff 흡수

#### 분할 결과 매핑 (A 단계 출력)

3분류:
- **사이클** (multi-event handoff 포함) → §2.x
- **단일 user goal 완결** → §3.x ([§3 분할 단위](#분할-단위--cockburn-sea-level-use-case) 참조)
- **사이클·user goal 단위 아닌 것** (도메인 룰·계산식·기존 화면 미세 변경·자료 해석 파라미터·기타 규칙) → **미분류**

#### A 단계 채팅 출력 형식

- 3분류 일괄 나열 (§2 / §3 / 미분류)
- 자문·내부 절차 해설·출처 인용 X
- 화면·마스터 흡수 들여쓰기 X (E 단계 시야)
- 자료 부족 분리 항목은 별도 질문으로 함께 제시
- [G1](#g1-as-is-출처-인용-추정-차단) 적용 — 출처 인용 불가 항목은 후보에서 빼고 별도 질문으로 분리

### 헤더 명칭

- 외부 관계자 평면 기반 도메인 업무 동사구
- 시스템·문서·메뉴·양식 명칭 그대로 X
  - ❌ `발주서 변경 요청` (발주서 = 시스템 문서)
  - ✅ `구매처 발주 변경 요청`

### 본문 구조

#### 1. BPMN

- 사용자에겐 ASCII, 확정 후 mermaid 저장
- 노드 = 최종 사용자 액션·시스템 핵심 트랜잭션 (1행 동사구). 줄바꿈 `<br/>` X
- Actor·장치·매체(PDA·PC·종이)는 액션 컨텍스트로 노드에 명시 OK (예: `창고 작업자: PDA 박스 바코드 스캔`). OS·해상도·API·DB 등 시스템 내부 디테일 X
- 외부 채널 송신(메일·파일 다운로드)·외부 응답 수신(다음 사이클 자료 도착)도 노드. 처리 노드 = 사각(`T[...]`), 이벤트 노드 = 둥근(`E([...])`)
- 분기 = 사용자 의사결정 분기만

#### 2. 흐름 설명 bullet

- BPMN 아래·관련 섹션 위에 자유 나열 (별도 헤더 X)
- 노드 동사구만으로 안 잡히는 룰·조건·계산식·외부 약속·자료 출처 인용 등 1줄씩
- §4·§5 본문 작성 시 이 bullet 들을 근거로 매핑 → 누락 방지
- 단, §4 자체 디테일(필드·UI·파일 확장자) / §6·§7·§8 정형 명세는 여기 X — 각 § 본문에선 이름 참조

#### 3. 관련 섹션

`관련 섹션: [카테고리.이름], ...` 한 줄. 본 섹션이 참조/의존하는 다른 섹션을 콤마 나열 (카테고리 제한 없음).

### 안티패턴 (분할)

- ❌ 같은 사이클의 A→B→C 단계별 담당자·산출물 다르다는 이유로 §2.A·§2.B·§2.C 로 쪼개기
- ❌ 메뉴 그룹화·기존 화면 폴더·코드 As-Is 구조로 묶기/쪼개기

## §3 기타 요구사항

**임의안 단위**: §3.x 1개당 본문 짧으면 전체 1건. 길어지면 요구 의도 → 관련 섹션 분리.

**모범**: example-spec.md §3.1.

### 분할 단위 = Cockburn sea level use case

- "primary actor go away happy after having done this in a single sitting?"
- 행위자가 1회 작업으로 끝내고 만족하고 떠나는 단일 user goal
- 사이클·handoff·지연 없음
- 정형 분해(화면/자동 처리) 동반하는 사용자 직접 요구

### 본문 구조

- 요구 의도 (한 줄~한 문단)
- `관련 섹션: [카테고리.이름], ...` 한 줄
- 구현 디테일은 §4 화면 / §5 자동 처리 본문에서

## §4 화면

**임의안 단위**: §4.x 1개당 §4.x 표준 구조 절 단위로 위→아래, 각 확정 후 다음:
1. 헤더 인덱스 (Actor·관련 섹션) — 1건 (본문 짧으면 기능 개요와 묶기 가능)
2. 기능 개요 — 1건
3. 와이어프레임 — 1건
4. 항목 — 영역(좌/우 등) → 하위 요소(필터·시트 컬럼·입력/상세 폼) 단위로 쪼개기. 각 영역의 각 하위 요소별 1건
5. 동작 — 1건
6. 도메인 규칙·로직 (해당 시) — 규칙별 1건
7. 양식 매핑 (해당 시) — 양식별 1건

신규 작성·정정·골격 신설 무관 동일 적용.

E 단계 헤더 분할 시 §4.x 헤더 `[OPEN: 날짜]` 일괄 + 본문 = 관련 §2.x/§3.x 1줄 참조 포인터. 임의안 본문(와이어프레임·항목·동작) X.

**모범** (와이어 패턴별, [G8](#g8-와이어프레임-형식-일탈-금지) 참조):
- 시트 화면: example-spec.md §4.1·§4.3
- 폼 모달: §4.2·§4.4
- PDA: §4.5
- 분석 시트: §4.6

### 화면 목록 표 (§4 첫머리)

5컬럼: `§ | 분류 | 화면 | 유형 | 장치`

- 분류: 도메인 묶음/메뉴 그룹 자유 명칭 (예: `기준정보`·`입고`·`재고`). LLM 임의안 → 확정
- 유형: 마스터 / 트랜잭션 / 조회
- 장치: PC / PDA / 기타
- 화면 추가/제거 시 표도 갱신. 같은 분류끼리 인접 배치

### 화면 정의 (§4.x 표준 구조)

#### 헤더 인덱스 (한 줄씩)

- `Actor: <역할>`
- `관련 섹션: [카테고리.이름], ...` — 모달 호출·시트 영역 재활용 등 화면 간 의존 포함

화면 헤더에 (장치) 표기 유지: `### N.N 화면명 (PC) [확정: 날짜]` 또는 `(PDA)`.

#### 기능 개요

bullet 로 화면 기능 설명.

#### 와이어프레임 (텍스트)

ASCII 그림 공통 규칙은 output-style `sd-tone` "ASCII 그림" 절 참조. 형식 일탈 가드는 [G8](#g8-와이어프레임-형식-일탈-금지).

**원칙**: 영역 배치·구획만. 디테일(필드·시트 컬럼·실제 값·아이콘 placeholder·fixed 경계·비활성 행 등 시각 표현)은 와이어에 X — 항목 표·동작 절·도메인 규칙 절이 단일 출처.

- 데이터 영역: 영역 타입 + 짧은 라벨로 추상화 — `<필터>`·`<폼>`·`<시트>`·`<목록>`·`<PO 목록>`·`<라인 시트 - 작성영역>` 등. 영역 종류가 한눈에 안 잡히면 라벨 보강
- 액션 버튼: 텍스트 유지 (`[조회]`·`[저장]`·`[등록]`) — 버튼 위치가 와이어의 정보. 아이콘만 있는 버튼도 텍스트로
- 영역(좌/우 분할·탭·상단 command 바·시트 상단 버튼바)으로 나뉘면 박스 라인으로 구획
- 상태(작성중/확정 등)에 따라 영역 구획 크게 달라지면 상태별 분리. 제목: `와이어프레임 (<상태명>):` (예: `와이어프레임 (확정 전):`)

#### 항목

데이터 항목만 (필터·시트 컬럼·입력/상세 폼). 액션 버튼은 "동작" 에.

- **표 제목 = 와이어 영역 라벨과 동일 명칭** 굵게 — 와이어 `<필터>` → `**필터**`, `<품목 목록 시트>` → `**품목 목록 시트**`, `<스캔 입력 폼>` → `**스캔 입력 폼**`
- 한 영역에 종류 다른 표 여러 개 시 종류 부기 — `**<영역 라벨> — <필터|시트 컬럼|입력 폼>**` (예: `**헤더 목록 — 필터**`, `**BOA 시트 — 시트 컬럼**`)
- 시트 화면 기본 구성: 상단 command 바의 `<필터>` + 시트 영역 (시트 컬럼) 2 그룹

**표 형식** — 도메인 매핑은 별도 컬럼 (필터는 시트 컬럼과 중복이라 도메인 매핑 X):
- 입력 폼: `항목 | 종류 | 필수 | 도메인 매핑 | 비고`
- 시트 컬럼: `컬럼 | 종류 | 필수 | 도메인 매핑 | 비고`
- 필터: `항목 | 종류 | 필수 | 비고`

- 필수 컬럼: 무의미한 경우(조회 시트 컬럼 등) `-`
- 도메인 매핑: `[모델.X.Y]` (양식 매핑과 일관). 매핑 없는 항목(선택 체크박스 등) `-`
- 비고: 표시 내용 (이름과 도메인으로 자명하지 않을 때 어떤 값이 표시되는지) / 편집 방법 (편집 가능 항목 — 예: 클릭 시 모달, 인라인 편집) / 기타 자유 설명

**시트 컬럼 그룹핑** (해당 시): 컬럼이 도메인적으로 묶일 때 표 안에 그룹 표시.
- 그룹 헤더 행 1줄 (`**그룹명**`, 다른 셀 빈 칸)
- 자식 컬럼은 `└ ` 들여쓰기 prefix
- 표 컬럼 수 유지

#### 동작

액션 버튼·이벤트별 처리.

#### 시각 규칙 (해당 시)

행 단위·조건부 시각 표현 (비활성 행 취소선·상태별 강조 색·아이콘). 컬럼 단위 표현은 항목 표 비고에 작성하므로 여기 X.

#### 도메인 규칙·로직 (해당 시)

표준 절(항목/동작/양식 매핑)로 안 담기는 화면 고유 도메인 요구. 자유 명칭 절 필요한 만큼 추가. 누락 없이 적는 게 핵심.

#### 양식 매핑 (해당 시)

외부 양식 파일을 다루는 동작(업로드·다운로드)별로 매핑 표.

- 컬럼: `파일 컬럼` + `도메인 모델 ([모델.X.Y])` + `변환·규칙 (해당 시)`
- 양식별 별도 표 + 방향 명시 (예: `업로드 양식`, `다운로드 양식`)
- §6.x 명세 양식이면 컬럼명 일치, 일회성이면 화면 안에서 직접 명세

### 마스터 화면 표준

- ID 컬럼 = 편집 버튼 (`[E N]`)
- 첫 컬럼 = 선택 체크박스 (`[ ]`)
- 시트 상단 버튼바: `[등록] [선택 삭제] [선택 복구] [엑셀 업로드] [엑셀 다운로드]`
- ID 정렬 역방향

### 상단 command 바

페이지 제목 바로 아래, 시트/폼 영역 위. 페이지 전체 또는 현재 레코드 단위 액션 바.

- 좌측: 필터 입력 + 조회 (list) / 진입 컨텍스트 표시 (detail)
- 우측: 페이지·레코드 단위 액션 (있을 때만)
- 좌/우 사이 ` │ ` 세로 구분선

**우측 버튼 그룹 순서** (좌→우):
1. 기본 CRUD (저장 / 삭제 / 복구)
2. 워크플로 상태 전환 (확정 / 취소 / 승인 / 반려)
3. 유틸리티 (원본 다운로드 / 프린트 / PDF)

list·detail 동일. 새 버튼은 해당 그룹 끝에 추가. 시트 상단 버튼바도 동일 그룹 순서 (마스터 화면 표준은 구체 인스턴스).

### 모달 처리

- 모달 화면 = 별도 §4.x. 부르는 쪽 동작에 "→ [화면.X] 을 모달로 띄움" 명시
- 화면명에 "(모달)" 등 사용 맥락 표기 X — 같은 화면이 다른 곳에서 다르게 호출될 수 있음
- 화면 일부 영역(시트·탭)이 다른 화면에서 모달로 재활용되면 별도 §4.x 분리 X. 대신 그 화면 동작 절 끝에 영역별 모달 호출 시 제약(편집 가능 여부 / 선택 전용 / multiselect) 한 줄씩 명시. 호출하는 쪽 동작은 "→ [화면.X] 의 <영역> 을 모달로 띄움" 표기

## §5 자동 처리

**임의안 단위**: §5.x 1개당 위→아래 1건씩:
1. Flowchart (사용자에겐 ASCII, 확정 후 mermaid 저장)
2. 목적 (본문 짧으면 트리거와 묶기 가능)
3. 트리거
4. 처리
5. 예외 처리
6. 양식 매핑 (해당 시) — 양식별 1건
7. 관련 섹션

E 단계 헤더 분할 시 §5.x 헤더 `[OPEN: 날짜]` 일괄 + 본문 = 관련 §2.x/§3.x 1줄 참조 포인터.

**모범**: example-spec.md §5.1.

### 정의

스케줄러/이벤트 트리거로 도는 시스템 백그라운드 처리. 표준 프로토콜·채널을 통한 외부 자료 수집·적재(메일 폴링 + 첨부 적재, FTP 자료 흡수)도 §5 — 시스템별 협상 없는 표준 인터페이싱은 §8 X.

### 본문 구조

#### Flowchart

- 사용자에겐 ASCII, 확정 후 mermaid 저장
- 노드 = 시스템 처리 단계 (1행 동사구)
- 분기 = 시스템 조건 분기·예외
- 시작 = 트리거 (시간·이벤트). 종결 = 결과
- 특수 케이스(상태 머신 / 호출 시퀀스 등)는 mermaid 다른 type(stateDiagram-v2 / sequenceDiagram) 가능

#### 목적·트리거·처리·예외 처리

- 목적: 본문 짧으면 트리거와 묶기 가능
- 트리거: 스케줄·이벤트
- 처리: 처리 단계
- 예외 처리: 실패 케이스별 위험·대처·재시도 한계

#### 양식 매핑 (해당 시)

외부 양식 파일 다루는 처리(읽기·쓰기)별 매핑 표.

- 컬럼: `파일 컬럼` + `도메인 모델 ([모델.X.Y])` + `변환·규칙 (해당 시)`
- 양식별 별도 표 + 방향 명시 (예: `읽기 양식`, `쓰기 양식`)
- §6.x 명세 양식이면 컬럼명 일치, 일회성이면 처리 본문에서 직접 명세

#### 관련 섹션

`관련 섹션: [카테고리.이름], ...` 한 줄.

## §6 공통 정의

작성 중 자연 도출. 새 도메인 어휘·시스템 규격 발견 시 [G3](#g3-다른--영향-발생-시) 적용.

### §6.1 용어 사전 (고정)

도메인 어휘·약어·시스템 내 의미 정의.

### §6.2~ 기타 공통 규격

시스템 전반 규격 (바코드 형식·라벨·공통 정책 등).

특정 §2/§3 본문 단계에서 자연 흡수되는 규칙(업로드 자료 컬럼 매핑·자료 해석 규칙)은 §6 X → 사용처 본문에 둠.

### 외부 자료 명세

§6 외부 자료 = **시스템 외부에서 정의되어 도메인 모델로 자명하지 않은** 자료 (예: 외부 시스템 송신 파일, 거래처 표준 양식, 정해진 외부 규격). 도메인 모델의 자체 엑셀 업로드/다운로드 양식은 §6 X — §7 도메인 모델 + §4/§5 양식 매핑으로 자명.

자료별 표 필수. 외부 자료의 모든 컬럼/필드를 1행 1정의 (자료 자체 객관 명세).

**자료-레벨 메타** (송신 측·자료 라이프사이클·갱신 주기·식별 키 구조 등 자료 *전체* 적용 객관 명세):
- 근거 있을 때 컬럼 표 위에 자유 작성 (불릿/단락/`라벨: 값` 모두 허용)
- 컬럼 표 안에 욱여넣지 X

**컬럼 표**:
- 외부 자료 여러 개 (`SAP.XML`·`D1.CSV`)면 자료별 별도 표
- 컬럼: 자료 성격에 따라 LLM 임의안 → 확정. 예: `컬럼명/식별자 | 필수/선택 | 설명/규칙`
- 모든 컬럼 행 유지
- "설명/규칙" 류 = **그 컬럼이 무엇인지** (데이터 정의: 형식·값 범위·의미·제약) 만. **용도** (어디에 어떻게 쓰이는지) X — 용도·매핑은 사용처(§4·§5 양식 매핑)에서
- 각 셀 값은 근거 채택 가능 자료 있을 때만. 추정·임의 작성 X
- "원본 따름"·"전체 명세는 X.xlsx 참조" 등 외부 참조로 본문 정의 대체 X
- 원본 자료 LLM 접근 불가/누락/모호 → 임의 처리 X. 사용자에게 자료 또는 명세 정보 요청

## §7 도메인 모델

작성 중 자연 도출. 새 엔티티 발견 시 [G3](#g3-다른--영향-발생-시) 적용.

섹션 구조: `필드:` 표 + `키/제약:` 불릿.

### 필드 표

컬럼: `필드 | 타입 | 필수 | 비고`

- 비고: **그 필드가 무엇인지** (데이터 정의: 형식·값 범위·의미·제약·예시) 만. **용도·출처·외부 양식/외부 시스템과의 매핑** X — §4 양식 매핑 / §5 양식 매핑 / §8 자료 매핑에서

### 키/제약 불릿

- **식별 키**: 모든 엔티티에 `ID` 필드 (숫자, 자동 부여) 명시
- **비즈니스 키** (코드 등): 별도 명시. 수정 가능 (PK 아님)
- **활성/비활성 boolean** = 소프트 삭제 플래그. 마스터에 완전 삭제 없음. UI "삭제/복구" 액션과 매핑 → §6.1 용어 사전에 한 줄 명문화

## §8 외부 인터페이스

**임의안 단위** (한 §8.x 당 2회):
1. 기본 정보 + 자료 매핑 → "맞나요?"
2. 예외 처리 → "맞나요?"

작성 중 자연 도출. 외부 시스템 호출 발견 시 [G3](#g3-다른--영향-발생-시) 적용.

### 정의

§8 = 상대 시스템 고유 인터페이스 약속 (시스템마다 자료 매핑·방식 달라 협상 필요).

- 포함 예: 상대 ERP/거래처 자체 정의 API·EDI 규격, 외부 시스템 webhook 페이로드
- 제외 예: 표준 프로토콜·채널 (메일 SMTP/IMAP/Exchange/Graph, FTP, 표준 OAuth). 시스템별 협상 없음 → §5 자동 처리에서 트리거·처리·양식 매핑으로 흡수

### 디테일 범위

도메인 약속 레벨까지만. 구체 endpoint URL·HTTP method·query 파라미터·인증 흐름 단계(grant_type·scope·token endpoint)·SDK 메소드명 등 구현 디테일 X — 구현이 알아서. (§2 BPMN·§4 와이어·§7 도메인 모델과 동일 원칙)

### 본문 구조 (위→아래)

- **기본 정보** (헤더 없이 불릿 3개):
  - `상대 시스템: <시스템명>`
  - `방향: <방향 표기>`
  - `전송 방식: <방식>`
- **자료 매핑** 헤더 + 표 (상대 시스템 필드 ↔ 본 시스템 출처)
- **예외 처리** 헤더 + 본문 (실패 케이스별 위험·대처·재시도 한계)
- **관련 섹션** 한 줄 — 호출자 등 참조/의존 섹션을 `[카테고리.이름]` 콤마 나열

## §9 본문 외 결정사항

§1~§8 어디에도 자연스럽게 안 들어가는 결정·메모만. §1~§8 적절한 자리에 박을 수 있는 결정은 §9 X (누락 도피처 X).

### 분석 제외 항목

A 단계 도메인 분할 + 이후 본문 작성 중 사용자가 명시 제외 결정한 항목 누적. 본 spec 에서 다루지 않지만 다른 spec / sd-dev / 운영 잔손 등 후속 처리 항목 포함.

형식:
```
- <요구 한 줄>
  - 후속 처리: <다른 spec 신규 / 다른 spec 합류 / sd-dev / 운영 잔손 등>
  - 자료 위치: <Requirement Source 파일·범위 (예: 회의록.md L12-30)>
  - 메모: <대화 발췌 1~2줄, 필요 시>
```

- `후속 처리`·`자료 위치` 필수. `메모` 는 후속 단서가 있을 때만
- 다른 spec / sd-dev 후속 항목은 자료 위치 충실히 — 후속 도구가 재독

## §10 변경 이력

- 본문 수정·추가 사항을 날짜순 누적
- 형식: `- YYYY-MM-DD: <변경 내용>` (간결·명확)

## spec.md 골격 (초기 생성)

B 단계에서 spec.md 에 박는 초기 골격. `{name}` = 사용자와 합의한 시스템명.

```markdown
# {name} 요구 분석서

## 1. 개요

### 1.1 핵심 목적

### 1.2 주요 목표

### 1.3 최종 사용자/이해관계자

### 1.4 환경/장치

## 2. 업무 프로세스

## 3. 기타 요구사항

## 4. 화면

## 5. 자동 처리

## 6. 공통 정의

## 7. 도메인 모델

## 8. 외부 인터페이스

## 9. 본문 외 결정사항

## 10. 변경 이력
```

B 단계에서 위 골격 + A 결과 반영:
- §2.x/§3.x 헤더 일괄 추가 (헤더 마커 `[OPEN: 날짜]` + 본문 = 재분석 포인터)
- §9 분석 제외 항목 = A 단계 사용자 명시 제외 결정 반영

## 외부 참조

| 자료                                                                                                       | 용도                                                                               |
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| [references/example-spec.md](references/example-spec.md)                                                   | spec.md 형식 모범 (WMS 예시 1건). 각 § 작성법의 "모범" 표기에서 참조               |
| [.claude/references/sd-requirement-source-handling.md](../../references/sd-requirement-source-handling.md) | Requirement Source 부정확성 처리 (STT 오타·화자 모호·발화 모호·도메인 용어 다의성) |
| [.claude/rules/sd-base-rules.md](../../rules/sd-base-rules.md)                                             | 결정 근거·임의안 단위·진행 근거 인용·표현 톤 등 상위 룰 (자동 로드, 중복 명시 X)   |

## 운용 메타

- **_wip.md**: sd-spec 은 _wip.md 를 직접 작성·갱신·삭제 X. 진행 중 분석 컨텍스트는 대화에 두고, 사용자가 sd-wip 호출 시 그 시점까지의 컨텍스트가 압축됨. sd-spec 은 진입 시 동반 읽기만.
