...
3. 병합된 지적들을 파일별로 그룹핑. 각 그룹에 대해 `sd-improve-validate` 스킬을 Task(subagent_type:general-purpose)를 병렬 호출 → 검증된 지적 목록 수집
   - 검증된 지적이 없으면 보고 후 종료

4. 검증된 지적 각각에 대해, logic, code, architect, test, docs순+중요도순으로 정렬한다.

5. logic, code, architect 지적에 대해 순차적으로 **하나씩** 선택하여 자세히 설명하고 `AskUserQuestion` 도구로 사용자의 동의를 받는다.
   - test, docs는 모두 동의한것으로 봄.
   - 절대로 한꺼번에 설명하거나 한꺼번에 동의받지 말것!!!
