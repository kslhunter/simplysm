...
sit.md 체크리스트를 위에서 아래로 자동 실행하며 통과 항목을 `[x]`로 갱신한다. 한계 도달 시 즉시 중단하고 다른 세션 처리를 안내한 뒤 사용자 응답을 대기한다.
...
위 분류에 해당하지 않는 단계 헤더의 항목은 처리 방식을 `sd-clarify` 지침에 따라 명확화한다.
...
| 데이터 시드 부족 | 시드 SQL/스크립트 실행 |
| 권한·설정 누락 | DB 권한 부여·환경 변수 설정 |

CRITICAL: 현재 세션에서 후속 액션을 자동 호출하지 않는다 (`Skill` tool 직접 실행 금지). 사용자가 다른 세션에서 처리하도록 안내만 한다.
