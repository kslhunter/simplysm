...
2. **CRITICAL: 반드시 `.claude/skills/sd-debug/SKILL.md`를 Read 도구로 읽는다. 이 단계를 건너뛰면 안 된다.**
3. 읽은 sd-debug의 2~5단계를 그대로 수행한다. 단, 문서 기록(6단계)은 수행하지 않는다.
   - 2단계: 컨텍스트 수집
   - 3단계: ACH 가설 경쟁
   - 4단계: 방안 제시
   - 5단계: 사용자 선택
4. 사용자가 선택한 방안에 따라 코드를 수정한다
