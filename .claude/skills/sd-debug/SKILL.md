...
```mermaid
flowchart TD
    S1[1단계: 문제 식별] --> C1{정보 충분?}
    C1 -- 아니오 --> Ask[AskUserQuestion으로 질문]
    Ask --> S1
    C1 -- 예 --> S2[2단계: 컨텍스트 수집]
    S2 --> S3[3단계: 근본 원인 추적]
    S3 --> C2{근본 원인 도달?<br/>최소 3단계 이상}
    C2 -- 아니오 --> S3
    C2 -- 예 --> S4[4단계: 방안 경쟁]
    S4 --> S5[5단계: 문서화]
    S5 --> S6[완료 후 출력]
...
`.claude/rules/sd-problem-solving.md`의 금지 목록을 따른다.
