...
- **남은 가설 1개 & `C(code)` 또는 `C(doc)` 증거가 1개 이상** → 확정 → 4단계 (방안만 경쟁)
- **남은 가설 1개 & `C(infer)` 증거만 존재** → 미확정 → 4단계 (원인+방안 경쟁). 추론만으로는 확정하지 않는다.
...
[결과]
확정: H1 — {근본 원인 설명} (C(code) 1건, C(doc) 1건으로 확정 요건 충족)
또는
미확정: H1, H2 — C(infer)만 존재하여 확정 불가, 4단계에서 사용자 판단 요청
```
