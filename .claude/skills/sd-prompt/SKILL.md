...
### 4-2. Eval workspace 준비

각 Eval 실행마다 격리된 workspace를 생성한다:

```
.tmp/{yyMMddHHmmss}_eval-{스킬명}/       ← {yyMMddHHmmss}는 반드시 Bash `date +%y%m%d%H%M%S`로 얻는다. CWD 기준 상대 경로이므로 .claude/와 구분된다
...
4. workspace의 `.claude/rules/sd-option-scoring.md`를 eval 버전으로 교체한다 — AskUserQuestion은 eval에서 사용 불가하므로, 지시 충돌을 원천 제거한다:
   ```bash
   sed -i \
     -e 's/AskUserQuestion으로 질문한다/선택을 묻는 질문을 텍스트로 출력한 뒤, 합리적인 기본값을 자동 선택하여 선택 결과를 명시한다/g' \
     -e 's/사용자 선택 후 다음으로 넘어간다/자동 선택 후 다음으로 넘어간다/g' \
     "{workspace}/.claude/rules/sd-option-scoring.md"
   ```
...
**FAIL 합리화 금지:** FAIL을 인정하면서 수정 없이 다음 단계로 넘어가는 모든 발화를 금지한다 — "테스트 환경의 한계", "사소한 차이", "의도는 충족", "Judge 편차" 등 어떤 이유든 FAIL 상태에서 진행하는 것은 허용되지 않는다.
...
사용자에게 후보를 보여주고 승인/거부/수정을 요청한다. 승인된 candidate로 eval workspace 내의 프롬프트 복사본을 수정한다. **CRITICAL: 개선 루프에서 수정할 파일은 반드시 eval workspace 경로(`.tmp/{timestamp}_eval-{name}/{시나리오}/`)의 복사본이다. 메인 프로젝트의 원본(`.claude/skills/...`)을 절대 수정하지 않는다. 경로에 `.tmp/`가 포함되어야 한다 — `.claude/`로 시작하는 경로는 원본이므로 절대 수정 금지.** 수정 전 프롬프트를 `_history/v{N}.md`에 백업한다.
...
- **FAIL** → 사용자에게 다음 선택지를 제시한다:
  - **(A) 롤백**: 해당 리팩터링을 되돌린다
  - **(B) 개선 루프**: Step 4-6 방식(원인 판별 → Contrastive 분석 → Meta-Prompting 개선안 → 사용자 승인 → 수정 → Eval 재실행)으로 수정한다
