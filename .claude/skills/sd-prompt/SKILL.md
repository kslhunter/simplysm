...
/tmp/{yyMMddHHmmss}_eval-{스킬명}/       ← {yyMMddHHmmss}는 반드시 Bash `date +%y%m%d%H%M%S`로 얻는다
  _history/              ← 프롬프트 버전 히스토리 (개선 루프에서 Contrastive 분석용)
  {시나리오명}/          ← 시나리오별 작업 디렉토리
...
3. `_history/` 디렉토리를 생성하고, 현재 프롬프트를 `_history/v1.md`로 복사한다 — Contrastive 분석의 기준점이 된다.
...
MSYS_NO_PATHCONV=1 claude -p "{eval 시나리오의 입력}" \
  ...
  --append-system-prompt "이것은 Eval 테스트 환경이다. AskUserQuestion 도구를 사용하지 말고, 해당 질문 내용을 텍스트로 출력 후, 합리적인 기본값을 자동 선택하여 선택한 것을 출력후 계속 진행하라. (절대 질문 자체를 생략하지 말것)" \
  > run-output.json 2>&1)
```
...
사용자에게 후보를 보여주고 승인/거부/수정을 요청한다. 승인된 candidate로 프롬프트를 수정한 뒤, 수정 전 프롬프트를 `_history/v{N}.md`에 백업한다.
