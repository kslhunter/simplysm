...
---
name: {skill-name}
description: {스킬 설명. 트리거 조건을 포함하여 작성.} (...하는 스킬. "...", "..."등을 요청할 때 사용한다. 
---
...
- **간결하게 유지한다** — SKILL.md는 200줄 이하를 목표로 한다. 불가피할 경우 "Progressive Disclosure" 원칙에 따라 파일을 분리한다.
...
Judge 보고서의 개선 제안을:
1. `.claude/rules/sd-option-scoring.md`의 규칙에 따라 사용자에게 질문한다.
2. 승인된 제안을 작성 원칙에 따라 수정한다.
3. 수정 후 workspace를 재구성하여 Eval을 재실행한다.
