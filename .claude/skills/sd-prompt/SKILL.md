...
파악 결과(유형·트리거·입력·출력) 전체를 Step 1 종료 시점에 `/sd-clarify`로 모두 명확화 한다.
명확화 결과는 의도 정의에 반영한다. 재작성이 필요하면 Step 1 본문으로 되돌아간다.

## Step 2: Eval 시나리오 정의

체크리스트·rubric 전체를 Step 2 종료 시점에 `/sd-clarify`로 모두 명확화 한다.
명확화 결과는 Eval 시나리오에 반영한다. 재작성이 필요하면 Step 2 본문으로 되돌아간다.
...
1. Judge의 각 FAIL 항목이 진짜 문제인지, 그리고 제안된 수정 방향이 맞는지 `/sd-clarify`로 재검증한다. 문제 인식은 맞아도 제안이 틀릴 수 있으므로 제안 측면에 무게를 둔다.
...
Smell 탐지 결과 전체를 `/sd-clarify`로 모두 명확화 한 뒤 리팩터링한다.
명확화 결과는 프롬프트에 반영한다. 프롬프트 재작성이 필요하면 Step 3로 되돌아간다.
