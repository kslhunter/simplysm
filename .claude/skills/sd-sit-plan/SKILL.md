...
- SIT는 시나리오 단위 로직 검증이다. 입력값별 케이스를 나열하거나 시드값·결과값을 함께 고정하지 않는다. 시나리오 흐름과 그 안에서 성립해야 할 관계식·규칙으로 표현하여 시드값이 바뀌어도 항목이 유효해야 한다.
  - 금지 예: "B=300·C=500 입력 → A=800 표시", "주문수량 5 입력 시 단가 X / 주문수량 10 입력 시 단가 Y" 같은 입력값별 케이스 나열.
  - 허용 예: "A 표시값이 B와 C의 합산과 일치", "주문수량 변경 시 단가가 단가표 규칙대로 재계산".
  - 적용 대상: 데이터 시드·CRUD 흐름·매트릭스/결과 검증의 입력↔결과 매핑 항목.
  - 예외 흐름(빈 입력·권한 없음·임계값 초과 등)은 입력 케이스 나열이 아니라 독립된 시나리오 항목으로 분리한다.
  - 예외: 입력 자료가 특정 임계값·고정 정책값(예: "할인율 10% 초과 시 결재 필요")을 명시한 경우, 그 값은 그대로 옮긴다.
...
6. **시나리오 검증 위반 검출** — 시드값이 달라지면 깨지는 단정(값 고정), 입력값별 케이스 나열식 항목이 있는지 확인. 위반 예: "B=300, C=500 입력 → A=800 표시", "주문수량 5 입력 시 단가 X / 10 입력 시 단가 Y". 권장: "A 표시값이 B와 C의 합산과 일치" 같은 시나리오 흐름 + 관계식 표현. 예외 흐름은 독립 시나리오 항목으로 분리되어 있는지도 같이 본다. 입력 자료가 특정 정책값을 명시한 경우는 예외.
...
sit.md 본문 구조(단계 헤더)에 따라 환경 정보를 사용자에게 사전 요청한 뒤, sit.md "참조 자료" 섹션 다음에 "환경 정보" 섹션을 작성한다.

### 수집 항목

sit.md에 등장하는 단계 헤더에 따라 필요한 항목만 묻는다.
...
- 접속 주소: {수집한 값}
- DB 연결: {수집한 값 — 해당 단계 존재 시}
- 시드 처리: {sd-sit 자체 시도 | {담당자} 위임} — 해당 단계 존재 시
````
