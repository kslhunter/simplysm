...
export class ProductService {
  async getProductById(prodId: number) {
    const product = await db.query("SELECT * FROM products WHERE id = $1", [prodId]);
    return product;
  }

  async findProductsByCategory(category: string, limit: number) {
  ...
  async updateProduct(name: string, price: number, productId: number) {
    await db.query("UPDATE products SET name = $1, price = $2 WHERE id = $3", [name, price, productId]);
  }
...
- 입력: "/sd-review src"
- 체크리스트:
  - [ ] 네이밍 불일치를 식별했다 — ID 파라미터명이 `userId`, `id`, `prodId`, `productId`로 제각각이다
  - [ ] 메서드 네이밍 패턴 불일치를 식별했다 — 삭제가 `deleteUser` vs `removeProduct`, 단건 조회가 `getUserById` vs `getProductById`(일관) vs `findByEmail`(불일치)
  - [ ] 반환 구조 불일치를 식별했다 — `getUserById`는 필드 선택 객체, `getProductById`는 raw 결과, `findProductsByCategory`는 `{ items, total }` 래핑
  - [ ] 파라미터 순서 불일치를 식별했다 — `updateUser(id, name, email)` vs `updateProduct(name, price, productId)` (id 위치가 다름)
  - [ ] 로직 버그가 아닌 일관성/통일성 관점(CONSIST 카테고리)의 이슈로 분류했다
