...
- **한 번에 하나씩만** 처리한다.
- 질문 전에 **전체 질문 목록을 먼저 파악**하고, 진행 상황을 표시한다.
- **텍스트로 상세 내용을 제시** 후 **사용자의 선택을 요청**한다.
   ```
...
모든 질문이 완료되면 리뷰 결과를 md 파일로 저장한다.

- 파일 위치: `docs/reviews/{{yyyyMMdd}}_{{리뷰대상-kebab-case}}_{{seq}}.md`
  - seq: 동일 파일이 존재하면, 순번지정 
  - 예: "solid 패키지 리뷰" → `docs/plans/260203_solid_1.md`
- 리뷰 결과에는 다음 섹션을 포함한다:
...
- "재리뷰" 선택 시: 작업 흐름 3번으로 돌아가 전체 과정 재수행 (리뷰 보고서 갱신)
- "수정 실행" 선택 시: 새 세션에서 진행하라는 안내 출력 (`/sd-plan @{{review}}.md` 프롬프트 예시 출력)
- "완료" 선택 시: 스킬 종료
