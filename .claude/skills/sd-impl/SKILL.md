...
4. 각 R마다:
   ...
   - 자동 진행 (사용자 개입 X)
   - **R 끝마다 전체 검증** (타입체크 / 린트(+fix) / 테스트)
   - 통과 → impl.md 갱신 (템플릿: [references/impl-md-template.md](references/impl-md-template.md)) → 다음 R
   - 실패 → 막힘 처리 ([references/modes-and-failure.md](references/modes-and-failure.md))

5. 모든 R `완료` → 체크포인트 (메타 `상태: implemented`, `최근 체크포인트` 기록)

## 검증 정책

매 R 끝마다: **타입체크 / 린트(+fix) / 테스트** (해당 R + 전체 회귀 검증)
- 모두 통과 → 다음 R
- 하나라도 실패 → 막힘 처리

(빌드는 별도 항목 아님. 타입체크 + 린트로 갈음.)
