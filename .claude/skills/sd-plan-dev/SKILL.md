...
> [!IMPORTANT]
> **절대 중간에 멈추지 마라.** 모든 작업의 TDD 사이클(RED → GREEN → Refactor → Commit)과 자가검토(4단계), 완료 안내(5단계)까지 반드시 끝까지 완수한다. 한 Phase가 끝나면 즉시 다음 Phase로, 한 작업이 끝나면 즉시 다음 작업으로 진행한다. 중간에 멈추거나 완료된 척 하는 것은 허용되지 않는다.
...
- 실패 확인 후 멈추지 말고 즉시 GREEN Phase로 진행한다
...
- 테스트 생략인 경우: 구현만 수행
- 테스트 통과 확인 후 멈추지 말고 즉시 Refactor Phase로 진행한다
...
- 커밋 완료 후 멈추지 말고 즉시 다음 작업 또는 4단계(자가검토)로 진행한다
