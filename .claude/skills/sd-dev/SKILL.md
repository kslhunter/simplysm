...
모든 Phase 완료 후, 실행 결과를 대화에 출력한다. 중간 Phase부터 재개한 경우, 실행하지 않은 Phase의 섹션은 생략한다.

### Feature

Feature 문서 경로와 한 줄 설명.

### 구현

완료된 Slice를 테이블(Slice, 설명)로 출력한다. 중간 재개 시, 이번 세션에서 완료한 Slice와 이전에 완료된 Slice를 구분하여 표시한다.

### check

대상 패키지별 typecheck · lint · test 결과를 테이블(패키지, typecheck, lint, test)로 출력한다. 해당 항목이 없으면 `—`으로 표시한다. review 수정 후 sd-check를 재실행한 경우, 최종(재실행) 결과를 출력한다.

### review

발견된 **모든** 이슈를 테이블(id, severity, location, title, 수정여부, 내용)로 출력한다. 수정여부 컬럼은 `수정` 또는 `미수정`이고, 내용 컬럼에는 수정한 경우 수정 내용을, 미수정인 경우 미수정 사유를 기재한다. 이슈가 0건이면 "review 이슈: 0건"으로 명시한다.