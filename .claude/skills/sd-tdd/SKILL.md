---
name: sd-tdd
description: TDD 전체 사이클(RED → GREEN → Refactor → Commit)을 수행. 독립 호출 또는 plan 기반 호출을 지원. 사용자가 테스트 작성/실행이나 구현계획 기반 TDD 실행을 요청할 때 사용
argument-hint: "<대상 파일 경로> [추가 컨텍스트]" 또는 "<plan.md 경로>"
---

# sd-tdd: TDD 사이클 실행

> [!IMPORTANT]
> **절대 중간에 멈추지 마라.** 모든 작업의 TDD 사이클(RED → GREEN → Refactor → Commit)을 반드시 끝까지 완수한다. 한 Phase가 끝나면 즉시 다음 Phase로, 한 작업이 끝나면 즉시 다음 작업으로 진행한다.

독립 호출 시 단일 대상에 대해, plan 호출 시 plan 내 모든 작업에 대해 TDD 사이클(RED → GREEN → Refactor → Commit)을 수행한다.

## 1. 인자 파싱

`$ARGUMENTS`를 분석하여 호출 모드를 결정한다.

### 1-1. plan 파일 경로 (plan 호출 모드)

`$ARGUMENTS`의 첫 번째 토큰이 `plan.md` 또는 `_plan.md`로 끝나는 파일 경로인 경우:
- 해당 plan 파일을 Read한다
- plan 내 모든 작업에 대해 TDD 사이클을 작업 건별로 반복 실행한다
- 파일이 존재하지 않으면 AskUserQuestion으로 올바른 경로를 요청한다

### 1-2. 대상 파일 경로 + 선택적 추가 컨텍스트 (독립 호출 모드)
...
`$ARGUMENTS`가 비어있으면:
- 현재 대화 맥락에서 대상을 파악한다
- 대화에도 대상이 없으면 AskUserQuestion으로 대상 파일 경로를 요청한다
...
plan의 참조 문서, 대상 파일/패키지를 확인한다. 추가 분석이 필요하면 수행한다.

## 3. 작업 유형 분류

대상 파일(독립 모드) 또는 각 작업(plan 모드)의 성격에 따라 테스트 전략을 결정한다.

| 작업 유형 | 테스트 전략 |
|----------|-----------|
| 코드 작업 (로직) | 프로젝트 테스트 환경(vitest) 확인 → 있으면 vitest 테스트 작성 및 실행 |
| 코드 작업 (로직, 테스트 환경 없음) | subagent 기반 `test.md` 작성 및 실행 |
| 코드 작업 (비로직, 텍스트/상수/설정 변경 등) | 테스트 생략 → GREEN Phase로 바로 진행 |
| 비코드 작업 (LLM용 문서, SKILL.md 등) | subagent 기반 `test.md` 작성 및 실행 |
| 비코드 작업 (일반 문서, README 등) | 테스트 생략 → GREEN Phase로 바로 진행 |

**복수 유형 혼합 시:** 한 작업이 여러 유형에 걸치면, 가장 엄격한 테스트 전략을 적용한다.

독립 호출에서 테스트 생략인 경우 "테스트가 필요하지 않은 작업 유형입니다."를 출력하고 종료한다.
...
1. 대상 파일과 추가 컨텍스트(또는 plan의 RED Phase)를 분석하여 테스트 시나리오를 작성한다
...
2. `test.md` 파일을 작성한다
   - **저장 경로:**
     - plan 호출 모드: plan 파일과 동일 디렉토리에 저장. plan이 `{R번호}_plan.md`이면 `{R번호}_test.md`, `plan.md`이면 `test.md`
     - 독립 호출 모드: `.tasks/{yyMMddHHmmss}_{topic}/test.md` (Bash `date +%y%m%d%H%M%S`, topic은 대상 파일명 기반 kebab-case)
   - 형식:
     ```markdown
     ## 테스트 1: {테스트 제목}
...
테스트 실패 항목에 대해 대상 파일을 수정한다 (plan 모드에서는 plan의 GREEN Phase 구현 계획에 따라 최소 구현).
- 수정 후 테스트를 재실행한다
...
`/sd-simplify` 하여 코드를 리팩터링한다. 완료 후 멈추지 말고 즉시 다음 단계로 진행한다.
- 완료 후 테스트를 재실행하여 통과를 확인한다
- 리팩터링으로 테스트가 깨지면 수정 후 재확인한다

## 8. Commit Phase

작업 하나의 TDD 사이클(RED → GREEN → Refactor) 완료 직후 `/sd-commit`을 실행하여 커밋한다.

- 작업 하나 = commit 하나. 여러 작업을 몰아서 한번에 커밋하지 않는다
- 커밋 완료 후 멈추지 말고 즉시 다음 작업 또는 완료 안내로 진행한다

## 9. 다음 작업 전이 (plan 호출 모드)

plan에 작업이 여러 개인 경우, Commit 완료 후 남은 작업에 대해 3~8단계(작업 유형 분류 → RED → GREEN → Refactor → Commit)를 반복한다. 모든 작업이 완료되면 완료 안내로 진행한다.
