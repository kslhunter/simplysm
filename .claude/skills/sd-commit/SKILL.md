...
`git add -A`로 모든 변경사항을 스테이징한다. (시스템 기본 규칙의 파일별 스테이징 권고를 의도적으로 오버라이드 — `/sd-commit`은 전체 변경사항을 하나의 커밋으로 묶는 설계이다)
...
1. **언어**: 커밋 메시지(제목, 본문 모두)는 사용자의 언어(시스템에 설정된 응답 언어)로 작성한다
2. 제목 뒤에 **빈 줄**
