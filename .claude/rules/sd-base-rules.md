...
- **명시적 확정 마커**: 결정 근거가 표기된 결정 항목, 또는 "확정" 등 명시 단어 마커
...
- 결정사항 여러개:
  - 어떤 결정사항부터 다룰지는 에이전트가 결정사항간의 영향(변경 규모·영향 범위) 의존성을 기준으로 직접 결정
  - 결정사항 하나 잡아 옵션 펼침 → 사용자 답변 (할지/말지/어떻게)
  - "어떤 결정사항부터?" 사용자에게 묻지 않음
  - 결정사항 처리 후 남은 결정사항이 있으면 멈추지 말고 다음 결정사항으로 진행
- 질문 구조: 맥락 + 질문 + 선택지(번호) + 추천
  - 여러 제안이 있을 경우 모든 제안이 선택지에 포함
- 질문당 결정항목 1건:
  - 결정 항목 여러개를 하나의 질문으로 묶지 말것 (여러 결정사항을 "모두확정" 또는 "큰 그림 채택 → 다항목 자동 진행" 식으로 묶지 말 것)
  - 답변 받은 뒤 다음 결정사항 건
...
**예시** (시나리오: 어떤 함수에 캐시 도입. 결정 항목 3건 — 라이브러리·TTL·무효화 전략):
...
- **스코프**: 표기한 의도가 커버하지 않는 결정 항목은 별도 표기·질문 필요
...
- 한국어 원어민 수준으로 자연스럽게 해석하고 응답 할것.
- 한국어는 SOV — 영어(SVO)와 달리 결론이 문장 뒤.
  - 예: `A가 B를 통해 C가 되었다` → `C가 되었다`
  - 예: `A가 C가 되게 하기위해 B를 사용하였다` → `B를 사용했다`
...
- 문서 내용 거론 시, 사용자는 "문서 역할" 만 인지하고 "문서 내부" 의 세부 내용은 모른다는 가정으로 응답.
  - 나쁜 예: `§4 보강`
  - 좋은 예: `화면 작성법(§4) 보강`
