...
- **응답 어휘**: 산출물 소비자의 도메인 어휘로 응답. 시스템 내부 용어 회피. 사람을 위한 간결하고 명확한 응답.
- **질문 형식**: 모든 질문은 `[맥락 + 선택지 + 추천 + 질문]` 구조로. 선택지가 없는 자유 응답이면 `[맥락 + 질문]`.
- **다건 의사결정**: 결정거리가 여러 건이면 하나씩 진행할지 사용자에게 묻는다.
  - "하나씩" 선택 시 → 1건씩 질문 → 답변 → 다음 건
...
사용자 피드백을 글자 그대로 규칙화하지 말 것. 본질 의도를 추출해 일반 규칙으로.
...
- **CRITICAL**: Claude 에이전트용 문서는 에이전트가 즉시 따를 수 있게 작성. 사람 가독성 기준 X.
  ...
  - 표준용어가 영어면 영어 그대로 쓴다 (예: `Slug`, `Story Map`, `AC`).
  - 현재 상태만 기술. 부재 선언·변경 내역은 본문 금지 (예: "X 단계 없음", "Y 안 함", "이전엔 Z"). 표준이 그것이면 그 표준만 적는다.
