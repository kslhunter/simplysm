...
- ❌ 모호 (의미가 여러 갈래로 해석 가능): **Open Question 등록**, 원문 그대로 발췌
...
판정:
- ✅ 컨텍스트로 명확: 추정
- ❌ 모호: **Open Question 등록**, 화자 미상으로 발췌
...
- **Open Question 등록** — "그거"가 무엇인지, "그렇게"가 어떤 방식인지 사용자에게 확인

## 다중 화자 발언 섞임 (회의록)

여러 사람이 같은 주제를 다른 입장에서 말한 경우:
- 양쪽(또는 모든) 발언 모두 발췌 보존
- 사용자에게 어느 의견 채택할지 Open Question

## 비격식 메일 / paste

주어 생략, 맥락 누락:
- 보충 가능한 부분(자명한 주어 등): 발췌 시 `[추정 보충: ...]` 표기로 원문에 덧붙이지 않고 명시
- 모호한 부분: Open Question
...
처리:
- **자동 매핑 금지**
- **Open Question 등록** — "이 단어가 시스템상 어떤 동작을 가리키는지" 사용자에게 확인
