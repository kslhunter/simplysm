...
- **페이지 이동/정렬/필터 제출 후 `_refresh` 내부에서 `selectedItems`를 현재 페이지 항목으로만 유지한다.** `trackByFn`이 반환하는 key를 기준으로 남은 항목만 걸러낸다. `<sd-sheet>`는 items 변경 시 selectedItems를 자동 초기화하지 않으므로, 소비자가 직접 필터링해야 한다.
