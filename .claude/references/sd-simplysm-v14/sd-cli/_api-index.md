...
| `SdConfigFn` | type | [sd-config.md](./config/sd-config.md) | `sd.config.ts`의 default export 함수 타입이 필요할 때 |
| `SdConfigParams` | interface | [sd-config.md](./config/sd-config.md) | `sd.config.ts` 함수 매개변수 타입이 필요할 때 |
| `SdPackageConfig` | type | [sd-package-config.md](./config/sd-package-config.md) | 패키지 target별 설정 discriminated union이 필요할 때 |
...
| `BuildTarget` | type | [build-target.md](./config/build-target.md) | 빌드 플랫폼 값을 참조할 때 |
...
| `SdPublishConfig` | type | [sd-publish-config.md](./config/sd-publish-config.md) | 배포 설정 discriminated union이 필요할 때 |
...
| `SdStoragePublishConfig` | interface | [sd-publish-config.md](./config/sd-publish-config.md) | FTP/SFTP 배포 설정이 필요할 때 |
...
| `NpmConfig` | interface | [npm-config.md](./config/npm-config.md) | `package.json` 타입이 필요할 때 |

## TypeScript Compiler

| API | Kind | 문서 | 언제 쓰나 |
|-----|------|------|-----------|
| `SdTsCompiler` | class | [sd-ts-compiler.md](./ts-compiler/sd-ts-compiler.md) | Angular/TS 패키지를 프로그래매틱하게 AOT 컴파일할 때 |
