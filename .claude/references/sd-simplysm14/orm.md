...
- `.default(...)`: DB가 값을 결정해야 할 때만 (예: `CURRENT_TIMESTAMP`). 의미 없는 기본값(`0`/`false`/`""`)으로 `NOT NULL` 우회 금지.
...
- **프로세스 문서(트랜잭션)**: 물리 delete. 상세 행 포함 캐스케이드. 단, 다른 테이블이 FK로 참조 중이면 삭제 차단하고 사용자에게 사유 안내.
