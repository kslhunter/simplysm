...
- **angular** — Angular 21 zoneless 프론트엔드 UI 라이브러리 — provideSdAngular 부트스트랩, 모달/토스트/busy/인쇄 오버레이, 폼 입력·버튼·셀렉트 등 컨트롤, sd-sheet 그리드, sd-crud-list/detail 화면 골격, 공유 마스터 데이터 선택, 사이드바/탑바 레이아웃, 라우팅·메뉴·권한, core 디렉티브·signal 헬퍼, 칸반/권한표/에디터/시각화. 앱 화면 작성·수정 시 사용. 자세히: [apis/angular/README.md](./apis/angular/README.md)
