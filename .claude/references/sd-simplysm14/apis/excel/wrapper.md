...
동작: 새 워크북에 시트 1개를 만들고, 0행에 표시명 헤더, 1행부터 각 레코드 값을 스키마 키 순서대로 쓴다. 전체 표에 4변 테두리, 필수(non-optional·non-nullable·non-default)이며 boolean 이 아닌 필드의 헤더 셀에 노란 배경(`"00FFFF00"`) 강조, zoom 85%, 0행 틀고정, 표 전체 범위(헤더행~마지막 데이터행)에 헤더 자동 필터(드롭다운)를 적용한다. **반환된 워크북의 close 는 호출자 책임** — 사용 후 반드시 `close()`.
