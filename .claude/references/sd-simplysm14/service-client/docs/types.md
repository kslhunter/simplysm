...
Web Worker 인터페이스. DOM `Worker`를 대체하며 브라우저 `Worker`와 구조적으로 호환된다.
...
| Field | Type | Description |
|-------|------|-------------|
| `onmessage` | `((ev: MessageEvent) => void) \| null` | 메시지 수신 핸들러 |
...
브라우저 Web Worker를 생성한다. Worker가 지원되지 않는 환경(Node.js)에서는 `undefined`를 반환한다.
...
|-----------|------|-------------|
| `url` | `URL` | Worker 스크립트 URL |
| `options` | `{ type: string }` | Worker 옵션 (`type: "module"` 등) |

반환: `WorkerLike | undefined` — Worker 미지원 환경이면 `undefined`
