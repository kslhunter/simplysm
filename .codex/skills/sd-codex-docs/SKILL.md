...
디렉토리가 존재하면 모든 `.md` 파일을 읽어 이미 다루고 있는 주제를 목록화한다. 해당 주제는 **AGENTS.md에서 제외**한다 — 파일 간 규칙 중복은 LLM이 고유 지침 대신 중복 컨텍스트를 처리하게 되어 지침의 효과를 약화시킨다.
...
처리 대상 목록의 각 경로에 대해, 이 스킬 실행 요청을 `spawn_agent` 사용 승인으로 보고 `spawn_agent`로 subagent(effort: `low`)를 병렬 실행한다. 하나의 메시지에서 모든 `spawn_agent` 호출을 동시에 보낸다.
...
Step 1 결과와 Step 3의 각 패키지 AGENTS.md를 조합하여 루트 AGENTS.md를 생성한다. `.codex/rules/`에서 이미 다루는 주제(1-4에서 감지)는 **제외**한다. 패키지 AGENTS.md에 기술된 상세(아키텍처, 패턴 등)는 루트에 중복하지 않고, 아키텍처 섹션에서 패키지 간 의존 관계와 역할 요약만 포함한다.
