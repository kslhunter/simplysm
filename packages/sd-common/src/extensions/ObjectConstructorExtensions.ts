...
declare global {
  ...
  interface ObjectConstructor {
    ...
    merge<T extends { [key: string]: any } | undefined, O extends { [key: string]: any } | undefined>(
      source: T,
      obj: O
    ): T & O;
    ...
    validates(
      source: any,
      defs: (item: any) => { [key: string]: IValidateDef },
      throwNotUse?: boolean
    ): IValidateResult[];

    validates(source: any, defs: { [key: string]: ValidateDef }, throwNotUse?: boolean): IValidateResult[];

    validatesArray<T, K extends keyof T>(
      arr: T[],
      displayName: string,
      def: (item: T) => { [P in K]: IValidateDef }
    ): void;
...
Object.clone = function(source: any, options?: { excludeProps?: string[] }): any {
  if (source instanceof Array) {
    ...
    for (const key of Object.keys(source).filter(
      sourceKey => !options || !options.excludeProps || !options.excludeProps.includes(sourceKey)
    )) {
...
Object.equal = function(source: any, target: any, options?: { excludeProps?: string[] }): boolean {
  if (source instanceof Date) {
    ...
    const sourceKeys = Object.keys(source).filter(
      sourceKey =>
        (!options || !options.excludeProps || !options.excludeProps.includes(sourceKey)) &&
        source[sourceKey] !== undefined
    );
    const targetKeys = Object.keys(target).filter(
      targetKey =>
        (!options || !options.excludeProps || !options.excludeProps.includes(targetKey)) &&
...
Object.validates = function(
  source: any,
  defs: { [propertyKey: string]: ValidateDef } | ((item: any) => { [propertyKey: string]: ValidateDef }),
  throwNotUse?: boolean
): IValidateResult[] {
...
Object.validatesArray = function(
  arr: any[],
  displayName: string,
  def: { [propertyKey: string]: ValidateDef } | ((item: any) => { [propertyKey: string]: ValidateDef })
): void {
  const errorMessages = [];
  for (const item of arr) {
    ...
    if (result.length > 0) {
      ...
      errorMessages.push(
        `- '${displayName}'의 ${arr.indexOf(item) + 1}번째 항목중 '${propertyDisplayNames.join("', '")}'`
      );
    }
  }
