...
declare global {
  ...
  interface Array<T> {
    ...
    groupBy<KK extends Extract<keyof T, string>, K extends { [P in KK]: T[P] }, V>(
      keys: KK[],
      valueSelector: (item: T, index: number) => V
    ): { key: K; values: V[] }[];

    groupBy<
      KK extends Extract<keyof T, string>,
      K extends { [P in KK]: T[P] },
      VK extends Extract<keyof T, string>,
      V extends { [P in VK]: T[P] }
    >(
      keys: KK[],
      valueKeys: VK[]
    ): { key: K; values: V[] }[];
    ...
    groupBy<K, VK extends Extract<keyof T, string>, V extends { [P in VK]: T[P] }>(
      keySelector: (item: T, index: number) => K,
      valueKeys: VK[]
    ): { key: K; values: V[] }[];
    ...
    diffs<K extends Extract<keyof P, string>, P extends T>(
      target: P[],
      options?: { keyProps?: K[]; excludeProps?: K[] }
    ): { source?: T; target?: P }[];

    merge<K extends Extract<keyof T, string>>(
      target: Partial<T>[],
      options?: { keyProps?: K[]; replacement?: boolean }
    ): void;
...
Array.prototype.groupBy = function(
  keySelectorOrKeys: ((item: any, index: number) => any) | string[],
  valueSelectorOrValueKeys?: ((item: any, index: number) => any) | string[]
): { key: any; values: any[] }[] {
...
Array.prototype.orderBy = function(selector?: (item: any) => any, desc?: boolean): any[] {
  return this.concat().sort((p: any, n: any) => {
    ...
    return desc ? (pn < pp ? -1 : pn > pp ? 1 : 0) : pn > pp ? -1 : pn < pp ? 1 : 0;
  });
};
...
Array.prototype.remove = function(arg: any): void {
  const items = typeof arg === "function" ? this.filter(arg) : arg instanceof Array ? arg : [arg];
...
Array.prototype.diffs = function(
  target: any[],
  options?: { keyProps?: string[]; excludeProps?: string[] }
  ...
  for (const item of this) {
    ...
    if (!existsTargetItem) {
      ...
      if (
        options &&
        options.keyProps &&
        !Object.equal(item, existsTargetItem, { excludeProps: options.excludeProps })
      ) {
...
Array.prototype.merge = function(target: any[], options?: { keyProps?: string[]; replacement?: boolean }): void {
  if (!options || !options.keyProps) {
    ...
    for (const targetItem of target) {
      const item = this.single(sourceItem =>
        options.keyProps!.every(keyProp => sourceItem[keyProp] === targetItem[keyProp])
      );
      if (item) {
        if (options && options.replacement) {
          for (const key of Object.keys(item)) {
