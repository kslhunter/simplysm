...
아래 코드 블록을 그대로 출력한 후 종료한다. 다른 스킬을 호출하지 않는다.
...
SKILLS
  ...
  문서
    sd-codex-docs       AGENTS.md + usage 문서 생성
...
스킬 카탈로그를 참조하여 사용자의 요청을 가장 적합한 스킬에 라우팅한다. 단일 스킬에 명확히 라우팅되지 않으면 후보 스킬들을 선택지로 제시하여 사용자에게 확인한다. (`.codex/rules/sd-options.md`를 읽고 따른다)

### 3-2. 안내 및 실행

선택된 스킬을 사용자에게 알리는 텍스트 메시지를 출력한 후 `$스킬명` 형식으로 스킬을 호출한다. 안내는 필수이다. 예시: `> **sd-commit** 스킬을 실행합니다.`
