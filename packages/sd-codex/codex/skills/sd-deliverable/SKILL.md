...
- 대상 경로가 없으면 사용자에게 직접 질문하고 응답을 기다린다.
...
분석 대상 파일이 20개 이상이면, 이 스킬 실행 요청을 `spawn_agent` 사용 승인으로 보고 `spawn_agent`로 병렬 분석한다. 각 `spawn_agent` 호출에는 `fork_context: false`, `model: "gpt-5.3-codex-spark"`, `reasoning_effort: "low"`를 명시한다.
