...
> **읽어야 하는 상황**: 클라이언트에서 `DbContext`를 통해 원격 DB 트랜잭션을 실행할 때. 서버 측 직접 DB 접근은 `@simplysm/orm-node` 참조. executor를 수동 제어할 때는 [`OrmClientDbContextExecutor`](./orm-client-db-context-executor.md) 참조.
