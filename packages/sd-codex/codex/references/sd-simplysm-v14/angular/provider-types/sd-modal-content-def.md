...
> **읽어야 하는 상황**: `SdModalProvider.showAsync()`로 열리는 모달 컴포넌트를 구현할 때. 모달 옵션, 선택 모달 인터페이스도 이 문서에 포함.
...
```typescript
interface SelectModalOutputResult<TKey = any> {
  selectedKeys: TKey[];
}
```
...
| `selectedKeys` | `InputSignal<TKey[]>` | 이미 선택된 항목 키 배열 (복원용) |
