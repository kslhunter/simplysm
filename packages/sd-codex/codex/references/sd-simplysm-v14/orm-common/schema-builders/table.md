...
> **읽어야 하는 상황**: 테이블 스키마를 정의할 때 (컬럼, PK, 인덱스, 관계 포함). 뷰 정의는 [`View`](./view.md), Procedure 정의는 [`Procedure`](./procedure.md) 참조.
...
```typescript
export function Table(name: string): TableBuilder<typeof name, never, never>;
```
...
`TableBuilder<typeof name, never, never>` — 빈 TableBuilder 인스턴스
