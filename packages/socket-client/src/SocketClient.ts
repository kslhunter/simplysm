...
import {CodeException, JsonConvert, Logger, NotImplementedException, Type, Uuid} from "@simplism/core";
...
export class SocketClient {
    ...
    async send(cmd: string, args: any[], header?: { [key: string]: any }): Promise<any> {
        ...
        if (!this.connected) {
            ...
            if (!this.connected) {
                throw new CodeException("NotConnected", "소켓이 연결되지 않았습니다.\n인터넷 연결을 확인하세요.");
            }
        }
    ...
    private async _send(cmd: string, args?: any[], header?: { [key: string]: any }): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ...
            this._socket!.once(`response(${requestId})`, (responseJson: string) => {
                ...
                this._logger.log("응답받음", JsonConvert.stringify(response, {
                    space: 2,
                    hideBuffer: true
                }));
            ...
            this._logger.log("요청", JsonConvert.stringify(request, {
                space: 2,
                hideBuffer: true
            }));
