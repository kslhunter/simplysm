...
import { exec, execSync } from "../../src/utils/cp";
...
describe("exec", () => {
  ...
  it("stderr를 캡처하여 반환", async () => {
    const result = await exec("node", ["-e", "console.error('err')"], { reject: false });
    expect(result.stderr.trim()).toBe("err");
  });

  it("reject: false 시 실패해도 결과 반환", async () => {
    const result = await exec("node", ["-e", "process.exit(1)"], { reject: false });
  ...
  it("shell 옵션으로 셸 명령 실행", async () => {
    const result = await exec("echo hello", [], { shell: true });
    expect(result.stdout.trim()).toBe("hello");
  });

  it("kill()로 프로세스 종료", async () => {
    const proc = exec("node", ["-e", "setTimeout(() => {}, 60000)"], { reject: false });
    proc.kill();
