...
|-----------|------|-------------|
| `filePath` | string | 워커 파일 경로 (file:// URL 또는 절대 경로). 개발 환경에서는 .ts를 권장하며, 내부적으로 tsx를 통해 실행된다. |
| `opt` | WorkerRawOptions (optional) | Worker thread 옵션. stdout/stderr는 자동으로 메인 프로세스로 파이프된다. |
