...
export class IndexedStore<T extends object> {
  get def() {
  ...
  async bulkAdds(...items: T[]) {
    ...
    /*return await new Promise<void>((resolve, reject) => {
      ...
      for (const item of items) {
        const req = this._db.indexedTrans!.objectStore(this.def.name).add(item);
        req.onerror = () => {
          reject(req.error);
        };
  ...
  async bulkPuts(...items: T[]) {
    ...
    return await new Promise<void>((resolve, reject) => {
      let lastReq: IDBRequest | undefined;
      for (const item of items) {
        const req = this._db.indexedTrans!.objectStore(this.def.name).put(item);
        req.onerror = () => {
          reject(req.error);
          return;
        };
