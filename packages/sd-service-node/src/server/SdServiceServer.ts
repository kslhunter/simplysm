    ...
    const runners = this.middlewares.concat([
      async (req, res, next): Promise<void> => {
        ...
        try {
          ...
          if ((await FsUtils.lstatAsync(localPath)).isDirectory()) {
          ...
          const fileStream = FsUtils.createReadStream(filePath);
          const indexFileSize = (await FsUtils.lstatAsync(filePath)).size;
