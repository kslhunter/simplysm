...
export interface TypecheckOptions {
  /** 타입체크할 경로 필터 (예: `packages/core-common`). 빈 배열이면 전체 대상 */
  targets: string[];
  /** debug 로그 출력 */
  debug: boolean;
}
 ...
 * - `sd.config.ts`를 로드하여 패키지별 타겟 정보 확인
...
export async function runTypecheck(options: TypecheckOptions): Promise<void> {
  ...
  try {
    const sdConfigModule: unknown = await import(pathToFileURL(sdConfigPath).href);
    if (
      sdConfigModule == null ||
      typeof sdConfigModule !== "object" ||
      !("default" in sdConfigModule)
    ) {
      throw new Error("sd.config.ts에 default export가 없습니다.");
    }
    const configFn = sdConfigModule.default;
    if (typeof configFn !== "function") {
      throw new Error("sd.config.ts의 default export는 함수여야 합니다.");
    }
