      ...
      <div class="flex-1 overflow-auto p-6">
        <div class="space-y-8">
          ...
          <section>
            ...
            <p class="mb-4 text-sm text-slate-600 dark:text-slate-400">외부 클릭, Escape, Tab, 스크롤 시 자동으로 닫힙니다.</p>
            <div class="flex items-center gap-4">
              ...
              {basicSelected() != null && <span class="text-sm text-slate-600 dark:text-slate-400">선택: {basicSelected()}</span>}
            </div>
                ...
                <For each={menuItems}>
                  {(item) => (
                    <li
                      role="menuitem"
                      tabIndex={0}
                      class="cursor-pointer px-4 py-2 outline-none hover:bg-slate-100 focus:bg-slate-100 dark:hover:bg-zinc-700 dark:focus:bg-zinc-700"
                      onClick={() => {
          ...
          <section>
            ...
            <p class="mb-4 text-sm text-slate-600 dark:text-slate-400">아래 영역에서 우클릭하세요.</p>
            <div
              class="flex h-32 items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 dark:border-zinc-600 dark:bg-zinc-800"
              onContextMenu={handleContextMenu}
            >
              <span class="text-slate-500 dark:text-slate-400">우클릭 영역</span>
            </div>
            {contextSelected() != null && <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">선택: {contextSelected()}</p>}
          ...
          <section>
            ...
            <div class="flex items-center gap-4">
              ...
              {maxHeightSelected() != null && <span class="text-sm text-slate-600 dark:text-slate-400">선택: {maxHeightSelected()}</span>}
            </div>
            <Dropdown
