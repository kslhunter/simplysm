...
export class SdTypescriptWatcher extends EventEmitter {
  ...
  public async watchAsync(options: {
    ...
    const prevWriteFile = host.writeFile;
    host.writeFile = (fileName, data, writeByteOrderMark, onError, sourceFiles1): void => {
      if (
    ...
    const prevGetSourceFile = host.getSourceFile;
    ...
    // 파일 감지할 목록 구성
    let watchFilePaths = Object.keys(this._fileInfoMapObj)
      .concat(...Object.values(this._fileInfoMapObj).mapMany(v => v.scssDependencies));

    if (options.useDependencyChanges) {
      watchFilePaths = watchFilePaths.concat(...Object.values(this._fileInfoMapObj).mapMany(v => this._getDependencies(v.sourceFile!)));
    }
    watchFilePaths = watchFilePaths.distinct();

    // 소스가 아닌 파일 제외 (옵션에 따라)
    let firstChangedFilePaths = watchFilePaths;
    if (!options.includeExternalFiles) {
      firstChangedFilePaths = firstChangedFilePaths.filter(item => !path.relative(srcPath, item).includes(".."));
    }

    // 최초 변경 이벤트 알림
    const firstChangeInfos = firstChangedFilePaths.map(item => ({
      type: "add" as const,
      filePath: item
    }));
    ...
    if (this._tsConfig.files === undefined) {
      watchFilePaths.insert(0, path.resolve(srcPath, "**", "*.ts"));
      watchFilePaths.remove(item => anymatch(path.resolve(srcPath, "**", "*.ts"), item));
    }
    ...
    const watcher = await FsWatcher.watchAsync(watchFilePaths, async changedInfos => {
      ...
      // 타입스크립트 프로그램 재 정의
      const newParsedTsConfig = this._getParsedTsConfig();
      this._createProgram({
        rootNames: newParsedTsConfig.fileNames,
        options: newParsedTsConfig.options,
        host,
      ...
      if (options.useDependencyChanges) {
        newWatchFilePaths = newWatchFilePaths.concat(...Object.values(this._fileInfoMapObj).mapMany(v => this._getDependencies(v.sourceFile!)));
      }
      newWatchFilePaths = newWatchFilePaths.distinct().filter(item => !watchFilePaths.includes(item));

      if (this._tsConfig.files === undefined) {
        newWatchFilePaths.remove(item => anymatch(path.resolve(srcPath, "**", "*.ts"), item));
      }
      ...
      // 소스가 아닌 파일 제외 (옵션에 따라)
      if (!options.includeExternalFiles) {
        newChangedInfos = newChangedInfos.filter(item => !path.relative(srcPath, item.filePath).includes(".."));
      }

      // 로직 수행
      const results = await cb(newChangedInfos);

      this._logger.debug("변경감지 이벤트 처리 완료");
    ...
    // 로직 수행
    const results = await cb(firstChangeInfos);

    this._logger.debug("변경감지 시작 완료");

    this.emit("complete", results);
  }

  private _createProgram(config: {
    rootNames: string[];
