...
export class SdVirtualWatchFileSystemDecorator extends NodeWatchFileSystem {
  ...
  public constructor(private readonly _virtualInputFileSystem: VirtualFileSystemDecorator,
    ...
    this._generateModule(this._builder.getFilePaths().map(item => ({type: "add", filePath: item})));
  }
  ...
  public watch(files: string[],
    ...
    const newCallback = async (
      ...
      const filePaths = filesModified
        .map(item => item.replace(/\.js$/, ".d.ts"));
      ...
      const allChangeInfos = await this._builder.applyChanges(changeInfos, currChangeInfos => {
        return this._generateModule(currChangeInfos);
      });
  ...
  private _generateModule(changedInfos: ITsFileChangeInfo[]): IFileChangeInfo[] {
    const newChangeInfos: IFileChangeInfo[] = [];

    const isModuleChange = (filePath: string) => {
      return filePath.startsWith(this._controlsDirPath) || filePath.startsWith(this._pagesDirPath) || filePath.startsWith(this._modalsDirPath);
    };

    const ngModules = changedInfos.some(item => isModuleChange(item.filePath))
      ? this._builder.getNgModules()
      : undefined;

    const ngComponentOrDirectives = changedInfos.some(item => isModuleChange(item.filePath))
      ? this._builder.getNgComponentAndDirectives()
      : undefined;
    ...
    for (const changedInfo of changedInfos) {
      try {
        // ...Module.ts 파일 세팅
        if (
          changedInfo.filePath.startsWith(this._controlsDirPath) ||
          changedInfo.filePath.startsWith(this._pagesDirPath) ||
          changedInfo.filePath.startsWith(this._modalsDirPath)
        ) {
          const ngModule = this._generateNgModule(ngModules!, ngComponentOrDirectives!, changedInfo);
          if (ngModule) {
            newChangeInfos.push(ngModule);
          }
        }

        if (changedInfo.filePath.startsWith(this._pagesDirPath)) {
  ...
  private _generateNgModule(ngModules: ITsNgModuleInfo[], ngComponentOrDirectives: ITsNgComponentOrDirectiveInfo[], changedInfo: ITsFileChangeInfo): IFileChangeInfo | undefined {
    ...
    if (changedInfo.type === "unlink") {
      ...
      if (fs.pathExistsSync(outFilePath)) {
        if (fs.readFileSync(outFilePath).toString() !== content) {
  ...
  private _generateRoutingModule(changeInfo: ITsFileChangeInfo): IFileChangeInfo[] {
    const pageFilePath = this._isRoutingModule(changeInfo.filePath)
      ? changeInfo.filePath.replace(/RoutingModule\.ts$/, ".ts").replace(/\\_modules\\/g, "\\")
      : changeInfo.filePath;

    const className = path.basename(pageFilePath, path.extname(pageFilePath));
    const outDirPath = path.resolve(this._modulesDirPath, path.relative(this._builder.rootDirPath, path.dirname(pageFilePath)));
    const outFilePath = path.resolve(outDirPath, className + "RoutingModule.ts");

    // console.log("...RoutingModule.ts", changedInfo.filePath, "outFilePath", outFilePath);
    ...
    if (changeInfo.type === "unlink") {
      ...
      let fileRelativeImportPath = path.relative(outDirPath, pageFilePath);
      ...
      const subPageFilePaths = glob.sync(path.resolve(path.dirname(pageFilePath), pageId, "*"));
      if (subPageFilePaths.length > 0) {
        ...
        const addSubClassContent = (subPageFilePath: string, sub?: boolean) => {
          ...
          let subPageRelativeImportPath = path.relative(path.dirname(pageFilePath), subPageFilePath);
    ...
    const parentRoutingModuleFilePaths: string[] = [];
    let parentDirPath = outDirPath;
    while (true) {
      const basename = path.basename(parentDirPath);
      parentDirPath = path.resolve(parentDirPath, "..");
      if (!parentDirPath.startsWith(this._modulesDirPath)) {
        break;
      }

      const parentRoutingModuleFileName = basename[0].toUpperCase() + basename.slice(1).replace(/-[a-z]/g, match => match.slice(1).toUpperCase()) + "PageRoutingModule.ts";
      const parentRoutingModuleFilePath = path.resolve(parentDirPath, parentRoutingModuleFileName);
      if (fs.pathExistsSync(parentRoutingModuleFilePath)) {
        console.log(outFilePath, parentRoutingModuleFilePath);
        parentRoutingModuleFilePaths.push(parentRoutingModuleFilePath);
      }
    }

    if (parentRoutingModuleFilePaths.length > 0) {
      for (const parentRoutingModuleFilePath of parentRoutingModuleFilePaths) {
        result.push(
          ...this._generateRoutingModule({
            filePath: parentRoutingModuleFilePath,
            type: "dependency"
          })
        );
      }
