  ...
  public findReferenceTargetMetadata(refMetadata: MetadataSymbolicReferenceExpression, currMetadata: SdMetadataBase): ISdMetadataReferenceTarget | ISdMetadataReferenceTarget[] {
    ...
    if (convertedRefMetadata.module.startsWith(".")) {
      const tempTargetModuleMetadata = this.bundleMetadata.modules
        .single((module) =>
          module.filePath === path.resolve(path.dirname(currMetadata.moduleMetadata.filePath), convertedRefMetadata.module! + ".metadata.json")
        );
...
export class SdBundleMetadata {
  ...
  public isForGeneratedFileObj: { [filePath: string]: boolean } = {};
    ...
    for (const module of this.modules) {
      for (const moduleClass of module.classes) {
        ...
        for (const decorator of moduleClass.decorators) {
          ...
          if (!this.isForGeneratedFileObj[module.filePath] && decorator.expression.module === "@angular/core" && decorator.expression.name === "NgModule") {
            ...
            moduleDef = {
              ...
              isForGeneratedFile: this.isForGeneratedFileObj[module.filePath],
              exports: []
            };
          ...
          if (
            !this.isForGeneratedFileObj[module.filePath] &&
            !module.name &&
  ...
  public async registerAsync(metadataFilePath: string, isForGeneratedFile: boolean): Promise<void> {
    ...
    this.isForGeneratedFileObj[metadataFilePath] = isForGeneratedFile;
  }
