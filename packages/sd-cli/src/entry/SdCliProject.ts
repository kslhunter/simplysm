...
export class SdCliProject {
  ...
  static async publishAsync(opt: {
    ...
    try {
      ...
      // yarn scoped 체크 (@simplysm)
      const yarnWhoami = await SdProcess.spawnAsync("yarn", [
        "npm",
        "whoami",
        "--scope",
        "simplysm",
      ...
      logger.debug(`yarn(@simplysm) 로그인 확인: ${yarnWhoami.trim()}`);
    } catch {
      throw new Error(
        "npm 토큰이 유효하지 않거나 만료되었습니다.\n" +
          ...
          "  yarn config set npmScopes.simplysm.npmAuthToken <토큰> --home",
      );
    }
