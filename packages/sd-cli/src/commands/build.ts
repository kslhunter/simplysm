...
export async function runBuild(options: BuildOptions): Promise<void> {
  ...
  for (const { name, config } of buildPackages) {
    ...
    buildTasks.push(async () => {
      logger.debug(`${name} (${config.target}) 시작`);
      ...
      logger.debug(`${name} (${config.target}) 완료`);
    });
  }
