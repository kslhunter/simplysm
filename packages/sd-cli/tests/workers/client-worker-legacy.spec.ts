...
describe("client.worker — legacy", () => {
  describe("startWatch — legacy mode", () => {
    // Acceptance: Scenario "legacyModule 활성화 시 startLegacyWatch 실행"
    it("returns success when legacy watch starts with legacyModule: true", async () => {
      mockLoadSdConfig.mockResolvedValue({
        packages: {
          "my-client": { target: "client", browserSupport: { legacyModule: true } },
        },
      });
    ...
    it("sends buildStart and build events via callbacks", async () => {
      mockLoadSdConfig.mockResolvedValue({
        packages: {
          "my-client": { target: "client", browserSupport: { legacyModule: true } },
        },
      ...
      const mockWatcher = createMockWatcher();
      mockViteBuild.mockImplementation(() => {
        capturedOnBuildStart?.();
        capturedOnBuild?.(buildResult);
        setTimeout(() => mockWatcher.emit("event", { code: "END" }), 0);
        return Promise.resolve(mockWatcher);
      });
