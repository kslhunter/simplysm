...
import {ExcelWorkbook} from "../../../sd-excel/src/ExcelWorkbook";
...
export class SdExcelMappingControl {
  ...
  @Output() public readonly submit = new EventEmitter<{ [key: string]: string | undefined }[]>(); // tslint:disable-line:no-output-named-after-standard-event
          ...
          .then(wb => {
            this._zone.run(() => {
              ...
              this.displayExcelItemList = this.excelItemList.filter((item, index) =>
                index < (this.pagination.page + 1) * 30 &&
                index >= this.pagination.page * 30);
              this.busy = false;
            });
          })
          .catch(e => {
  ...
  public titleTrackByFn(index: number, value: string): string {
    return value;
  }

  public fieldTrackByFn(index: number, value: string): string {
    return value;
  }
  ...
  public async submitExcelFile(): Promise<void> {
    ...
    if (dupKeys.length > 0) {
      this._toast.danger(`매핑이 중복되었습니다.\n- ${dupKeys.join(", ")}`);
      return;
    }
