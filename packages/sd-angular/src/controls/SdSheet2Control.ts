              ...
              <tr>
                ...
                <ng-container *ngFor="let headerCell of headerRow; let c = index; trackBy: trackByFnForHeaderCell">
                  <th *ngIf="headerCell"
                      ...
                      [style.minWidth]="headerCell.isLastDepth ? headerCell.width : undefined"
                      [style.maxWidth]="headerCell.isLastDepth ? headerCell.width : undefined"
...
export class SdSheet2Control<T> implements OnInit, AfterContentChecked, DoCheck {
    ...
    this._ngDoCheckAnimationFrame = requestAnimationFrame(() => {
      this._zone.run(() => {
            ...
            if (prevFocusTdEl) {
              ...
              requestAnimationFrame(() => {
                if (document.activeElement !== prevFocusEl) {
                  const newFocusTdEl = this._elRef.nativeElement.findFirst(`._sheet-container > table > tbody > tr[r='${addr.r}'] > td[r='${addr.r}'][c='${addr.c}']`);
                  newFocusTdEl?.focus();
                }
              });
            }
        ...
        if (!(
          ...
          const selectRowIndicatorContainerEl = sheetContainerEl.findFirst("> ._select-row-indicator-container")!;
  ...
  public onFixedCellResizeOutside(c: number): void {
    ...
    for (const nextFixedColumnIndex of nextFixedColumnIndexes) {
      const thEl = sheetContainerEl.findFirst(`> table > thead > tr > th._last-depth[c='${nextFixedColumnIndex - 1}']`);
      const nextLeft = thEl ? thEl.offsetLeft + thEl.offsetWidth - scrollLeft : 0;
      const nextEls = sheetContainerEl.findAll(`> table > * > tr > *[c='${nextFixedColumnIndex}']`);
  ...
  private _onChildrenFocus(event: FocusEvent): void {
    ...
    const sheetContainerEl = this._elRef.nativeElement.findFirst("._sheet-container")!;
    const focusRowIndicatorEl = sheetContainerEl.findFirst("> ._focus-row-indicator")!;
    const theadEl = sheetContainerEl.findFirst("> table > thead")!;
    const fixedHeaderLastDepthEls = theadEl.findAll("> tr > th._last-depth._fixed")!;
  ...
  private _onChildrenBlur(event: FocusEvent): void {
    if (
      ...
      const focusRowIndicatorEl = this._elRef.nativeElement.findFirst("._focus-row-indicator")!;
  ...
  private _moveCellIfExists(el: HTMLTableCellElement, offsetR: number, offsetC: number, isEditMode: boolean): boolean {
    ...
    const targetEl = this._elRef.nativeElement.findFirst(`._sheet-container > table > tbody > tr > td[r='${targetAddr.r}'][c='${targetAddr.c}']`);
    if (targetEl) {
      targetEl.focus();
      if (isEditMode) {
