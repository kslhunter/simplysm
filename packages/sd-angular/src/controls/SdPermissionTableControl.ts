    ...
    >
      @if ((item.children && item.children.length !== 0) || (item.perms && item.perms.length > 0)) {
        ...
        >
          ...
          @for (_ of arr(getDepthLength() - (depth + 1)); let i = $index; track i) {
            <td class="_after">&nbsp;</td>
          }
