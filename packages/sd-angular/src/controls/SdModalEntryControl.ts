    ...
    this._zone.runOutsideAngular(() => {
      this._dialogEl.addEventListener("resize", event => {
        if (event.prevHeight !== event.newHeight) {
          ...
          if (style.paddingTop) {
            const paddingTopMatch = style.paddingTop.match(/(\d*)/);
            if (!paddingTopMatch || !paddingTopMatch[1]) throw new NotImplementError();
      ...
      this._dialogEl.addEventListener("focus", event => {
