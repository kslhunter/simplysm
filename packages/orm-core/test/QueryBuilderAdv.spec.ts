...
import {QueryBuilderAdv} from "../src/QueryBuilderAdv";
import {Employee} from "./models/Employee";
import {sorm} from "../src/sorm";
...
FROM [GLINT].[dbo].[Employee] AS [TBL]`.trim());
  });
...
SELECT
  ...
  [TBL].[encryptedPassword] AS [encryptedPassword],
  [TBL].[isDisabled] AS [isDisabled]
FROM [GLINT].[dbo].[Employee] AS [TBL]
WHERE ([TBL].[id] = 1)
...
FROM [GLINT].[dbo].[Employee] AS [TBL]
LEFT OUTER JOIN [GLINT].[dbo].[Company] AS [cmp] ON ([cmp].[id] = [TBL].[companyId])`.trim());
  });
...
SELECT
  ...
  [cmp].[name] AS [cmp.name]
FROM [GLINT].[dbo].[Employee] AS [TBL]
LEFT OUTER JOIN [GLINT].[dbo].[Company] AS [cmp] ON ([cmp].[id] = [TBL].[companyId])
...
VALUES (1, '관리자', 'c3018f1e248c3182a5196f20cb2c64507e51f0241c734a21a3bba79d3ba5cd84', 0)`.trim());
  ...
  it("[UPSERT] 기본쿼리", () => {
    ...
    assert.strictEqual(query, `
MERGE [GLINT].[dbo].[Employee] AS [TBL]
