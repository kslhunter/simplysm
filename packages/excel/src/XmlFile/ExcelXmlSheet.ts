...
export class ExcelXmlSheet {
    constructor(public sheet: ExcelWorksheet,
                private _sharedStrings: string[],
                private _styles: ExcelCellStyle[]) {
    }

    toString(): string {
        const rowsStr = this.sheet.cells.map((row, rowIndex) => {
            const cellsStr = row.map((item, colIndex) => {
                const styleIndex = this._styles.map(item => JSON.stringify(item)).indexOf(JSON.stringify(item.style));
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
    ...
    static async parseAsync(sheetName: string, xmlString: string, sharedStrings: string[], styles: ExcelCellStyle[]): Promise<ExcelXmlSheet> {
        return new Promise<any>((resolve, reject) => {
            XML.parseString(xmlString, (err, parsed) => {
                ...
                for (let rowIndex = 0; rowIndex < rowsXml.length; rowIndex++) {
                    ...
                    for (let colIndex = 0; colIndex < colsLength; colIndex++) {
                        ...
                        if (t === "s") {
                            ...
                            sheet.cell(rowIndex, colIndex).value = (v === undefined || v === "") ? undefined : parseInt(v) ? parseInt(v) : v;
                        }
                ...
                if (parsed.worksheet.sheetViews[0].sheetView[0].pane) {
                    const col = parseInt(parsed.worksheet.sheetViews[0].sheetView[0].pane[0].$.xSplit || "0");
                    const row = parseInt(parsed.worksheet.sheetViews[0].sheetView[0].pane[0].$.ySplit || "0");
    ...
    private static _getExcelTimeNumber(date: Date | DateOnly): number {
        ...
        const inputDateNumberUtc = date.addMinutes(-date.getTimezoneOffset()).getTime();
    ...
    private static _getAddress(rowIndex: number, colIndex: number): string {
        ...
        for (let i = 0; i < 26; i++) {
            colAddress = colAddress.replace(new RegExp(i.toString(26), "g"), String.fromCharCode(64 + i));
        }
