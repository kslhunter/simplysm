...
export class ExcelXmlStyles {
    constructor(public styles: ExcelCellStyle[]) {
    }
        ...
        const xfElements = this.styles
            .map(item => {
                ...
                const alignmentElement = (item.textAlign || item.verticalAlign || item.wrapText)
                    ? `<alignment`
                    + (item.wrapText ? ' wrapText="1"' : "")
                    + (item.textAlign ? ' horizontal="' + item.textAlign + '"' : "")
                    + (item.verticalAlign ? ' vertical="' + item.verticalAlign + '"' : "")
                    + "/>"
                    : "";
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
    ...
    static async parseAsync(xmlString: string): Promise<ExcelXmlStyles> {
        return new Promise<any>((resolve, reject) => {
            XML.parseString(xmlString, (err, parsed) => {
                ...
                const styles = parsed.styleSheet.cellXfs[0].xf.map((item: any) => {
                    ...
                    const font = fonts[parseInt(item.$.fontId)];
                    if (font && font.color) {
                    ...
                    const fill = fills[parseInt(item.$.fillId)];
                    ...
                    const border = borders[parseInt(item.$.borderId)];
