...
describe("ExcelUtils", () => {
  describe("stringifyColAddr / parseColAddr", () => {
    ...
    it("handles bijective base-26 boundary (ZA=676, ZZ=701)", () => {
      expect(ExcelUtils.stringifyColAddr(675)).toBe("YZ");
      expect(ExcelUtils.stringifyColAddr(676)).toBe("ZA");
      expect(ExcelUtils.stringifyColAddr(701)).toBe("ZZ");
    });
    ...
    it("round-trip: stringify → parse returns original value for full range", () => {
  ...
  describe("convertNumFmtCodeToName", () => {
    ...
    it("recognizes time-only patterns as Time (mm after h/hh is minutes, not months)", () => {
      expect(ExcelUtils.convertNumFmtCodeToName("h:mm")).toBe("Time");
      expect(ExcelUtils.convertNumFmtCodeToName("h:mm:ss")).toBe("Time");
      expect(ExcelUtils.convertNumFmtCodeToName("hh:mm:ss")).toBe("Time");
      expect(ExcelUtils.convertNumFmtCodeToName("mm:ss")).toBe("Time");
    ...
    it("recognizes date+time pattern as DateTime", () => {
      ...
      expect(ExcelUtils.convertNumFmtCodeToName("yy/mm/dd h:mm")).toBe("DateTime");
      expect(ExcelUtils.convertNumFmtCodeToName("yyyy/mm/dd hh:mm:ss")).toBe("DateTime");
    });
