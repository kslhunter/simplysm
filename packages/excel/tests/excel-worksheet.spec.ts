...
describe("ExcelWorksheet", () => {
  ...
  describe("데이터 테이블 중복 헤더", () => {
    it("should throw error when duplicate headers exist", async () => {
      const wb = new ExcelWorkbook();
      const ws = await wb.addWorksheet("Test");

      await ws.cell(0, 0).setValue("이름");
      await ws.cell(0, 1).setValue("금액");
      await ws.cell(0, 2).setValue("이름"); // duplicate
      await ws.cell(1, 0).setValue("Alice");
      await ws.cell(1, 1).setValue(100);
      await ws.cell(1, 2).setValue("Bob");

      await expect(ws.getDataTable()).rejects.toThrow("이름");
    });
  });
  ...
  describe("셀 병합", () => {
    ...
    it("should expand multi-row merge when inserting within its range", async () => {
      const wb = new ExcelWorkbook();
      const ws = await wb.addWorksheet("Test");
      ...
      // Create vertical merge A1:A3 (rows 0-2, col 0)
      await ws.cell(0, 0).merge(2, 0);

      // Insert copy of row 0 at position 2 (within the merge range)
      await ws.insertCopyRow(0, 2);

      const wsData = await ws["_getWsData"]();
      const merges = wsData.getMergeCells();

      // The merge should expand from A1:A3 to A1:A4
      expect(merges).toHaveLength(1);
      expect(merges[0].s).toEqual({ r: 0, c: 0 });
      expect(merges[0].e).toEqual({ r: 3, c: 0 });
    });

    it("should not modify multi-row merge when inserting below its range", async () => {
      const wb = new ExcelWorkbook();
      const ws = await wb.addWorksheet("Test");
      ...
      await ws.cell(1, 0).setValue("Row1");
      await ws.cell(2, 0).setValue("Row2");
      await ws.cell(4, 0).setValue("Row4");

      // Create vertical merge A1:A3 (rows 0-2)
      await ws.cell(0, 0).merge(2, 0);

      // Insert copy of row 0 at position 5 (well below the merge)
      await ws.insertCopyRow(0, 5);

      const wsData = await ws["_getWsData"]();
      const merges = wsData.getMergeCells();

      // The merge should remain A1:A3 unchanged
      expect(merges).toHaveLength(1);
      expect(merges[0].s).toEqual({ r: 0, c: 0 });
      expect(merges[0].e).toEqual({ r: 2, c: 0 });
    });

    it("should copy single-row merge to inserted row", async () => {
      const wb = new ExcelWorkbook();
      const ws = await wb.addWorksheet("Test");
      ...
      // Create horizontal merge A1:C1 (row 0, cols 0-2)
      await ws.cell(0, 0).merge(0, 2);
      ...
      // Should have original merge at row 0 and copied merge at row 1
      expect(merges).toHaveLength(2);
      expect(merges).toContainEqual({ s: { r: 0, c: 0 }, e: { r: 0, c: 2 } });
      expect(merges).toContainEqual({ s: { r: 1, c: 0 }, e: { r: 1, c: 2 } });
    });
