...
describe("ExcelWrapper", () => {
  ...
  describe("write", () => {
    it("can convert records to Excel", async () => {
      ...
      // Check headers
      expect(await ws.cell(0, 0).getValue()).toBe("이름");
      expect(await ws.cell(0, 1).getValue()).toBe("나이");
      expect(await ws.cell(0, 2).getValue()).toBe("이메일");
      expect(await ws.cell(0, 3).getValue()).toBe("활성");
  ...
  describe("write error cleanup", () => {
    it("should close workbook and re-throw when write encounters an error", async () => {
      const closeSpy = vi.spyOn(ExcelWorkbook.prototype, "close");

      const badSchema = z.object({
        name: z.string().describe("이름"),
        data: z.any().describe("데이터"),
      });

      const wrapper = new ExcelWrapper(badSchema);
      const records = [{ name: "test", data: { nested: "object" } }];

      await expect(wrapper.write("Test", records)).rejects.toThrow("지원하지 않는 타입");
