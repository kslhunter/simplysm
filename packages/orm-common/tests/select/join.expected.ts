...
export const include3Depth: ExpectedSql = {
  ...
  mssql: tsql`
    SELECT
      ...
      [T1.user.posts.user].[createdAt] AS [user.posts.user.createdAt]
    FROM [TestDb].[TestSchema].[Post] AS [T1]
    LEFT OUTER JOIN [TestDb].[TestSchema].[User] AS [T1.user] ON [T1.user].[id] = [T1].[userId]
    LEFT OUTER JOIN [TestDb].[TestSchema].[Post] AS [T1.user.posts] ON [T1.user.posts].[userId] = [T1.user].[id]
    LEFT OUTER JOIN [TestDb].[TestSchema].[User] AS [T1.user.posts.user] ON [T1.user.posts.user].[id] = [T1.user.posts].[userId]
  `,
  postgresql: pgsql`
    SELECT
      "T1"."id" AS "id",
      ...
      "T1.user.posts.user"."createdAt" AS "user.posts.user.createdAt"
    FROM "TestSchema"."Post" AS "T1"
    LEFT OUTER JOIN "TestSchema"."User" AS "T1.user" ON "T1.user"."id" = "T1"."userId"
    LEFT OUTER JOIN "TestSchema"."Post" AS "T1.user.posts" ON "T1.user.posts"."userId" = "T1.user"."id"
    LEFT OUTER JOIN "TestSchema"."User" AS "T1.user.posts.user" ON "T1.user.posts.user"."id" = "T1.user.posts"."userId"
  `,
};
