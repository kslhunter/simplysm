...
export const updateMultiCol: ExpectedSql = {
  mysql: mysql`
    UPDATE \`TestDb\`.\`Employee\` AS \`T1\`
    SET \`T1\`.\`name\` = 'New Name', \`T1\`.\`departmentId\` = 2
    WHERE \`T1\`.\`id\` <=> 1
  `,
  mssql: tsql`
    UPDATE [T1]
    SET [T1].[name] = N'New Name', [T1].[departmentId] = 2
    FROM [TestDb].[TestSchema].[Employee] AS [T1]
    WHERE (([T1].[id] IS NULL AND 1 IS NULL) OR [T1].[id] = 1)
  `,
  postgresql: pgsql`
    UPDATE "TestSchema"."Employee" AS "T1"
    SET "name" = 'New Name', "departmentId" = 2
    WHERE "T1"."id" IS NOT DISTINCT FROM 1
  `,
};
...
export const updateMixed: ExpectedSql = {
  mysql: mysql`
    UPDATE \`TestDb\`.\`Employee\` AS \`T1\`
    SET \`T1\`.\`name\` = 'New Name', \`T1\`.\`managerId\` = \`T1\`.\`managerId\` + 1
    WHERE \`T1\`.\`id\` <=> 1
  `,
  mssql: tsql`
    UPDATE [T1]
    SET [T1].[name] = N'New Name', [T1].[managerId] = [T1].[managerId] + 1
    FROM [TestDb].[TestSchema].[Employee] AS [T1]
    WHERE (([T1].[id] IS NULL AND 1 IS NULL) OR [T1].[id] = 1)
  `,
  postgresql: pgsql`
    UPDATE "TestSchema"."Employee" AS "T1"
    SET "name" = 'New Name', "managerId" = "T1"."managerId" + 1
    WHERE "T1"."id" IS NOT DISTINCT FROM 1
  `,
};
...
export const updateWithTop: ExpectedSql = {
  ...
  `,
  postgresql: pgsql`
    WITH _limited AS (SELECT "T1".ctid FROM "TestSchema"."Employee" AS "T1" WHERE "T1"."departmentId" IS NOT DISTINCT FROM 1 LIMIT 10) UPDATE "TestSchema"."Employee" AS "T1" SET "name" = 'New Name' WHERE "T1".ctid IN (SELECT ctid FROM _limited)
  `,
};
