...
describe("result-parser", () => {
  ...
  describe("type parsing", () => {
    ...
    it("Bytes 변환 - Uint8Array 패스스루", async () => {
  ...
  describe("error processing", () => {
    it("number throw if parsing fails", async () => {
      ...
      await expect(parseQueryResult(raw, meta)).rejects.toThrow("숫자 파싱 실패");
    });

    it("파싱 실패 시 DateTime 오류 발생", async () => {
    ...
    it("파싱 실패 시 Uuid 오류 발생", async () => {
      ...
      await expect(parseQueryResult(raw, meta)).rejects.toThrow("UUID 형식");
    });

    it("파싱 실패 시 Bytes 오류 발생", async () => {
  ...
  describe("1-level JOIN", () => {
    ...
    it("isSingle: true - throws when multiple different results exist", async () => {
      ...
      await expect(parseQueryResult(raw, meta)).rejects.toThrow("여러 개의 다른 결과");
    });
