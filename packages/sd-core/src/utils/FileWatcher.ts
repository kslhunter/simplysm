...
export class FileWatcher {
  public static async watch(
    paths: string | string[],
    sits: ("add" | "change" | "unlink")[],
    callback: (changedFiles: { type: string; filePath: string }[]) => void | Promise<void>,
    millisecond?: number
  ): Promise<void> {
    await new Promise<void>(resolve => {
      const watcher = chokidar
        .watch((typeof paths === "string" ? [paths] : paths).map(item => item.replace(/\\/g, "/")))
        .on("ready", () => {
