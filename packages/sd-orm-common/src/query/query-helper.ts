...
export class QueryHelper {
  ...
  endsWith(
    ...
    return [
      this.getQueryValue(source),
      " LIKE ",
      this.getQueryValue(this.concat("%", target)),/*, "'%'", this._dialect === "sqlite" ? " || " : " + ", this.getQueryValue(target)*/
    ];
  }
  ...
  dateDiff<T extends DateTime | DateOnly | Time>(
    ...
    if (this._dialect === "mysql") {
      ...
      return new QueryUnit<number>(
        Number,
        ["DATEDIFF(", separator, ", ", this.getQueryValue(from), ", ", this.getQueryValue(to), ")"],
      );
    }
  }
  ...
  dateToString<T extends DateTime | DateOnly | Time>(
    ...
    if (this._dialect === "mysql") {
      if (code === 112) {
        return new QueryUnit(
          String,
          ["DATE_FORMAT(", this.getQueryValue(value), ", '%Y%m%d')"],
        ) as any;
      }
      else if (code === 120) {
        return new QueryUnit(
          String,
          ["DATE_FORMAT(", this.getQueryValue(value), ", '%Y-%m-%d %H:%i:%s')"],
        ) as any;
      }
      else if (code === 114) {
        return new QueryUnit(
          String,
          ["DATE_FORMAT(", this.getQueryValue(value), ", '%H:%i:%s')"],
        ) as any;
      }
      else {
      ...
      return new QueryUnit(
        String,
        ["CONVERT(NVARCHAR(25), ", this.getQueryValue(value), ", ", this.getQueryValue(code), ")"],
      ) as any;
    }
  }
  ...
  greatest<T extends undefined | number | Number | DateOnly | DateTime | Time | string | String>(...args: TEntityValue<T>[]): QueryUnit<T> {
    ...
    return new QueryUnit<T>(
      type,
      ["GREATEST(", ...args.mapMany((arg) => [this.getQueryValue(arg), ", "]).slice(0, -1), ")"],
    );
  }
  ...
  cast<T extends TQueryValue>(
    ...
    if (this._dialect === "mysql") {
      return new QueryUnit(
        targetType,
        ["CONVERT(", this.getQueryValue(src), ", ", this.mysqlConvertType(targetType), ")"],
      );
    }
    else {
      return new QueryUnit(
        targetType,
        ["CONVERT(", this.type(targetType), ", ", this.getQueryValue(src), ")"],
      );
    }
  }
  ...
  left(
    ...
    return new QueryUnit<string>(
      String,
      ["LEFT(", this.getQueryValue(src), ", ", this.getQueryValue(num), ")"],
    );
  }
  ...
  getQueryValue(value: TEntityValue<any> | Queryable<any, any>): string | ISelectQueryDef {
    if (value instanceof QueryUnit) {
      ...
      if (this._dialect === "mysql") {
        return "STR_TO_DATE('"
          + value.toFormatString("yyyy-MM-dd HH:mm:ss")
          + "', '%Y-%m-%d %H:%i:%s')";
      }
      else {
  ...
  type(type: Type<TQueryValue> | TSdOrmDataType | string | undefined): string {
    if (typeof type === "string") {
      ...
      switch (currType.type) {
        ...
        case "DECIMAL":
          return "DECIMAL(" + currType.precision + ((currType.digits
            == null
            || currType.digits
            === 0) ? "" : (", "
            + currType.digits)) + ")";
