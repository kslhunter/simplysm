...
export abstract class DbContext {
  ...
  async getTableInfosAsync(database: string, schema?: string): Promise<{
    schema: string;
    name: string
  }[]> {
    return (
  ...
  async getTableColumnInfosAsync(database: string, schema: string, table: string): Promise<{
    name: string;
    dataType: string;
    length?: number;
    precision?: number;
  ...
  async getTableFksAsync(database: string, schema: string, table: string): Promise<{
    name: string;
  ...
  async getTableIndexesAsync(database: string, schema: string, table: string): Promise<{
    name: string;
    columns: {
      name: string;
      orderBy: "ASC" | "DESC";
  ...
  async initializeAsync(
    ...
    // 강제 아닐때
    if (!force) {
      const isDbExists = this.opt.dialect === "sqlite"
        ? true
        : await this.getIsDbExistsAsync(this.opt.database);
        ...
        addFkQueryDefs.push(...[
          {
            ...
            foreignKey: {
              name: fkDef.name,
              fkColumns: fkDef.columnPropertyKeys.map((propKey) => tableDef.columns.single((col) => col.propertyKey
                === propKey)!.name),
      ...
      createIndexQueryDefs.push({
        ...
        index: {
          ...
          columns: indexDef.columns.orderBy((item) => item.order).map((item) => ({
            name: tableDef.columns.single((col) => col.propertyKey
              === item.columnPropertyKey)!.name,
