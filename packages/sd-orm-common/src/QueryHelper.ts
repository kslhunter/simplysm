...
import { DateOnly, DateTime, NotImplementError, Time, type Type, Uuid, type WrappedType } from "@simplysm/sd-core-common";
  ...
  public dateDiff<T extends DateTime | DateOnly | Time>(
    ...
    if (this._dialect === "mysql") {
      return new QueryUnit<number>(
        Number,
        ["TIMESTAMPDIFF(", separator, ", ", this.getQueryValue(from), ", ", this.getQueryValue(to), ")"],
      );
    }
    else {
      return new QueryUnit<number>(
  ...
  public type(type: Type<TQueryValue> | TSdOrmDataType | string | undefined): string {
    if (typeof type === "string") {
      ...
      switch (currType.type) {
        ...
        case "DECIMAL":
          return "DECIMAL(" + currType.precision + ((currType.digits == null || currType.digits === 0) ? "" : (", "
            + currType.digits)) + ")";
        case "STRING":
