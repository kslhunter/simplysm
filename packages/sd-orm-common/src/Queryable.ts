...
export class Queryable<D extends DbContext, T> {
      ...
      for (const orgEntityKey of Object.keys(orgEntity)) {
        ...
        if (SdOrmUtil.canConvertToQueryValue(orgEntityValue)) {
          resultEntity[orgEntityKey] = new QueryUnit(
            SdOrmUtil.getQueryValueType(orgEntityValue),
  ...
  public pivot<V extends TQueryValue, P extends string>(
  ...
  public unpivot<VC extends string, PC extends string, RT extends TQueryValue>(
    valueColumn: VC,
    pivotColumn: PC,
    pivotKeys: string[],
    resultType: Type<RT>,
  ...
  public join<A extends string, J, R>(
  ...
  public joinSingle<A extends string, J, R>(
    joinTypeOrQrs: Type<J> | Queryable<D, J>[],
    as: A,
    fwd: (qr: Queryable<D, J>, en: TEntity<T>) => Queryable<D, R>,
  ...
  public search(
    ...
    // WHERE
    const whereFnName: "having" | "where" = result._def.groupBy && result._def.groupBy.length > 0 ? "having" : "where";
          ...
          for (const text of splitSearchText) {
            if (text.includes("*")) {
              splitSearchTextWhereArr.push(
                this.db.qh.notLike(this.db.qh.toLowerCase(field), text.substring(2).replace(/\*/g, "%").toLowerCase()),
              );
            }
            else {
              splitSearchTextWhereArr.push(
                this.db.qh.notIncludes(this.db.qh.toLowerCase(field), text.substring(2).toLowerCase()),
              );
            }
          }
      ...
      result = result[whereFnName]((item) => {
        ...
        for (const field of fields) {
          ...
          for (const text of splitSearchText) {
            if (text.includes("*")) {
              splitSearchTextWhereArr.push(
                this.db.qh.like(this.db.qh.toLowerCase(field), text.replace(/\*/g, "%").toLowerCase()),
              );
            }
            else {
  ...
  public getInsertIfNotExistsQueryDef(
  ...
  public getUpsertQueryDef<U extends TUpdateObject<T>>(
    updateObj: U,
  ...
  public async insertAsync<OK extends keyof T>(
  ...
  public async insertWithoutFkCheckAsync<OK extends keyof T>(
    records: TInsertObject<T>[],
    outputColumns: OK[],
  ): Promise<{ [K in OK]: T[K] }[]>;
  ...
  public async updateAsync<OK extends keyof T>(
    recordFwd: (entity: TEntity<T>) => TUpdateObject<T> | Promise<TUpdateObject<T>>,
    outputColumns: OK[],
  ): Promise<{ [K in OK]: T[K] }[]>;
  ...
  public async upsertAsync<U extends TUpdateObject<T>, OK extends keyof T>(
    ...
    const insertRecord = (
      insertFwd ? await insertFwd(updateRecord) : ObjectUtil.clone(updateRecord)
    ) as TInsertObject<T>;
  ...
  public upsertPrepare<U extends TUpdateObject<T>>(
    ...
    for (const key of Object.keys(fromEntity)) {
      ...
      if (SdOrmUtil.canConvertToQueryValue(entityValue)) {
        result[key] = new QueryUnit(
          SdOrmUtil.getQueryValueType(entityValue),
