...
export class OrmService extends ServiceBase implements OrmServiceType {
  ...
  async bulkInsert(
    ...
    // ColumnMeta[]를 Record<string, ColumnMeta>로 변환
    // 클라이언트는 records의 키와 동일한 순서로 columnDefs를 전달
    const columnMetas: Record<string, ColumnMeta> = {};
    if (records.length > 0) {
      const keys = Object.keys(records[0]);
      for (let i = 0; i < columnDefs.length && i < keys.length; i++) {
        columnMetas[keys[i]] = columnDefs[i];
      }
    }
