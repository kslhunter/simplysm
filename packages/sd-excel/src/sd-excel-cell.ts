...
export class SdExcelCell {
  ...
  async #deleteAddrAsync(addr: string): Promise<void> {
    const wsData = await this.#getWsDataAsync();
  ...
  async #setStyleAsync(style: ISdExcelStyle): Promise<void> {
    const wsData = await this.#getWsDataAsync();
