...
import {type ISdExcelXml, type ISdExcelXmlWorkbookData} from "../types";
...
export class SdExcelXmlWorkbook implements ISdExcelXml {
  ...
  public getWorksheetNameById(id: number): string | undefined {
    return this.data.workbook.sheets?.[0].sheet.single((item) => NumberUtils.parseInt(item.$["r:id"]) === id)?.$.name;
  }
}
