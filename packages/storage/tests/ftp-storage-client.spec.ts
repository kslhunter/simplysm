...
describe("FtpStorageClient", () => {
  ...
  describe("exists", () => {
    it("파일이 존재하면 true를 반환해야 함 (size로 확인)", async () => {
      await client.connect({ host: "test" });
    ...
    it("디렉토리가 존재하면 true를 반환해야 함 (list로 확인)", async () => {
    ...
    it("루트 디렉토리에서 파일 존재 확인해야 함", async () => {
      mockSize.mockRejectedValueOnce(new Error("Not a file"));
      await client.connect({ host: "test" });
    ...
    it("슬래시 없는 경로에 대해 파일 존재 확인해야 함", async () => {
      mockSize.mockRejectedValueOnce(new Error("Not a file"));
      await client.connect({ host: "test" });
      const result = await client.exists("file.txt");
