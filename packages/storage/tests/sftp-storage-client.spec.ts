...
describe("SftpStorageClient", () => {
  ...
  describe("connect", () => {
    ...
    it("이미 연결된 클라이언트에서 connect 호출 시 오류를 발생시켜야 함", async () => {
  ...
  describe("연결 전 메서드 호출", () => {
    it("연결 전 mkdir 호출 시 오류를 발생시켜야 함", async () => {
      await expect(client.mkdir("/test")).rejects.toThrow("연결되어 있지");
    });

    it("연결 전 rename 호출 시 오류를 발생시켜야 함", async () => {
      await expect(client.rename("/from", "/to")).rejects.toThrow(
        "연결되어 있지",
      );
    });

    it("연결 전 list 호출 시 오류를 발생시켜야 함", async () => {
      await expect(client.list("/")).rejects.toThrow("연결되어 있지");
    });
  });
  ...
  describe("exists", () => {
    ...
    it("파일이 존재하지 않으면 false를 반환해야 함", async () => {
    ...
    it("디렉토리가 존재하면 true를 반환해야 함 (type='d')", async () => {
    ...
    it("심볼릭 링크가 존재하면 true를 반환해야 함 (type='l')", async () => {
  ...
  describe("readFile", () => {
    ...
    it("예상치 못한 타입에 대해 오류를 발생시켜야 함", async () => {
      mockGet.mockResolvedValueOnce({ unexpected: "object" });
      await client.connect({ host: "test" });

      await expect(client.readFile("/file.txt")).rejects.toThrow("응답 타입");
    });
  });
  ...
  describe("put", () => {
    it("fastPut을 사용하여 로컬 경로에서 업로드해야 함", async () => {
      await client.connect({ host: "test" });
    ...
    it("put을 사용하여 Uint8Array에서 업로드해야 함", async () => {
      await client.connect({ host: "test" });
  ...
  describe("close", () => {
    it("연결 전 close 호출 시 오류 없이 종료해야 함", async () => {
    ...
    it("close 후 메서드 호출 시 오류를 발생시켜야 함", async () => {
      await client.connect({ host: "test" });
      await client.close();
