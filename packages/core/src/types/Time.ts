...
export class Time {
  ...
  public constructor(date: number, hour: number, minute: number, second: number, millisecond: number);
  ...
  public constructor(args1?: number | Date, args2?: number, args3?: number, args4?: number, args5?: number) {
    if (args1 === undefined) {
      ...
      throw new InvalidArgumentsException({args: [args1, args2, args3, args4]});
    }
  }
  ...
  public static parse(value: string): Time {
    ...
    // 1050103001, 01050103001 => 1일 5시간 1분 3초 1밀리초
      ...
      const day = Number(matches[1] || "0");
      const hour = Number(matches[2] || "0");
      const minute = Number(matches[3] || "0");
      const second = Number(matches[4] || "0");
      const millisecond = Number(matches[5] || "0");
