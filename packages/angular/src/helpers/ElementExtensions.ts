...
declare global {
    interface Element {
        on<T extends Event>(eventKey: string, callback: (event: T) => void, includeChildren?: boolean): void;
...
Element.prototype.on = function (eventKey: string, callback: Function, includeChildren?: boolean): void {
...
Element.prototype.off = function (eventKey: string): void {
    ...
    for (const listener of listeners.filter(item => item.eventKey === eventKey)) {
        this.removeEventListener(eventKey.split(".")[0], listener.callback as any, listener.includeChildren);
    ...
    if (typeof predicate === "string") {
        ...
        const getMatchedChild = (cursor: Element): (HTMLElement | undefined) => {
            ...
            for (const child of children) {
                ...
                if (matchedChildChild) return matchedChildChild;
            }
        };
