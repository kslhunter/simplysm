...
export class SdSelectControl<M extends "single" | "multi", T> {
  ...
  _itemControls = contentChildren(SdSelectItemControl);
  private readonly _dropdownControl = viewChild.required(SdDropdownControl);
  private readonly _dropdownElRef = viewChild.required<SdDropdownControl, ElementRef<HTMLElement>>(
    SdDropdownControl,
    { read: ElementRef },
  );
  ...
  _itemOfTpl = contentChild(SdItemOfTemplateDirective, { read: TemplateRef });
