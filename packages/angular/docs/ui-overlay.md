...
busy 상태를 관리할 때 `boolean` signal 대신 **카운트 signal**을 사용한다. 여러 비동기 작업이 동시에 진행될 때 마지막 작업이 끝나야 busy가 해제되도록 보장한다.
...
긴 작업(엑셀 변환, 대량 데이터 처리 등)에서만 진행 문구가 필요하다. 일반적인 CRUD 조회/저장에는 사용하지 않는다.
...
- [crud-list.md §3 최소 뼈대: 조회 전용 page](./recipes/crud-list.md#3-최소-뼈대-조회-전용-page) — busyCount 카운트 패턴
- [crud-list.md §11 확장 G: 엑셀 업로드/다운로드](./recipes/crud-list.md#11-확장-g-엑셀-업로드다운로드) — busyMessage 사용
- [crud-detail.md §3 최소 뼈대: 읽기 전용 상세 폼](./recipes/crud-detail.md#3-최소-뼈대-읽기-전용-상세-폼) — busyCount 카운트 패턴
