...
describe("fetchUrlBytes", () => {
  ...
  it("throws when received data exceeds Content-Length", async () => {
    const chunk1 = new Uint8Array([1, 2, 3, 4, 5]);
    const chunk2 = new Uint8Array([6, 7, 8, 9, 10]);
    const { response } = createMockResponse({
      contentLength: 7, // Only 7, but 10 bytes will be received
      chunks: [chunk1, chunk2],
    });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response));

    await expect(fetchUrlBytes("https://example.com/overflow")).rejects.toThrow();
  });

  it("throws when received data is less than Content-Length", async () => {
    const chunk1 = new Uint8Array([1, 2, 3]);
    const { response } = createMockResponse({
      contentLength: 10, // Expects 10, but only 3 bytes received
      chunks: [chunk1],
    });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response));

    await expect(fetchUrlBytes("https://example.com/underflow")).rejects.toThrow();
  });

  it("succeeds when received data matches Content-Length exactly", async () => {
    const chunk1 = new Uint8Array([1, 2, 3, 4, 5]);
    const chunk2 = new Uint8Array([6, 7, 8, 9, 10]);
    const { response } = createMockResponse({
      contentLength: 10,
    ...
    const result = await fetchUrlBytes("https://example.com/exact");
