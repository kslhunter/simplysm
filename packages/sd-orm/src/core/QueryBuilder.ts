...
`;
  ...
  createPrimaryKey(tableName: string,
  ...
  createForeignKey(tableName: string, foreignKey: {
    name: string;
    columnNames: string[];
...
`;
  ...
  createFunction(def: IFunctionDefinition): string {
    if (!def.returnType) {
...
END`;
  ...
  createProcedure(def: IStoredProcedureDefinition): string {
    const q = `
