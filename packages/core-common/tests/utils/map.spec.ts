...
import { mapGetOrCreate, mapUpdate } from "../../src/utils/map";
...
describe("map utils", () => {
  ...
  describe("mapGetOrCreate()", () => {
    it("키가 없으면 직접 값을 설정하고 반환한다", () => {
      ...
      const result = mapGetOrCreate(map, "key", 100);
    ...
    it("키가 없으면 팩토리 함수를 호출하여 설정하고 반환한다", () => {
      ...
      const result = mapGetOrCreate(map, "key", () => {
        factoryCalled = true;
        return [1, 2, 3];
      });
    ...
    it("빈 배열을 기본값으로 설정할 수 있다", () => {
      ...
      const arr = mapGetOrCreate(map, "key", []);
      arr.push(1, 2, 3);
    ...
    it("팩토리 함수로 복잡한 객체를 생성할 수 있다", () => {
      ...
      const obj = mapGetOrCreate(map, "key", () => ({ count: 0, items: [] }));
      obj.count++;
    ...
    it("data-last 스타일로 사용할 수 있다", () => {
      const map = new Map<string, number>();
      const fn = mapGetOrCreate("key", 100);
  ...
  describe("mapUpdate()", () => {
    ...
    it("객체 값을 업데이트할 수 있다", () => {
      ...
      mapUpdate(map, "key", (v) => ({ count: (v?.count ?? 0) + 1 }));
    ...
    it("data-last 스타일로 사용할 수 있다", () => {
      const map = new Map<string, number>();
      map.set("key", 10);
      const fn = mapUpdate("key", (v: number | undefined) => (v ?? 0) + 5);
