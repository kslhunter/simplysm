...
import { SdAsyncFnSerialQueue, Wait, SdError } from "@simplysm/core-common";

describe("SdAsyncFnSerialQueue", () => {
  //#region 순차 실행
  ...
  describe("gap 간격", () => {
    it("gap이 설정되면 작업 사이에 대기한다", async () => {
      const queue = new SdAsyncFnSerialQueue(50);
      const timestamps: number[] = [];
    ...
    it("gap이 0이면 즉시 다음 작업을 실행한다", async () => {
      const queue = new SdAsyncFnSerialQueue(0);
      const timestamps: number[] = [];
    ...
    it("마지막 작업 후에는 gap을 대기하지 않는다", async () => {
      const queue = new SdAsyncFnSerialQueue(100);
      const timestamps: number[] = [];
