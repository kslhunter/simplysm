import { describe, it, expect } from "vitest";
import "@simplysm/core-common"; // Enable $ extension

describe("Array 프로토타입 확장", () => {
  //#region single

  describe("single()", () => {
    it("일치하는 단일 요소 반환", () => {
      const result = [1, 2, 3].single((x) => x === 2);

      expect(result).toBe(2);
    });

    it("일치하는 요소 없으면 undefined 반환", () => {
      const result = [1, 2, 3].single((x) => x === 4);

      expect(result).toBe(undefined);
    });

    it("일치하는 요소가 여럿이면 오류 발생", () => {
      expect(() => [1, 1, 2].single((x) => x === 1)).toThrow();
    });

    it("조건 없으면 전체 배열 대상", () => {
      expect([1].single()).toBe(1);
      expect(([] as number[]).single()).toBe(undefined);
      expect(() => [1, 2].single()).toThrow();
    });
  });

  //#endregion

  //#region Async methods

  describe("parallelAsync()", () => {
    it("병렬 비동기 실행", async () => {
      const result = await [1, 2, 3].parallelAsync(async (x) => Promise.resolve(x * 2));

      expect(result).toEqual([2, 4, 6]);
    });
  });

  describe("mapAsync()", () => {
    it("순차 비동기 매핑", async () => {
      const result = await [1, 2, 3].mapAsync(async (x) => Promise.resolve(x * 2));

      expect(result).toEqual([2, 4, 6]);
    });
  });

  describe("filterAsync()", () => {
    it("비동기 필터링", async () => {
      const result = await [1, 2, 3, 4, 5].filterAsync(async (x) => Promise.resolve(x > 2));

      expect(result).toEqual([3, 4, 5]);
    });
  });

  //#endregion

  //#region Map conversion

  describe("toMap()", () => {
    it("키 함수로 Map 생성", () => {
      const result = [
        { id: 1, name: "a" },
        { id: 2, name: "b" },
      ].toMap((x) => x.id);

      expect(result.get(1)).toEqual({ id: 1, name: "a" });
      expect(result.get(2)).toEqual({ id: 2, name: "b" });
    });

    it("값 함수로 값 변환", () => {
      const result = [
        { id: 1, name: "a" },
        { id: 2, name: "b" },
      ].toMap(
        (x) => x.id,
        (x) => x.name,
      );

      expect(result.get(1)).toBe("a");
      expect(result.get(2)).toBe("b");
    });

    it("중복 키에 대해 오류 발생", () => {
      expect(() =>
        [
          { id: 1, name: "a" },
          { id: 1, name: "b" },
        ].toMap((x) => x.id),
      ).toThrow("중복된 key");
    });
  });

  describe("toMapAsync()", () => {
    it("비동기 키/값 함수로 Map 생성", async () => {
      const result = await [
        { id: 1, name: "a" },
        { id: 2, name: "b" },
      ].toMapAsync(async (x) => Promise.resolve(x.id));

      expect(result.get(1)).toEqual({ id: 1, name: "a" });
      expect(result.get(2)).toEqual({ id: 2, name: "b" });
    });
  });

  describe("toArrayMap()", () => {
    it("배열 값으로 Map 생성", () => {
      const result = [
        { type: "a", v: 1 },
        { type: "b", v: 2 },
        { type: "a", v: 3 },
      ].toArrayMap((x) => x.type);

      expect(result.get("a")).toHaveLength(2);
      expect(result.get("b")).toHaveLength(1);
    });

    it("값 함수로 값 변환", () => {
      const result = [
        { type: "a", v: 1 },
        { type: "a", v: 2 },
      ].toArrayMap(
        (x) => x.type,
        (x) => x.v,
      );

      expect(result.get("a")).toEqual([1, 2]);
    });
  });

  describe("toSetMap()", () => {
    it("Set 값으로 Map 생성", () => {
      const result = [
        { type: "a", v: 1 },
        { type: "a", v: 1 }, // duplicate
        { type: "a", v: 2 },
      ].toSetMap(
        (x) => x.type,
        (x) => x.v,
      );

      expect(result.get("a")?.size).toBe(2); // duplicates removed
    });
  });

  describe("toMapValues()", () => {
    it("그룹별 집계 결과로 Map 생성", () => {
      const result = [
        { type: "a", v: 10 },
        { type: "b", v: 20 },
        { type: "a", v: 30 },
      ].toMapValues(
        (x) => x.type,
        (items) => items.reduce((sum, x) => sum + x.v, 0),
      );

      expect(result.get("a")).toBe(40);
      expect(result.get("b")).toBe(20);
    });
  });

  //#endregion

  //#region Tree conversion

  describe("toTree()", () => {
    it("트리 구조로 변환", () => {
      interface Item {
        id: number;
        parentId?: number;
        name: string;
      }

      const items: Item[] = [
        { id: 1, name: "root" },
        { id: 2, parentId: 1, name: "child1" },
        { id: 3, parentId: 1, name: "child2" },
        { id: 4, parentId: 2, name: "grandchild" },
      ];

      const result = items.toTree("id", "parentId");

      expect(result).toHaveLength(1);
      expect(result[0].children).toHaveLength(2);
      expect(result[0].children[0].children).toHaveLength(1);
    });
  });

  //#endregion

  //#region Array comparison

  describe("diffs()", () => {
    it("배열 간 차이 분석", () => {
      interface Item {
        [key: string]: unknown;
        id: number;
        value: string;
      }

      const source: Item[] = [
        { id: 1, value: "a" },
        { id: 2, value: "b" },
        { id: 3, value: "c" },
      ];

      const target: Item[] = [
        { id: 2, value: "b" },
        { id: 3, value: "changed" },
        { id: 4, value: "d" },
      ];

      const result = source.diffs(target, { keys: ["id"] });

      const deleted = result.find((d) => d.source?.["id"] === 1);
      expect(deleted?.target).toBe(undefined);

      const updated = result.find((d) => d.source?.["id"] === 3);
      expect(updated?.target?.["value"]).toBe("changed");

      const inserted = result.find((d) => d.target?.["id"] === 4);
      expect(inserted?.source).toBe(undefined);
    });
  });

  describe("oneWayDiffs()", () => {
    it("단방향 차이 분석", () => {
      interface Item {
        id: number;
        value: string;
      }

      const orgItems: Item[] = [
        { id: 1, value: "a" },
        { id: 2, value: "b" },
      ];

      const items: Item[] = [
        { id: 2, value: "changed" },
        { id: 3, value: "c" },
      ];

      const result = items.oneWayDiffs(orgItems, "id");

      const updated = result.find((d) => d.item.id === 2);
      expect(updated?.type).toBe("update");

      const created = result.find((d) => d.item.id === 3);
      expect(created?.type).toBe("create");
    });

    it("includeSame=true 시 변경 없는 항목 포함", () => {
      interface Item {
        id: number;
        value: string;
      }

      const orgItems: Item[] = [
        { id: 1, value: "a" },
        { id: 2, value: "b" },
      ];

      const items: Item[] = [
        { id: 1, value: "a" }, // 변경 없음
        { id: 2, value: "changed" },
      ];

      const result = items.oneWayDiffs(orgItems, "id", { includeSame: true });

      const same = result.find((d) => d.item.id === 1);
      expect(same?.type).toBe("same");

      const updated = result.find((d) => d.item.id === 2);
      expect(updated?.type).toBe("update");
    });
  });

  describe("merge()", () => {
    it("수정된 항목 병합", () => {
      interface Item {
        [key: string]: unknown;
        id: number;
        value: string;
      }

      const source: Item[] = [
        { id: 1, value: "a" },
        { id: 2, value: "b" },
      ];
      const target: Item[] = [
        { id: 1, value: "a" },
        { id: 2, value: "changed" },
      ];

      const result = source.merge(target, { keys: ["id"] });

      expect(result).toHaveLength(2);
      expect(result.find((r) => r["id"] === 2)?.["value"]).toBe("changed");
    });
  });

  //#endregion

  //#region first, last

  describe("first()", () => {
    it("첫 번째 요소 반환", () => {
      expect([1, 2, 3].first()).toBe(1);
    });

    it("첫 번째 일치 요소 반환", () => {
      expect([1, 2, 3, 4, 5].first((x) => x > 3)).toBe(4);
    });

    it("빈 배열에 대해 undefined 반환", () => {
      expect(([] as number[]).first()).toBe(undefined);
    });
  });

  describe("last()", () => {
    it("마지막 요소 반환", () => {
      expect([1, 2, 3].last()).toBe(3);
    });

    it("마지막 일치 요소 반환", () => {
      expect([1, 2, 3, 4, 5].last((x) => x < 4)).toBe(3);
    });

    it("빈 배열에 대해 undefined 반환", () => {
      expect(([] as number[]).last()).toBe(undefined);
    });
  });

  //#endregion

  //#region filterExists, ofType

  describe("filterExists()", () => {
    it("null/undefined 제거", () => {
      const arr = [1, null, 2, undefined, 3];
      const result = arr.filterExists();
      expect(result).toEqual([1, 2, 3]);
    });
  });

  describe("ofType()", () => {
    it("string 타입 요소만 필터링", () => {
      const arr = [1, "a", 2, "b", true];
      const result = arr.ofType("string");
      expect(result).toEqual(["a", "b"]);
    });

    it("number 타입 요소만 필터링", () => {
      const arr = [1, "a", 2, "b", 3];
      const result = arr.ofType("number");
      expect(result).toEqual([1, 2, 3]);
    });

    it("boolean 타입 요소만 필터링", () => {
      const arr = [1, "a", true, false, 2];
      const result = arr.ofType("boolean");
      expect(result).toEqual([true, false]);
    });
  });

  //#endregion

  //#region mapMany

  describe("mapMany()", () => {
    it("매핑 후 평탄화", () => {
      const result = [1, 2, 3].mapMany((x) => [x, x * 10]);
      expect(result).toEqual([1, 10, 2, 20, 3, 30]);
    });
  });

  describe("mapManyAsync()", () => {
    it("비동기 매핑 후 평탄화", async () => {
      const result = await [1, 2, 3].mapManyAsync(async (x) => Promise.resolve([x, x * 10]));
      expect(result).toEqual([1, 10, 2, 20, 3, 30]);
    });

    it("비동기 중첩 Promise 배열 매핑 후 평탄화", async () => {
      const result = await [1, 2].mapManyAsync(async (x) => Promise.resolve([x, x + 1, x + 2]));
      expect(result).toEqual([1, 2, 3, 2, 3, 4]);
    });
  });

  //#endregion

  //#region groupBy

  describe("groupBy()", () => {
    it("키별 그룹화", () => {
      const items = [
        { type: "a", value: 1 },
        { type: "b", value: 2 },
        { type: "a", value: 3 },
      ];
      const result = items.groupBy((x) => x.type);

      expect(result).toHaveLength(2);
      expect(result.find((g) => g.key === "a")?.values).toHaveLength(2);
      expect(result.find((g) => g.key === "b")?.values).toHaveLength(1);
    });
  });

  //#endregion

  //#region toObject

  describe("toObject()", () => {
    it("배열을 객체로 변환", () => {
      const items = [
        { key: "a", value: 1 },
        { key: "b", value: 2 },
      ];
      const result = items.toObject(
        (x) => x.key,
        (x) => x.value,
      );

      expect(result).toEqual({ a: 1, b: 2 });
    });

    it("중복 키에 대해 오류 발생", () => {
      const items = [
        { key: "a", value: 1 },
        { key: "a", value: 2 },
      ];
      expect(() => items.toObject((x) => x.key)).toThrow();
    });
  });

  //#endregion

  //#region distinct

  describe("distinct()", () => {
    it("중복 제거", () => {
      expect([1, 2, 2, 3, 3, 3].distinct()).toEqual([1, 2, 3]);
    });

    it("객체 배열에서 중복 제거", () => {
      const arr = [{ a: 1 }, { a: 2 }, { a: 1 }];
      const result = arr.distinct();
      expect(result).toHaveLength(2);
    });

    it("keyFn으로 커스텀 키 사용 가능", () => {
      const arr = [
        { id: 1, name: "a" },
        { id: 2, name: "b" },
        { id: 1, name: "c" },
      ];
      const result = arr.distinct({ keyFn: (x) => x.id });
      expect(result).toHaveLength(2);
    });

    it("matchAddress=true로 참조 기반 중복 제거", () => {
      const obj1 = { a: 1 };
      const obj2 = { a: 1 }; // same value but different reference
      const arr = [obj1, obj1, obj2];
      const result = arr.distinct({ matchAddress: true });
      expect(result).toHaveLength(2);
      expect(result).toContain(obj1);
      expect(result).toContain(obj2);
    });
  });

  //#endregion

  //#region orderBy, orderByDesc

  describe("orderBy()", () => {
    it("오름차순 정렬", () => {
      expect([3, 1, 2].orderBy()).toEqual([1, 2, 3]);
    });

    it("selector로 정렬 기준 지정 가능", () => {
      const items = [
        { name: "b", age: 30 },
        { name: "a", age: 20 },
        { name: "c", age: 25 },
      ];
      const result = items.orderBy((x) => x.age);
      expect(result.map((x) => x.age)).toEqual([20, 25, 30]);
    });
  });

  describe("orderByDesc()", () => {
    it("내림차순 정렬", () => {
      expect([1, 3, 2].orderByDesc()).toEqual([3, 2, 1]);
    });
  });

  //#endregion

  //#region sum, min, max

  describe("sum()", () => {
    it("합계 반환", () => {
      expect([1, 2, 3, 4, 5].sum()).toBe(15);
    });

    it("selector로 값 추출 가능", () => {
      const items = [{ value: 10 }, { value: 20 }, { value: 30 }];
      expect(items.sum((x) => x.value)).toBe(60);
    });

    it("빈 배열에 대해 0 반환", () => {
      expect(([] as number[]).sum()).toBe(0);
    });

    it("숫자가 아닌 타입에 대해 오류 발생", () => {
      expect(() => (["a", "b"] as unknown as number[]).sum()).toThrow("숫자에만");
    });
  });

  describe("min()", () => {
    it("최솟값 반환", () => {
      expect([3, 1, 2].min()).toBe(1);
    });

    it("빈 배열에 대해 undefined 반환", () => {
      expect(([] as number[]).min()).toBe(undefined);
    });

    it("숫자/문자열이 아닌 타입에 대해 오류 발생", () => {
      expect(() => ([true, false] as unknown as number[]).min()).toThrow("숫자/문자열");
    });
  });

  describe("max()", () => {
    it("최댓값 반환", () => {
      expect([1, 3, 2].max()).toBe(3);
    });

    it("빈 배열에 대해 undefined 반환", () => {
      expect(([] as number[]).max()).toBe(undefined);
    });

    it("숫자/문자열이 아닌 타입에 대해 오류 발생", () => {
      expect(() => ([{}, {}] as unknown as number[]).max()).toThrow("숫자/문자열");
    });
  });

  //#endregion

  //#region shuffle

  describe("shuffle()", () => {
    it("배열 셔플 (원본 유지)", () => {
      const original = [1, 2, 3, 4, 5];
      const shuffled = original.shuffle();

      // 원본 변경 없음
      expect(original).toEqual([1, 2, 3, 4, 5]);
      // has same elements
      expect(shuffled.sort()).toEqual([1, 2, 3, 4, 5]);
    });
  });

  //#endregion

  //#region Mutating methods

  describe("distinctThis()", () => {
    it("원본 배열에서 중복 제거", () => {
      const arr = [1, 2, 2, 3, 3, 3];
      const result = arr.distinctThis();

      expect(arr).toEqual([1, 2, 3]);
      expect(result).toEqual([1, 2, 3]);
    });
  });

  describe("orderByThis()", () => {
    it("원본 배열을 오름차순 정렬", () => {
      const arr = [3, 1, 2];
      arr.orderByThis();

      expect(arr).toEqual([1, 2, 3]);
    });
  });

  describe("orderByDescThis()", () => {
    it("원본 배열을 내림차순 정렬", () => {
      const arr = [1, 3, 2];
      arr.orderByDescThis();

      expect(arr).toEqual([3, 2, 1]);
    });
  });

  describe("insert()", () => {
    it("원본 배열에 항목 삽입", () => {
      const arr = [1, 3];
      arr.insert(1, 2);

      expect(arr).toEqual([1, 2, 3]);
    });
  });

  describe("remove()", () => {
    it("원본 배열에서 항목 제거", () => {
      const arr = [1, 2, 3];
      arr.remove(2);

      expect(arr).toEqual([1, 3]);
    });

    it("조건 함수로 항목 제거", () => {
      const arr = [1, 2, 3, 4];
      arr.remove((x) => x % 2 === 0);

      expect(arr).toEqual([1, 3]);
    });
  });

  describe("toggle()", () => {
    it("항목이 있으면 제거", () => {
      const arr = [1, 2, 3];
      arr.toggle(2);

      expect(arr).toEqual([1, 3]);
    });

    it("항목이 없으면 추가", () => {
      const arr = [1, 3];
      arr.toggle(2);

      expect(arr).toEqual([1, 3, 2]);
    });
  });

  describe("clear()", () => {
    it("원본 배열 비우기", () => {
      const arr = [1, 2, 3];
      arr.clear();

      expect(arr).toEqual([]);
    });
  });

  //#endregion
});
