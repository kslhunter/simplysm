...
export function objClone<T>(): (source: T) => T;
export function objClone<T>(source?: T): T | ((source: T) => T) {
  if (arguments.length === 0) return (s: T) => objCloneImpl(s) as T;
...
function objEqualArray(
  ...
  if (options?.ignoreArrayIndex) {
    ...
    if (options?.onlyOneDepth) {
      ...
      for (let i = 0; i < source.length; i++) {
        if (!objEqual(source[i], target[i], options)) {
          return false;
        }
      }
    }
  ...
  for (const sourceKey of sourceKeys) {
    ...
    if (typeof sourceKey === "string") {
      ...
      if (options?.onlyOneDepth) {
        ...
        if (!objEqual(sourceValue, targetValue, { ignoreArrayIndex: options?.ignoreArrayIndex, onlyOneDepth: options?.onlyOneDepth })) {
          return false;
        }
      }
    } else {
...
function objEqualSet(
  ...
  if (options?.onlyOneDepth) {
    ...
    for (const sourceItem of source) {
      const hasMatch = targetArr.some((targetItem) =>
        objEqual(sourceItem, targetItem, options),
      );
      if (!hasMatch) {
        return false;
      }
...
export function objOmit<T extends Record<string, unknown>, K extends keyof T>(
  itemOrKeys: T | K[],
  omitKeys?: K[],
): Omit<T, K> | ((item: T) => Omit<T, K>) {
  if (omitKeys === undefined) {
    // data-last
    const keys = itemOrKeys as K[];
    return (item: T) => objOmitImpl(item, keys);
  }
  // data-first
  return objOmitImpl(itemOrKeys as T, omitKeys);
}

function objOmitImpl<T extends Record<string, unknown>, K extends keyof T>(
  item: T,
  omitKeys: K[],
): Omit<T, K> {
  const result: Record<string, unknown> = {};
 ...
 *
 * // pipe에서 사용
 * pipe(data, objOmitByFilter((key) => key.startsWith("_")));
 */
...
export function objOmitByFilter<T extends Record<string, unknown>>(
  itemOrFn: T | ((key: keyof T) => boolean),
  omitKeyFn?: (key: keyof T) => boolean,
): T | ((item: T) => T) {
  if (omitKeyFn === undefined) {
    // data-last
    const fn = itemOrFn as (key: keyof T) => boolean;
    return (item: T) => objOmitByFilterImpl(item, fn);
  }
  // data-first
  return objOmitByFilterImpl(itemOrFn as T, omitKeyFn);
}

function objOmitByFilterImpl<T extends Record<string, unknown>>(
  item: T,
  omitKeyFn: (key: keyof T) => boolean,
): T {
  const result: Record<string, unknown> = {};
...
export function objPick<T extends Record<string, unknown>, K extends keyof T>(
  itemOrKeys: T | K[],
  keys?: K[],
): Pick<T, K> | ((item: T) => Pick<T, K>) {
  if (keys === undefined) {
    // data-last
    const pickKeys = itemOrKeys as K[];
    return (item: T) => objPickImpl(item, pickKeys);
  }
  // data-first
  return objPickImpl(itemOrKeys as T, keys);
}

function objPickImpl<T extends Record<string, unknown>, K extends keyof T>(
  item: T,
  keys: K[],
): Pick<T, K> {
  const result: Record<string, unknown> = {};
...
export function objClearUndefined<T extends object>(): (obj: T) => T;
export function objClearUndefined<T extends object>(obj?: T): T | ((obj: T) => T) {
  if (arguments.length === 0) return (o: T) => objClearUndefinedImpl(o);
...
export function objNullToUndefined<T>(): (obj: T) => T | undefined;
export function objNullToUndefined<T>(obj?: T): (T | undefined) | ((obj: T) => T | undefined) {
  if (arguments.length === 0) return (o: T) => objNullToUndefinedImpl(o);
  return objNullToUndefinedImpl(obj as T);
}
