...
describe("TextField 컴포넌트", () => {
  describe("기본 렌더링", () => {
    ...
    it("placeholder가 input에 적용된다", () => {
      const { container } = render(() => <TextField placeholder="Enter text" />);
      const input = container.querySelector("input") as HTMLInputElement;
      expect(input.placeholder).toBe("Enter text");
    });

    it("title이 input에 적용된다", () => {
      const { container } = render(() => <TextField title="Input title" />);
      const input = container.querySelector("input") as HTMLInputElement;
      expect(input.title).toBe("Input title");
    });

    it("autocomplete가 input에 적용된다", () => {
      const { container } = render(() => <TextField autocomplete="email" />);
      const input = container.querySelector("input") as HTMLInputElement;
  ...
  describe("controlled 패턴", () => {
    ...
    it("onValueChange가 입력 시 호출된다", () => {
      ...
      const { container } = render(() => <TextField value="" onValueChange={handleChange} />);
      const input = container.querySelector("input") as HTMLInputElement;
    ...
    it("외부 상태 변경 시 input 값이 업데이트된다", () => {
      ...
      const { container } = render(() => <TextField value={value()} onValueChange={setValue} />);
      const input = container.querySelector("input") as HTMLInputElement;
  ...
  describe("uncontrolled 패턴", () => {
    it("onValueChange 없이 내부 상태로 값이 관리된다", () => {
      const { container } = render(() => <TextField value="Initial" />);
      const input = container.querySelector("input") as HTMLInputElement;
  ...
  describe("disabled 상태", () => {
    it("disabled=true일 때 div로 렌더링된다", () => {
      const { container } = render(() => <TextField disabled value="Disabled text" />);
      const input = container.querySelector("input");
    ...
    it("disabled 상태에서 value가 표시된다", () => {
      const { getByText } = render(() => <TextField disabled value="Disabled text" />);
  ...
  describe("readonly 상태", () => {
    it("readonly=true일 때 div로 렌더링된다", () => {
      const { container } = render(() => <TextField readonly value="Readonly text" />);
      const input = container.querySelector("input");
      const div = container.querySelector("div.sd-text-field");
    ...
    it("readonly 상태에서 value가 표시된다", () => {
      const { getByText } = render(() => <TextField readonly value="Readonly text" />);
  ...
  describe("format 옵션", () => {
    it("format이 적용되어 표시된다", () => {
      const { container } = render(() => <TextField format="XXX-XXXX-XXXX" value="01012345678" />);
      const input = container.querySelector("input") as HTMLInputElement;
    ...
    it("format이 적용된 상태에서 입력 시 원본 값이 onValueChange로 전달된다", () => {
      ...
      const { container } = render(() => (
        <TextField format="XXX-XXXX-XXXX" value="" onValueChange={handleChange} />
      ));
      const input = container.querySelector("input") as HTMLInputElement;
  ...
  describe("class 병합", () => {
    it("사용자 정의 class가 기존 스타일과 병합된다", () => {
      ...
      const { container } = render(() => <TextField class="my-custom-class" />);
      const wrapper = container.firstChild as HTMLElement;
