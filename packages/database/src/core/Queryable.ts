...
import {DateOnly, Exception, LambdaParser, NotImplementedException, Safe, Type, Uuid} from "@simplism/core";
...
export class QueryUnit<T> {
    constructor(public type: Type<T> | undefined, public query: string) {
    }
}

export class Queryable<T> {
    private _distinct?: boolean;
    private _from: string | Queryable<any> | Queryable<any>[];
    private _as: string;
    private _top?: number;
    ...
    constructor(public db: Database, public tableType: Type<T> | undefined, from?: string | Queryable<T> | Queryable<any>[], as?: string) {
    ...
    async insertAsync(record: T): Promise<any> {
        if (!this.tableType) {
    ...
    async insertRangeAsync(records: T[]): Promise<any[]> {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    insertRangePrepare(records: T[]): void {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
...
END`;
    ...
    async bulkInsertAsync(objs: T[]): Promise<void> {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    async upsertAsync(record: T, keys?: (keyof T)[]): Promise<any> {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    upsertPrepare(record: T, keys?: (keyof T)[]): void {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    async upsertRangeAsync(records: T[], keys?: (keyof T)[]): Promise<T[]> {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    upsertRangePrepare(records: T[], keys?: (keyof T)[]): void {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
...
ON ${keys.map(key => `${this._key(tableDef.name, key)} = ${this._key("match", key)}`).join(" AND ")}
WHEN MATCHED THEN
...
END`;
    ...
    async insertToAsync<G extends { [key: string]: any }>(to: Queryable<G>, predicate: (q: QueryMaker<T>) => G): Promise<number[]> {
    ...
    async updateAsync(predicate: (q: QueryMaker<T>) => Partial<T> | void): Promise<number> {
        const selectQuery = this.query.replace(/\n/g, "\n\t");
        const entity = this._wrap()._entity;
        const obj = predicate(new QueryMaker(entity)) || entity;


        const getUpdateFieldQuery = (obj: any, parentNameChain?: string[]): string => {
            let fieldQuery = "";
            const realObj = obj instanceof Array ? obj[0] : obj;
            for (const columnName of Object.keys(realObj)) {
                ...
                if (this._join.some(item => item.queryable._as === (parentNameChain ? parentNameChain.concat(columnName).join(".") : columnName))) {
                    fieldQuery += getUpdateFieldQuery(realObj[columnName], (parentNameChain ? parentNameChain.concat([columnName]) : [columnName]));
                }
                else {
                    fieldQuery += "\t" + (parentNameChain ? this._key(parentNameChain.concat(columnName).join(".")) : this._key(columnName)) + " = ";

                    if (queryValue instanceof QueryUnit) {
                        if (queryValue.type === QueriedBoolean) {
                            fieldQuery += "CASE WHEN " + queryValue.query + " THEN 1 ELSE 0 END";
                        }
                        else {
                            fieldQuery += queryValue.query;
                        }
                    }
                    else {
                        fieldQuery += this._value(queryValue);
                    }
                    fieldQuery += `,\n`;
                }
            }
    ...
    updatePrepare(predicate: (q: QueryMaker<T>) => Partial<T> | void): void {
        const selectQuery = this.query.replace(/\n/g, "\n\t");
    ...
    async deleteAsync(): Promise<number> {
        if (!this.tableType) {
            throw new Exception("UNION시에는 사용할 수 없는 기능입니다.");
        }
    ...
    select<S extends { [key: string]: any }>(selectorFn: (queryHelper: QueryMaker<T>) => S): Queryable<S> {
    ...
    where(queriesFn: (queryHelper: QueryMaker<T>) => (boolean | QueryUnit<QueriedBoolean>)[]): Queryable<T> {
    ...
    between<P>(targetFwd: ((q: QueryMaker<T>) => QueryUnit<P> | P) | QueryUnit<P> | P,
    ...
    equal<P>(srcFwd: ((q: QueryMaker<T>) => QueryUnit<P> | P) | QueryUnit<P> | P, targetFwd: ((q: QueryMaker<T>) => QueryUnit<P> | P) | QueryUnit<P> | P): Queryable<T> {
        return this.where(q => [
            q.equal(
    ...
    find(filter: { [P in keyof T]?: (QueryUnit<T[P]> | T[P]) }, nonStrict: boolean = false): Queryable<T> {
        return this.where(q => {
    ...
    search(searchText: (string | undefined), fieldsPredicate: (entity: T) => any[]): Queryable<T> {
        if (!searchText) {
            return this;
        }
    ...
    orderBy(...fns: ((entity: T) => [any, OrderByRule])[]): Queryable<T> {
        const result = this._hasCustomSelect ? this._wrap() : this._clone();
    ...
    groupBy(fn: (entity: T) => any[]): Queryable<T> {
        const result = this._hasCustomSelect ? this._wrap() : this._clone();
        result._groupBy.pushRange(fn(result._entity));
        return result;
    }

    groupBySelect<G, S>(groupByFn: (q: QueryMaker<T>) => G, selectorFn: (q: QueryMaker<T>) => S): Queryable<G & S> {
        const result = this._hasCustomSelect ? this._wrap() : this._clone();
    ...
    limit(skip: number, take: number, orderBy: (entity: T) => [any, OrderByRule], ...thenBys: ((entity: T) => [any, OrderByRule])[]): Queryable<T> {
    ...
    private _joinFn<J, A extends string, R>(queryableOrTableType: Queryable<J> | Type<J>,
        ...
        const prevItem = this._join.singleOr(undefined, item => item.queryable._as === as);
    ...
    join<J, A extends string, R>(queryableOrTableType: Queryable<J> | Type<J>,
                                 as: A,
                                 joinQueryFn: (q: Queryable<J>, entity: T) => Queryable<R>,
    ...
    joinMulti<J, A extends string, R>(queryableOrTableType: Queryable<J> | Type<J>,
                                      as: A,
                                      joinQueryFn: (q: Queryable<J>, entity: T) => Queryable<R>,
                                      inner: boolean = false): Queryable<T & { [P in A]: R[] | undefined }> {
    ...
    include<J>(targetFn: ((entity: T) => (J | J[] | undefined)) | string,
    ...
    async singleOrAsync<R>(replacement: R): Promise<T | R> {
        const query = this.query;
        const result = await this.db.execute<T>(query);
        const parsedResult = this._parseResult(result.recordset);
        if (parsedResult.length > 1) {
    ...
    async countAsync(fieldSelector?: (en: T) => any): Promise<number> {
    ...
    private _getChainedForeignKeyDef(as: string): IForeignKeyDefinition | IForeignKeyTargetDefinition {
        ...
        for (let i = 0; i < asSplit.length - 1; i++) {
            const fk = tableDef.foreignKeys.singleOr(undefined, item => item.name === asSplit[i]);
            const fkt = tableDef.foreignKeyTargets.singleOr(undefined, item => item.name === asSplit[i]);
            const join = this._getJoinDef(asSplit.slice(0, i + 1).join("."));

            //-- FK 인 경우
            if (fk) {
                const targetTableType = fk.targetTableTypeForwarder();
                tableDef = Reflect.getMetadata(tableMetadataSymbol, targetTableType);
            }

            //-- FKT 인 경우
            else if (fkt) {
                const targetTableType = fkt.sourceTableTypeForwarder();
                tableDef = Reflect.getMetadata(tableMetadataSymbol, targetTableType);
            }
        ...
        const fk = tableDef.foreignKeys.singleOr(undefined, item => item.name === asSplit.last());
        const fkt = tableDef.foreignKeyTargets.singleOr(undefined, item => item.name === asSplit.last());
    ...
    private _getJoinDef(as: string): IJoinDefinition {
        ...
        let joinDef = joinList.lastOr(undefined, item => as === item.queryable._as || as.startsWith(item.queryable._as + "."));
        const asSplit = as.split(".");
    ...
    */
    ...
    private _parseResult(recordset: { [key: string]: any }[]): T[] {
        ...
        joinNames = joinNames.distinct().orderByDesc(item => item.split(".").length);
        ...
        for (const joinName of joinNames) {
            result = result
                .groupBy(item => {
                    const result = {...item};
                    for (const key of Object.keys(item).filter(item => item.startsWith(joinName + "."))) {
                        delete result[key];
                    }
                    return result;
                })
