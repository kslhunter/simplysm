import {Exception, Logger, Type} from "@simplism/core";
...
export abstract class Database {
    ...
    unionAll<T>(args: Queryable<T>[]): Queryable<T> {
    ...
    async execute<T>(query: string): Promise<IResult<T>> {
        if (this.logger) {
                ...
                req.query<T>(query, (err, result) => {
                    if (err) {
                        try {
                            ...
                            reject(new Exception(errorMessage, err));
                        }
                        catch (err) {
                            reject(err);
                            return;
    ...
    async executeBulkInsert(table: Table): Promise<void> {
    ...
    async executePrepared<T>(): Promise<IResult<T>> {
        const result = await this.execute<T>(this._preparedQueries.join("\r\n\r\n"));
        this._preparedQueries = [];
        return result;
    }

    async call(name: string, params: { [key: string]: any }, outputs: { [key: string]: ISqlType }): Promise<IProcedureResult<any>> {
        if (this.logger) {
    ...
    static async connect<T extends Database, R>(dbContextType: Type<T>, fn: (dbContext: T) => Promise<R>): Promise<R> {
