...
`@simplysm/capacitor-plugin-intent` — Android 인텐트 플러그인. 브로드캐스트 송수신, 실행 인텐트 조회, Activity 결과 수신(`startActivityForResult`)을 제공한다. 산업용 디바이스(바코드 스캐너, PDA 등) 연동에 특화되어 있다. TypeScript 소스 4개 파일.
...
├── web/
│   └── IntentWeb.ts   ← 브라우저 폴백 (WebPlugin 상속, stub 반환)
└── index.ts           ← public API re-exports
android/
└── src/main/kotlin/kr/co/simplysm/capacitor/intent/
    └── IntentPlugin.kt ← Android 네이티브 구현 (Kotlin)
tests/
├── intent-rename.spec.ts            ← IntentWeb stub 동작 검증
└── start-activity-for-result.spec.ts ← startActivityForResult 웹 폴백 검증
```
...
export abstract class Intent {
  static async subscribe(filters: string[], callback: (result: IntentResult) => void): Promise<() => Promise<void>> {
...
`Intent.subscribe()`는 구독 해제 함수를 반환한다. 플러그인 레벨의 `{ id }` 반환값을 감싸서 호출자가 ID를 직접 관리하지 않도록 한다.
...
`subscribe()` 콜백에서 `result.action != null` 조건으로 초기 resolve(id만 포함된 응답)를 걸러낸다. 이 조건을 제거하면 빈 콜백이 한 번 더 호출된다.

### 브라우저 폴백 (stub)

`IntentWeb`은 모든 메서드가 no-op 또는 stub 값을 반환한다. Android 전용 기능이므로 브라우저에서 실제 동작을 에뮬레이션하지 않는다.

- `subscribe()` → `{ id: "web-stub" }` 반환
- `getLaunchIntent()` → `{}` 반환
- `startActivityForResult()` → `{ resultCode: 0 }` 반환
- `send()`, `unsubscribe()`, `unsubscribeAll()` → 경고 로그 또는 무동작

## Android 네이티브

- 파일: `android/src/main/kotlin/kr/co/simplysm/capacitor/intent/IntentPlugin.kt`
- `subscribe()`: `RETURN_CALLBACK` 타입 메서드. `call.setKeepAlive(true)`로 콜백을 유지하며, `UUID`로 수신기 ID를 발급한다. Android 13+(TIRAMISU) 이상에서는 `RECEIVER_EXPORTED` 플래그를 명시해야 한다.
- `startActivityForResult()`: `@ActivityCallback`으로 `handleActivityResult()`를 콜백으로 등록한다.
- `handleOnDestroy()`: 등록된 모든 BroadcastReceiver를 정리한다. 리소스 누수 방지를 위해 이 로직을 유지해야 한다.
- `populateExtras()`: `String`, `Int`, `Long`, `Double`, `Boolean`, `JSONArray`, `JSONObject`를 Intent extra로 변환한다. `JSONObject`는 Bundle로 중첩 변환된다.

## Testing

**프레임워크**: Vitest

테스트는 `tests/` 디렉토리에 위치하며, 웹 폴백(`IntentWeb`) 동작만 검증한다. Android 네이티브 코드는 단위 테스트 범위에 포함하지 않는다.
