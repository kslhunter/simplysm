...
const hasIgnores = (
  config: ConfigItem,
): config is ConfigItem & { ignores: string[] } => {
  return "ignores" in config && Array.isArray(config.ignores);
};

const hasFiles = (
  config: ConfigItem,
): config is ConfigItem & { files: (string | string[])[] } => {
...
describe("recommended 설정", () => {
  ...
  it("JS 파일 설정이 포함되어야 함", () => {
    const jsConfig = recommended.find(
      (config) =>
        hasFiles(config) &&
        flattenFiles(config.files).some((f) => f.includes("*.js")),
    );
  ...
  it("TS 파일 설정이 포함되어야 함", () => {
    const tsConfig = recommended.find(
      (config) =>
        hasFiles(config) &&
        flattenFiles(config.files).some((f) => f.includes("*.ts")),
    );
  ...
  it("테스트 폴더에서 import/no-extraneous-dependencies가 off 되어야 함", () => {
    const testConfig = recommended.find(
      (config) =>
        hasFiles(config) &&
        flattenFiles(config.files).some((f) => f.includes("**/tests/**")),
    );
    expect(testConfig).toBeDefined();
    if (testConfig == null) return;
    if (hasRules(testConfig)) {
      expect(testConfig.rules).toHaveProperty(
  ...
  it("plugin.rules의 모든 규칙이 recommended 설정에 포함되어야 함", () => {
    ...
    if (hasRules(tsConfig)) {
      ...
      for (const ruleName of ruleNames) {
        ...
        expect(
          tsConfig.rules,
          `규칙 '${fullRuleName}'이 TS 설정에 포함되어야 함`,
        ).toHaveProperty(fullRuleName);
      }
    }
  });
