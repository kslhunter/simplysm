...
- **헤더 분할 직후 자동 OPEN**: SKILL.md 2단계(§2.x/§3.x) · 5단계(§4.x/§5.x) 확정 직후 본문 미작성 상태로 일괄 OPEN. 본문은 재분석용 참조 포인터 (분류·분할 시점엔 자료 위치 1줄로 시작, 이후 대화·확정 영향 메모 누적).
...
- 시스템·문서·메뉴·양식 명칭 그대로 X.
  ...
  - ✅ `구매처 발주 변경 요청`
- 외부 관계자 식별은 [SKILL.md 1단계](../SKILL.md) pivotal event 식별의 자연 도출.

### BPMN

- 사용자에겐 ASCII 로 보여주고, 확정 후 mermaid 로 문서 저장.
- 흐름 단위 = BPMN end-to-end process 1건 (start event ~ end event 의 풀 사이클). 단일 화면 입력 절차 X. 분할 단위 정의는 [SKILL.md 1단계](../SKILL.md) 참조.
...
- 단위 = Cockburn sea level use case (단일 user goal 완결). 정형 분해(화면/자동 처리) 동반하는 사용자 직접 요구. 분할 단위 정의는 [SKILL.md 1단계](../SKILL.md) 참조.
...
1단계 도메인 분할 + 이후 본문 작성 중 사용자가 명시 제외 결정한 항목을 누적. 본 spec 에서 다루지 않지만 다른 spec / sd-dev / 운영 잔손 등으로 후속 처리될 항목 포함. 형식 (`- <요구 한 줄>` 1행 + 자식 불릿 2~3종):
