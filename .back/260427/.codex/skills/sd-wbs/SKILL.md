...
항목별 근거가 없는 범위는 Feature에 포함하지 않는다. 필요해 보이지만 근거가 없으면 `$sd-inner-clarify`로 근거를 확인하고 포함 여부를 해소한다.

### 요구 명확화 규칙

**CRITICAL: Feature 범위에는 구현자가 바로 `$sd-dev`로 들어가도 해석이 갈리지 않는 고객 동작과 업무 규칙만 남긴다.**

- WBS Feature 안에는 "후속 확정", "보류", "추후 결정", "협의 필요" 같은 결정 대기 표현을 넣지 않는다.
- 원문/자료에서 해석이 갈리는 고객 요구, 업무 규칙, 정책, 예외 흐름은 WBS 작성 시점에 `$sd-inner-clarify`로 해소한 뒤 작성한다.
- `$sd-inner-clarify`를 거친 항목은 해소된 고객 동작과 업무 규칙만 Feature에 반영한다.
- 사용자가 명시적으로 제외한 항목만 "제외 사항"에 기재한다.
...
4. **제외로 결정되면** "제외 사항"에 기재하고 Feature는 만들지 않는다.
...
3. **중복/누락/역추적** — 요구사항(Impact Mapping의 Deliverable) 대비 양방향 커버리지를 점검한다.
   ...
   - **정방향 (Deliverable → Feature):** 요구사항에 있는데 어떤 Feature에도 없는 항목은 추가한다. 사용자가 제외했거나 Goal에 연결되지 않는 항목만 "제외 사항"으로 사유와 함께 이동한다.
...
7. **결정 대기 표현 제거** — "후속 확정", "보류", "추후 결정", "협의 필요" 같은 표현이 Feature 범위/경계/근거에 남아 있으면 `$sd-inner-clarify`로 해소한 뒤 Feature 범위/경계/근거를 재작성한다. 사용자가 제외한 항목은 "제외 사항"으로 이동한다.
8. **범위 항목별 근거 확인** — 모든 범위 항목 끝에 `(근거: ...)`가 있는지 확인한다. 근거가 없는 항목은 `$sd-inner-clarify`로 해소한 뒤 근거를 붙여 Feature에 남긴다. 사용자가 제외한 항목만 Feature에서 제거한다.
...
- 애니메이션 효과 — 사유: 범위 초과 / 출처: 사용자 답변 / 재검토 조건: 별도 UX 개선 Feature 요청 시
```
...
- 사유에는 Goal 미연결, 사용자 명시적 제외, 범위 초과 중 하나만 사용한다.
