    ...
    create(context) {
        ...
        return {
            "ClassDeclaration, ClassExpression"(classNode) {
                ...
                if (componentDecorator === undefined)
                    return;
                // template 문자열 추출
                const decoratorExpr = componentDecorator.expression;
                const args = decoratorExpr.arguments;
                const firstArg = args[0];
                if (firstArg === undefined || firstArg.type !== "ObjectExpression")
                    return;
                const templateProp = firstArg.properties.find((p) => p.type === "Property" &&
                ...
                const protectedReadonlyFields = classNode.body.body.filter((node) => node.type === "PropertyDefinition" &&
