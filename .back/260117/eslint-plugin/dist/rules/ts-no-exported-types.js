...
export default createRule({
    ...
    meta: {
        type: "problem",
        docs: {
            description: "지정된 타입이 export API나 public 클래스 멤버에서 노출되는 것을 금지하고, 대체 타입을 안내합니다.",
        },
        schema: [
            {
                type: "object",
        ...
        messages: {
            noExportedTypes: '타입 "{{typeName}}"은(는) 공개적으로 노출되어서는 안됩니다.{{safeSuggestion}}',
        },
    },
    ...
    create(context) {
        ...
        function isBannedTypeRecursive(type) {
            ...
            function visit(t) {
                ...
                if (typeof name === "string" && bannedTypeNames.has(name)) {
                    ...
                    if (entry.ignoreInGeneric)
                        return undefined;
                    return { ban: name, safe: entry.safe };
                }
                const aliasTypeArguments = t
                    .aliasTypeArguments;
                const typeArgs = aliasTypeArguments && Array.isArray(aliasTypeArguments)
                    ? aliasTypeArguments
        ...
        function reportIfBanned(type, node) {
            ...
            if (match) {
                context.report({
                    ...
                    data: {
                        typeName: match.ban,
                        safeSuggestion: match.safe != null
        ...
        return {
            ...
            VariableDeclarator(node) {
                ...
                if (modParent === undefined || !("modifiers" in modParent))
                    return;
                const modifiers = modParent.modifiers;
                const isExported = (modifiers ?? []).some((m) => m.kind === ts.SyntaxKind.ExportKeyword);
                if (!isExported)
                    return;
                let type;
                if (node.id.type === "Identifier" && node.id.typeAnnotation) {
                    const tsType = parserServices.esTreeNodeToTSNodeMap.get(node.id.typeAnnotation.typeAnnotation);
...
//# sourceMappingURL=ts-no-exported-types.js.map