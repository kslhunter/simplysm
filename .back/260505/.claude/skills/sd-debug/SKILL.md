...
분석 모드에서는 어떤 코드 수정도 하지 않는다. 진단용 print/assert 삽입도 직접 수행하지 않으며, 필요 시 Phase 4의 검증 실험으로만 *지시*한다.
...
누락 항목별로 `sd-clarify` 지침에 따라 명확화한다.
...
분류가 모호하면 `sd-clarify` 지침에 따라 명확화한다. 한 버그가 복수 레벨에 해당할 수 있다 (예: L3 + L4).
...
필요한 정보를 한 번에 수집 요청한다. 수집된 정보를 Phase 4의 ACH 매트릭스 증거로 등록한다. 상세는 `references/repro-collab.md` 참조.
...
- **C(infer) 인정**: Phase 4 ACH 매트릭스에서 C(infer) 비중이 자연스럽게 높아짐. 매트릭스 갱신 단계마다 *추가 사용자 데이터 요청*을 트리거하여 C(infer)를 C(code)/C(doc)으로 승격시킨다
...
Phase 5에서 단일 ROOT가 아니라 **두 개 이상의 원인이 결합되어야** 발생하는 경우만 적용한다.
...
보고 출력 후, 권장/대안 중 어느 방향으로 처리할지 `sd-clarify` 지침에 따라 명확화한다. 호출 컨텍스트(예: sd-tdd Slice 실패)에서 정의한 처리 옵션이 있으면 그 옵션을 선택지로 사용한다.

### 종료

채택 방향 명확화 후 즉시 종료한다. 사용자 지시 없이 다음을 하지 않는다.
