...
- **사용자가 대화에서 사용한 언어 + 3인칭 서술**. "~할 수 있습니다", "제가 ~합니다" 같은 1·2인칭 서술 금지 → "X를 처리한다", "Y를 반환한다" 형태. "적절히", "필요에 따라" 같은 모호한 표현 금지.
...
문서 작업 완료 후, **이 패키지를 import하는 사용자가 README/Entry 진입 전에 알아야 할 "표준 API → 이 패키지 API" 대체 매핑**을 0~N건 추출하여 반환한다. 호출자(메인 세션)가 SKILL.md Step 5에서 버전 인덱스 파일(`.claude/rules/{문서 루트 디렉토리명}.md`)의 `## 표준 API → simplysm 매핑` 섹션에 반영한다.

### 추출 원천

다음 출처에서만 추출한다 (없으면 빈 배열 반환).

- 생성·갱신한 Entry 파일의 `🚫 Anti-patterns` / `🚫 흔한 실수` 섹션 중 "표준 API/외부 라이브러리 vs 이 패키지 API" 형태
- 생성·갱신한 CLAUDE.md의 Key Patterns 중 동등 매핑이 명시된 패턴
- 소스/JSDoc/테스트에서 직접 확인된 대체 규약 (예: `@deprecated use X instead`)

### 선별 기준

| 채택 | 제외 |
|------|------|
| 표준 라이브러리·외부 라이브러리·플랫폼 빌트인을 이 패키지 API로 대체하는 매핑 | 이 패키지 내부 API 간 선택 안내 |
| 사용자가 첫 코드부터 표준 API로 잘못 작성할 가능성이 큰 매핑 | 특정 옵션 조합·엣지 케이스에서만 발생하는 함정 |
| 도메인 영역(바이트·이벤트·파일 시스템 등)으로 한 줄 라벨링 가능한 매핑 | Entry 1개 안의 세부 시그니처 함정 |

### 형식

각 항목은 한 줄 문자열. `{도메인 영역}: {금지 사항} → {대안}` 형식.

- `{도메인 영역}`: 사용자 의도 언어로 1~3 단어 (예: `바이트 배열`, `이벤트`, `파일 시스템`, `시간 대기`)
- `{금지 사항}`: 표준 API/외부 라이브러리 식별자. 사용 패턴이 핵심이면 함께 표기 가능 (예: `new Promise(r => setTimeout(r, ms))`)
- `{대안}`: 이 패키지의 대체 API 식별자. 보조 안내가 필요하면 괄호로 (예: `Uint8Array (복잡한 연산은 BytesUtils)`)
...
- 0건이 정상이다. 억지로 채우지 않는다.
- 상한 없음. 단 동일 도메인 영역의 유사 항목이 반복되면 1건으로 통합한다.

## 도메인 용어 후보 수집

subagent 프롬프트의 `프로젝트 유형`이 **소비앱**일 때만 수행한다. **라이브러리**면 빈 배열을 반환한다.

호출자(메인 세션)가 SKILL.md Step 4-3에서 루트 CLAUDE.md의 `## 도메인 용어집` 섹션 갱신에 사용한다.

### 추출 원천

다음 출처에서 도메인 어휘를 수집한다.

- 화면·라우트·메뉴: 페이지 컴포넌트 이름, 라우트 path, i18n 키, template 안의 라벨 텍스트
- 엔터티·필드: 클래스/인터페이스명, 컬럼명, DB 테이블 코멘트, JSDoc 설명
- 코드 코멘트: 도메인 의미를 설명하는 코멘트 본문
- 기존 docs/README: 패키지 내 docs/, README.md, 요구사항 문서

### 분류 기준

각 용어 후보를 `.claude/rules/sd-clarify.md`의 분류 기준으로 라벨링한다.

- `VERIFIED`: 코드 코멘트·JSDoc·docs·README에 의미가 명시된 용어
- `INFERRED-High`: 화면명+필드 조합, 명확한 작명 패턴(예: `SalesOrderPage` = 매출 주문 화면)으로 의미가 분명한 용어
- `INFERRED-Medium`: 마이그레이션 원본·이전 버전 코드에 근거가 있으나 신규 의도 보장이 약한 용어
- `INFERRED-Low`: 약한 유추로 추정한 용어
- `ASSUMED`: 의미 추정 불가 또는 동음이의 충돌이 있는 용어 (예: 같은 약칭 "PO"가 두 화면에 등장)

### 충돌 탐지

다음 패턴은 `ASSUMED`로 강제 분류한다.

- 동일 키워드를 포함한 화면·엔터티가 2개 이상이면서 코드/docs에서 구분 기준이 명시되지 않은 경우 (예: `customer-po-page` + `new-po-page` → `PO` 약칭 충돌)
- 동일 약칭(머리글자)이 서로 다른 도메인에서 등장하는 경우

### 형식

각 항목은 다음 정보를 포함한 객체로 반환한다.

- `term`: 용어/약칭 그대로 (예: `"PO"`, `"고객사 PO"`, `"New PO"`)
- `classification`: 위 5단계 중 하나
- `meaning`: 추정/확정 의미 한 줄. `ASSUMED`는 `null`.
- `occurrences`: 출처 좌표 1~3개 (`packages/app/src/pages/customer-po-page.ts:1` 형식)
- `confusion_with`: 동음이의/혼동 위험이 있는 다른 용어 배열. 없으면 `null`.

### 분량

- 0건이 정상이다 (도메인 어휘가 없는 패키지). 억지로 채우지 않는다.
- 동일 `term`이 여러 위치에 등장하면 1건으로 통합하고 `occurrences`에 좌표를 합친다.
