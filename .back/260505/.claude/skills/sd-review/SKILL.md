---
name: sd-review
description: 정적 분석이 잡지 못하는 관점으로 코드를 리뷰하고 확정 이슈를 보고하는 스킬. "코드 리뷰해줘", "문제점 찾아줘", "코드 분석해줘" 등을 요청할 때 사용한다.
---

# sd-review: 코드 리뷰

코드 리뷰를 수행하고 확정 이슈 목록을 대화로 보고한다.

## Step 1: 대상 결정

인자로 경로가 지정되면 해당 경로를 리뷰한다.
인자가 없으면 대화 맥락에서 사용자가 지정했거나 직전 작업으로 특정 가능한 범위를 우선 리뷰한다.
인자도 없고 대화 맥락에서 특정 가능한 범위도 없으면 프로젝트 전체 거시 리뷰를 수행한다.

- `/sd-review src/services` → `src/services` 하위만
- `/sd-review` → 대화 맥락상 특정 가능한 범위, 없으면 프로젝트 전체 거시 리뷰
...
Step 4에서 최종 대상으로 유지된 이슈들을 보고 품질을 위해 정리한다.
...
## Step 6: (CRITICAL) 남은 이슈 명확화

Step 5를 통과한 모든 이슈(Low 포함)를 `sd-clarify` 지침에 따라 명확화한다.
**CRITICAL:** 절대 임의대로 이 Step 을 무시하지 않는다. 남은 이슈가 1개라도 있으면 무조건 수행한다.

## Step 7: 보고

Step 6에서 정리된 결과를 다음 양식으로 사용자에게 보고한다.

### 확정 이슈가 없는 경우

- 검토 범위(인자 또는 추론된 대상 경로)와 함께 `확정 이슈 없음`을 출력한다.

### 확정 이슈가 있는 경우

확정 이슈 목록을 다음 정보와 함께 출력한다:

- 리뷰 대상 경로
- 요구사항 원천 또는 WBS/Feature 문서 경로 (해당 시)
- 사용자 결정사항·제외된 이슈가 있으면 그 사유
- 각 이슈:
  - 요약
  - 위치 (`파일경로:라인번호`)
