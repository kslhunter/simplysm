...
- 체크리스트:
  ...
  - [ ] 최상위 body.md 파일이 생성되었는가
  - [ ] 추출 디렉토리에 4개의 첨부파일(pptx, xlsb, docx, xlsx)이 존재하는가
  - [ ] 각 첨부파일에 대한 재귀 추출 하위 디렉토리(attachment_N_* 폴더)가 존재하는가
  - [ ] 각 첨부파일의 재귀 추출 결과로 attachment_N_*/body.md 파일이 생성되었는가
  - [ ] 추출 결과(body.md 경로, 추출 요약)를 사용자에게 텍스트로 출력했는가
...
### 시나리오 4: 인라인 배치 — PPTX
- 사전조건: 시나리오 1의 재귀 추출 결과 중 pptx body.md가 존재
- 입력: 시나리오 1과 동일 (생성된 pptx .md 내용 검증)
- 체크리스트:
  - [ ] pptx body.md에서 각 슬라이드(`[Slide N]`) 섹션 내에 해당 슬라이드에 속하는 이미지가 마크다운 링크로 인라인 배치되어 있다
...
- 체크리스트:
  - [ ] xlsx body.md에서 이미지가 있는 시트의 경우, 해당 이미지의 anchor(셀 좌표) 근처에 이미지가 인라인 배치되어 있다
...
- 체크리스트:
  - [ ] body.md 파일이 생성되었다
  - [ ] body.md에 `[VBA Macros]` 섹션이 존재한다
  - [ ] VBA 모듈별(ThisWorkbook, Sheet1, Module2 등) 소스코드가 fenced code block으로 포함되어 있다
  - [ ] Analysis 테이블이 존재하며, AutoExec 또는 Suspicious 항목이 1개 이상 포함되어 있다
