...
def extract_recursive(file_path: Path, out_dir: Path):
    ...
    # Generate body.md index inside out_dir
...
def _generate_index_md(out_dir: Path, file_path: Path, result: dict,
    ...
    """Generate body.md inside out_dir, summarizing extraction results.
    ...
    if text:
        ...
        def replace_emb(m):
            ...
            if 1 <= idx <= len(saved_embedded):
                ...
                if emb["recursed"]:
                    stem = Path(name).stem
                    return f"> embedded: [{name}]({stem}/body.md)"
                else:
        ...
        if unreferenced_imgs or unreferenced_embs:
            ...
            for _, img in unreferenced_imgs:
                idx += 1
                lines.append(f"| {idx} | [{img['filename']}]({img['filename']}) | 이미지 | {fmt_size(img['size'])} |")
            for _, emb in unreferenced_embs:
                idx += 1
                name = emb["filename"]
                if emb["recursed"]:
                    stem = Path(name).stem
                    link = f"[{name}]({stem}/body.md)"
                else:
                    link = f"[{name}]({name})"
