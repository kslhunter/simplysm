...
function formatModelEffort(ctx: ExtensionContext): string {
  return [ctx.model?.id, currentThinkingLevel]
...
function formatWindow(
  ...
  format: "hoursMinutes" | "daysHours",
): string {
  if (!rateLimitWindow) return "?%(?)";

  const time =
    rateLimitWindow.resetAt === undefined
      ? "?"
      : formatRemainingTime(rateLimitWindow.resetAt, format);
  return `${formatPercent(rateLimitWindow.usedPercent)}(${time})`;
}
...
function hasExpiredWindow(): boolean {
  ...
  return [rateLimitState.fiveHour, rateLimitState.weekly].some((rateLimitWindow) => {
    return rateLimitWindow?.resetAt !== undefined && rateLimitWindow.resetAt <= nowSeconds;
  });
}
...
function selectWindow(
  ...
  const candidates = [snapshot.primary, snapshot.secondary].filter(
    (rateLimitWindow): rateLimitWindow is RateLimitWindow => rateLimitWindow !== undefined,
  );
  const matched = candidates.find((rateLimitWindow) =>
    matchesWindowMinutes(rateLimitWindow.windowMinutes, targetMinutes),
  );
...
async function fetchRateLimits(
  ...
  if (generation !== rateLimitRefreshGeneration || !ctxToUpdate || !isCodexModel(ctxToUpdate))
    return;
...
function buildCodexUsageHeaders(
  apiKey: string,
...
function parseUsagePayload(
  ...
  if (modelId) {
    const modelSpecific = additionalSnapshots.find(({ entry }) =>
      matchesUsageEntryModel(entry, modelId),
    );
  ...
  if (snapshot.primary || snapshot.secondary || credits)
    return { snapshot, credits, modelSpecific: false };

  const firstAdditional = additionalSnapshots[0];
  return firstAdditional
    ? { snapshot: firstAdditional.snapshot, credits, modelSpecific: false }
    : undefined;
}
...
function parseUsageWindow(value: unknown): RateLimitWindow | undefined {
  const rateLimitWindow = asRecord(value);
  if (!rateLimitWindow) return undefined;

  const usedPercent = parseFiniteNumber(rateLimitWindow.used_percent);
  if (usedPercent === undefined) return undefined;

  const limitWindowSeconds = parseFiniteNumber(rateLimitWindow.limit_window_seconds);
  const resetAt = parseFiniteNumber(rateLimitWindow.reset_at);
  const resetAfterSeconds = parseFiniteNumber(rateLimitWindow.reset_after_seconds);
