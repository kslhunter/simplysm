...
- **공유 형식**(파일 배치·루트 인덱스·신뢰도 표기·본문 내 참조·sub-section 헤더 레벨·진행 방법)·**골격**·**§10** 은 `SKILL.md` 참조.
...
루트 `spec.md` §4 의 6컬럼 구조: `ref | 분류 | 화면 | 유형 | 장치 | 파일`.
...
화면 page 헤더에 ref·장치 표기를 유지: `# [화면.화면명] 화면명 (PC)` 또는 `(PDA)`. 구현 완료 단위는 끝에 `[구현]` 을 붙인다.
...
자동 처리 page 헤더는 `# [자동 처리.처리명] 처리명` 형식으로 작성한다. 구현 완료 단위는 끝에 `[구현]` 을 붙인다.
...
공통·기반 기능 page 헤더는 `# [기반.기능명] 기능명` 형식으로 작성한다. 구현 완료 단위는 끝에 `[구현]` 을 붙인다.
