...
- **IMPORTANT**: Read, Grep, Glob 등의 도구로 수행할 일을 PowerShell 도구로 수행하지 말것. 
...
- 옵션 간 우열 판단의 근거는 결과물의 품질(안정성·정확성·정합성 등). 구현·마이그레이션 노동량은 어떤 선택에도 따라오는 부수 비용이므로 우열 판단 근거로 삼지 말 것 — 비용 정보가 필요하면 판단과 분리해 별도 제시.
    - 나쁜 예: "B 가 더 안정적이나 전환 작업이 많아 A 유지 권장" (구현 비용으로 우열을 뒤집음).
...
- assistant 가 직접 dev 서버·외부 장기 실행 프로세스를 띄우지 않음 (예: `ng serve`·`npm run dev`·`vite`·`pnpm dev`·`pnpm watch`·서버 상시 실행 등).
- 브라우저 시연·동작 확인이 필요하면 사용자에게 실행을 요청하고, 사용자가 띄워 주소를 회신할 때까지 대기.
- 종료되는 단발 명령(빌드·테스트·typecheck·lint 등)은 본 금지 대상 아님.
