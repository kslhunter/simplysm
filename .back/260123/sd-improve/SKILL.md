...
이 스킬은 plan 모드를 자동으로 감지한다. plan 모드가 활성화되어있다면, "Plan 모드 작업 흐름"을, 그렇지 않으면 "일반 모드 작업 흐름"을 따른다.

## Plan 모드 작업 흐름

1. 모든 검토 관점(logic, code, architect, test, docs)에 대해 `sd-improve-scan` 스킬을 Task(subagent_type:general-purpose)를 통해 병렬 호출 → 지적 목록 수집
   - 수집된 지적이 없으면 보고 후 종료
...
3. 병합된 지적들을 `/sd-improve-validate`에 전달 → 검증된 목록 받음
   - 검증된 지적이 없으면 보고 후 종료

4. 검증된 지적을 수정하기위한 plan을 수립한다.
   - 형식:
     ```
     [{{관점}}] #N: {{문제 제목}}({{위치}})
       - 문제: {{현재 상태}}
...
3. 검증된 지적 각각에 대해:
   - 아래 형식으로 출력 후 수정 수행:
     ...
     ```
   - 출력 후 해당 수정 작업 수행
4. 1번부터 다시 수행한다. (1, 2단계에서 지적이 없을 경우에만 종료)
...
지침:
- 테스트 코드는 검토 대상이 아님.
- 로직 정확성: 조건문/반복문 흐름이 의도대로 동작하는지, off-by-one 에러
...
- 일관성: 기존 패턴과 다른 구현, 유사 기능의 상이한 방식, 유사 구현 재사용 권장
