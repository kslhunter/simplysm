...
- **Signal-based**: Do not use RxJS for state management. Use `$signal`, `$computed`, `$effect`
...
- **input()/output()**: Do not use `@Input()`, `@Output()` decorators → use `input()`, `output()`, `model()`
- **Control flow**: Use `@if`, `@for`, `@switch` (no `*ngIf`, `*ngFor`)
- **DI**: Use `inject()` (no constructor parameter injection)
- **Icons**: Use `@ng-icons/tabler-icons` (no FontAwesome)
- **Package imports**: Import `@simplysm/sd-angular` from root only (no subfolders)
...
Implement the `ISdModal<T>` interface:
...
- `$mark()` — mark dirty on cell edit (required)
