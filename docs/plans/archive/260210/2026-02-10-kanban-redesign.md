# KanbanBoard 컴포넌트 재설계

> 작성일: 2026-02-10
> Angular `sd-kanban-board` / `sd-kanban-lane` / `sd-kanban` → SolidJS `KanbanBoard` 컴파운드 컴포넌트

## 0. 작업 방침

기존에 구현된 `KanbanBoard.tsx` / `KanbanBoardContext.ts`를 **폐기하고 처음부터 재작성**한다.
...
```
KanbanBoard            (Board 루트, KanbanBoardContext Provider)
├── KanbanBoard.Lane       (레인/컬럼, KanbanLaneContext Provider)
│   ├── KanbanBoard.LaneTitle  (슬롯: 레인 제목)
│   ├── KanbanBoard.LaneTools  (슬롯: 도구 영역)
│   └── KanbanBoard.Card       (카드, 반복)
└── ... (Lane 반복)
```
...
| #   | 기능        | 설명                                                              | Phase |
| --- | ----------- | ----------------------------------------------------------------- | ----- |
| 16  | 제목 슬롯   | `KanbanBoard.LaneTitle`로 커스텀 제목 렌더링                           | 1     |
| 17  | 도구 슬롯   | `KanbanBoard.LaneTools`로 커스텀 도구 버튼 영역                        | 1     |
...
- KanbanBoardContext / KanbanLaneContext
- KanbanBoard (Board) — Context Provider, `inline-flex` 가로 레이아웃
- KanbanBoard.Lane — 레인 컨테이너 (배경/레이아웃), LaneContext Provider
- KanbanBoard.LaneTitle / KanbanBoard.LaneTools — splitSlots로 children에서 분리
- KanbanBoard.Card — Card 래핑, `value`, `contentClass`
...
- KanbanBoard (Board) — `document:dragend` 정리, `onDrop` 발행
- KanbanBoard.Lane — 빈 레인 드롭 (dragover/dragleave/drop)
- KanbanBoard.Card — `_drag-position` 오버레이, `_drop-position` 미리보기, DnD 이벤트 체인, `draggable` 제어
- 데모 페이지 확장 (DnD)
...
Phase 3 → 4: 도구 슬롯(`KanbanBoard.LaneTools`) 구조가 있어야 Phase 4의 전체 선택 체크박스를 도구 영역 옆에 배치할 수 있음. (슬롯은 Phase 1에서 구현되므로 Phase 3~4는 독립적으로도 가능)
...
// --- 슬롯 컴포넌트 ---
// KanbanBoard.LaneTitle, KanbanBoard.LaneTools: ParentComponent (data 속성 마커만)
```
...
**배치:** 도구 슬롯(`KanbanBoard.LaneTools`) 옆, `float: left` 또는 flex 정렬
...
`KanbanBoard.LaneTitle`과 `KanbanBoard.LaneTools`는 `data-*` 속성 마커를 가진 슬롯 컴포넌트:
...
function KanbanDemo() {
  ...
  return (
    <KanbanBoard selectedValues={selected()} onSelectedValuesChange={setSelected} onDrop={handleDrop}>
      <For each={lanes()}>
        {(lane) => (
          <KanbanBoard.Lane value={lane.id} collapsible busy={false}>
            ...
            <For each={lane.cards}>
              {(card) => (
                <KanbanBoard.Card value={card.id} selectable draggable>
...
├── KanbanBoardContext.ts       — Context 정의, 타입 export
└── KanbanBoard.tsx             — Board, Lane, Card, LaneTitle, LaneTools 모두 포함
...
| Angular                        | Solid                                   | 비고                |
| ------------------------------ | --------------------------------------- | ------------------- |
| `SdKanbanBoardControl`         | `KanbanBoard`                                | Board 루트          |
| `SdKanbanLaneControl`          | `KanbanBoard.Lane`                           | 레인 컨테이너       |
| `SdKanbanControl`              | `KanbanBoard.Card`                           | 카드                |
...
| `#titleTpl` / `#toolTpl`       | `splitSlots` + `KanbanBoard.LaneTitle/Tools` | 슬롯                |
