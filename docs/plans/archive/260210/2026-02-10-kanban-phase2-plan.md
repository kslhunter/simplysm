# KanbanBoard Phase 2: DnD 구현 플랜

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** KanbanBoard 컴포넌트에 네이티브 HTML5 드래그 앤 드롭 기능을 추가한다.
...
**Step 1: KanbanBoardContext.ts에 타입과 인터페이스 추가**
...
Expected: KanbanContextValue 인터페이스가 변경되어 KanbanBoard.tsx에서 타입 에러 발생 (정상 — Task 2에서 해결)

**Step 3: 커밋**

```bash
git add packages/solid/src/components/layout/kanban/KanbanBoardContext.ts
git commit -m "feat(solid): KanbanBoard DnD Context 타입 확장"
```

---

### Task 2: Board (KanbanBoard) 확장

**Files:**

- Modify: `packages/solid/src/components/layout/kanban/KanbanBoard.tsx` (Board 부분: KanbanProps, KanbanBase)
...
git commit -m "feat(solid): KanbanBoard Board에 DnD 상태 관리 추가"
```
...
- Modify: `packages/solid/src/components/layout/kanban/KanbanBoard.tsx` (Lane 부분: KanbanLane, LaneInner)
...
```bash
git add packages/solid/src/components/layout/kanban/KanbanBoard.tsx
git commit -m "feat(solid): KanbanBoard Lane에 DnD placeholder 및 카운터 추가"
```
...
- Modify: `packages/solid/src/components/layout/kanban/KanbanBoard.tsx` (Card 부분: KanbanCardProps, KanbanCard)
...
```bash
git add packages/solid/src/components/layout/kanban/KanbanBoard.tsx
git commit -m "feat(solid): KanbanBoard Card에 DnD 이벤트 핸들링 추가"
```
...
import { Button, Icon, KanbanBoard, type KanbanDropInfo, Topbar } from "@simplysm/solid";
      ...
      <div class="flex-1 overflow-auto p-6">
        <div class="space-y-8">
                ...
                <For each={lanes()}>
                  {(lane) => (
                    <KanbanBoard.Lane value={lane.id}>
                      ...
                      <For each={lane.cards}>
                        {(card) => (
                          <KanbanBoard.Card value={card.id} contentClass="p-2">
                            {card.title}
                          </KanbanBoard.Card>
                        )}
                      </For>
          ...
          <section>
            ...
            <div class="h-[300px]">
              <KanbanBoard onDrop={handleDrop}>
                <KanbanBoard.Lane value="mixed">
                  <KanbanBoard.LaneTitle>드래그 혼합</KanbanBoard.LaneTitle>
                  <KanbanBoard.Card value={100} contentClass="p-2">
                    드래그 가능 (기본)
                  </KanbanBoard.Card>
                  <KanbanBoard.Card value={101} draggable={false} contentClass="p-2">
                    드래그 불가
                  </KanbanBoard.Card>
                </KanbanBoard.Lane>
...
주의: `KanbanDropInfo`가 `@simplysm/solid`에서 export되는지 확인. 현재 `index.ts`에 `export * from "./components/layout/kanban/KanbanBoardContext"`가 있으므로 자동 export됨.

**Step 3: 커밋**

```bash
git add packages/solid-demo/src/pages/data/KanbanPage.tsx
git commit -m "feat(solid-demo): KanbanBoard 데모에 DnD 동작 연결"
```
