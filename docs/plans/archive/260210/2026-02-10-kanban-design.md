...
> 작성일: 2026-02-10
> 마이그레이션 항목: #6 KanbanBoard (Board + Lane)

## 컴포넌트 구조

```
KanbanBoard (Board 루트)
├── KanbanBoard.Lane (레인/열)
│   ├── KanbanBoard.LaneTitle (슬롯: 레인 제목)
│   ├── KanbanBoard.LaneTools (슬롯: 레인 도구 버튼)
│   ├── KanbanBoard.Card (카드)
│   └── ...
└── ...
```
...
| 컴포넌트           | 역할                                        |
| ------------------ | ------------------------------------------- |
| `KanbanBoard`           | Board 루트. DnD/선택 상태 관리              |
| `KanbanBoard.Lane`      | 레인(열). 접기/펼치기, busy, 카드 드롭 영역 |
| `KanbanBoard.LaneTitle` | 레인 헤더 제목 슬롯                         |
| `KanbanBoard.LaneTools` | 레인 헤더 도구 버튼 슬롯                    |
| `KanbanBoard.Card`      | 개별 카드. 드래그/선택 가능                 |

### Context

- `KanbanBoardContext` — Board가 제공. 드래그 중인 카드, 선택 값 목록, drop 핸들러
...
1. 카드 드래그 시작 → `KanbanBoardContext`에 드래그 중인 카드 값 저장
...
```tsx
<KanbanBoard onDrop={(info) => handleDrop(info)} value={selectedCards()} onValueChange={setSelectedCards}>
  <For each={lanes()}>
    {(lane) => (
...
├── KanbanBoard.tsx          # Board + Card + Lane + 슬롯 컴포넌트
├── KanbanBoardContext.ts    # KanbanBoardContext, KanbanLaneContext
└── (index.ts에 export 추가)
```
