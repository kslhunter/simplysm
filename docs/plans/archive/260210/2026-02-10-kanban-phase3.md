# KanbanBoard Phase 3: 레인 부가 기능 (접기/펼치기 + Busy) 구현 플랜

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** KanbanBoard Lane에 접기/펼치기(collapsible/collapsed)와 Busy 상태(BusyContainer) 기능을 추가한다.
...
- KanbanBoard 컴포넌트: `.worktrees/kanban-redesign/packages/solid/src/components/layout/kanban/`
...
- Modify: `.worktrees/kanban-redesign/packages/solid/src/components/layout/kanban/KanbanBoard.tsx:144-147`
...
- Modify: `.worktrees/kanban-redesign/packages/solid/src/components/layout/kanban/KanbanBoard.tsx`
...
```bash
git add packages/solid/src/components/layout/kanban/KanbanBoard.tsx
git commit -m "feat(solid): KanbanBoard.Lane 접기/펼치기 기능 구현"
```
...
```bash
git add packages/solid/src/components/layout/kanban/KanbanBoard.tsx
git commit -m "feat(solid): KanbanBoard.Lane busy 상태 (BusyContainer bar) 구현"
```
...
<section>
  ...
  <div class="h-[400px]">
    <KanbanBoard onDrop={handleDrop}>
      <KanbanBoard.Lane value="collapsible-lane" collapsible>
        <KanbanBoard.LaneTitle>접을 수 있는 레인</KanbanBoard.LaneTitle>
        <KanbanBoard.LaneTools>
          <Button size="sm" theme="primary" variant="ghost" class="size-8">
            <Icon icon={IconPlus} />
          </Button>
        </KanbanBoard.LaneTools>
        <KanbanBoard.Card value={200} contentClass="p-2">
          카드 A
        </KanbanBoard.Card>
        <KanbanBoard.Card value={201} contentClass="p-2">
          카드 B
        </KanbanBoard.Card>
      </KanbanBoard.Lane>

      <KanbanBoard.Lane value="busy-lane" busy>
        <KanbanBoard.LaneTitle>Busy 레인</KanbanBoard.LaneTitle>
        <KanbanBoard.Card value={210} contentClass="p-2">
          로딩 중...
        </KanbanBoard.Card>
      </KanbanBoard.Lane>

      <KanbanBoard.Lane value="both-lane" collapsible busy>
        <KanbanBoard.LaneTitle>접기 + Busy</KanbanBoard.LaneTitle>
        <KanbanBoard.Card value={220} contentClass="p-2">
          접어도 로딩 바가 보임
        </KanbanBoard.Card>
      </KanbanBoard.Lane>
    </KanbanBoard>
...
git commit -m "feat(solid-demo): KanbanBoard Phase 3 데모 (접기/펼치기 + Busy) 추가"
```
...
**Step 2: 브라우저에서 KanbanBoard 데모 페이지로 이동**
